...
int accept_object(object who, object ob,object me)
{
	...
	who->add("job_time/日月复兴", 1);
	
	total_job_times=who->query("job_time/日月复兴")+who->query("job_time/雪山");
	
	if (random(who->query("job_time/日月复兴"))>4000 
	   ...
	   && random(who->query("combat_exp"))>8000000
	   && random(10)<5
	   && total_job_times % 50 == 0 )
	{
		...
		log_file("quest/xixing",sprintf("%8s%-10s日月复兴任务时，去梅庄，日月：%4d，雪山：%4d，福：%2d。\n",
			who->name(1),"("+capitalize(me->query("id"))+")",who->query("job_time/日月复兴"), 
			who->query("job_time/雪山"), who->query("kar")),who);
	}   
...
{
       int count,exp,pot,total_job_times;
	...
	if (random(me->query("job_time/日月复兴"))>4000 
	   ...
	   && random(me->query("combat_exp"))>8000000
	   && random(10)<5
	   && total_job_times % 50 == 0 )
	{
		...
		log_file("quest/xixing",sprintf("%8s%-10s日月复兴任务时，去梅庄，日月：%4d，雪山：%4d，福：%2d。\n",
			me->name(1),"("+capitalize(me->query("id"))+")",me->query("job_time/日月复兴"), 
			me->query("job_time/雪山"), me->query("kar")),me);
	}   							
