...
int accept_object(object who, object ob,object me)
{
       ...
       switch (type){
		...
		TASKREWARD_D->get_reward(who,"日月复兴",-1,0,0,5+random(10),0,0,0,"向问天");  
    who->clear_condition("job_busy");
	who->clear_condition("riyue_job");
    who->apply_condition("riyue_job", 10);
	who->delete_temp("ry");
	who->delete_temp("riyue_job");
	count = who->query("job_time/日月复兴");
	if (count<1) count=1;
	
	if (random(who->query("job_time/日月复兴"))>1000 && random(who->query("kar"))>25 && !who->query("quest/ry/dichuan")&& who->query("family/family_name")=="日月神教")
	{
				message_vision("$N在"+who->query("job_time/日月复兴")+"次日月复兴任务中获得向问天的认可！把你当做核心弟子看待。\n",who);
				who->set("title",YEL"日月神教嫡传弟子"NOR);
				who->set("quest/ry/dichuan",1);
	}
    if (me->query("quest/ry/dichuan"))
	{
				me->set("title",YEL"日月神教嫡传弟子"NOR);
	}
	//log_file("job/riyue",sprintf("%8s%-10s第%4d次日月复兴任务获得%3d点经验，%2d点潜能，经验：%d\n",
...
void done(object target, object me, mapping quest, int i)
{
       ...
       if(me && i >= 0){
		if(target){
			...
			TASKREWARD_D->get_reward(me,"日月复兴",-1,0,0,10+random(10),0,0,0,"向问天");  
			me->add("job_time/日月复兴", 1);
			me->clear_condition("job_busy");
	        me->clear_condition("riyue_job");
			me->apply_condition("riyue_job", 10);
			
			if (random(me->query("job_time/日月复兴"))>800&&random(me->query("kar"))>25 && !me->query("quest/ry/dichuan") && me->query("family/family_name")=="日月神教")
			{
				message_vision("$N在"+me->query("job_time/日月复兴")+"次日月复兴任务中获得向问天的认可！把你当做核心弟子看待。\n",me);
				me->set("title",YEL"日月神教嫡传弟子"NOR);
			...
			count = me->query("job_time/日月复兴");
			
			if (count<1) count=1;
			/*
			log_file("job/riyue",sprintf("%8s%-10s第%4d次日月复兴任务\n",
