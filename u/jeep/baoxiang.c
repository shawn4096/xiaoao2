...
string ask_finish()
{
	...
	if (me->query("xs/girl_super")) {
		TASKREWARD_D->get_reward(me,"雪山",-1,0,0,21+ random(5),0,0,0,this_object());//random(3)调整为6
	}
    else
...
string ask_job()
{
	...
	beauty->set("long", beauty->query("long") + "看来就是血刀老祖要求"+me->query("name")+"("+capitalize(me->query("id"))+")强抢的美女。\n");
	...
	/*
	log_file( "job/xueshan",sprintf("%8s%-10s开始第%5d次雪山任务，目标：%s，目前经验：%d\n", me->name(), "("+me->query("id")+")", me->query("job_time/雪山")+1, me->query("xs/girl_place"), me->query("combat_exp")),me);
	*/
	return "给老祖爷爷干活，速去速回。";
}
