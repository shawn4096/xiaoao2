...
string play(object me1,object me2,int team)
{
	...
	log_file("job/seawar",sprintf("¿ªÊ¼ÈÎÎñ,%s%s:%d,%s%s:%d\n",me1->name(), "("+me1->query("id")+")", me1->query("combat_exp"),me2->name(), "("+me2->query("id")+")", me2->query("combat_exp") ),me1,me2);
...
void flee(object me,int idx,int team)
{
	...
	if(g > 0) {
		...
		if(team == 1) {
			if(objectp(me1)) me1->set("job_name","´óÇå¹úº£Õ½");
			if(objectp(me2)) me2->set("job_name","´óÇå¹úº£Õ½");
	...
	if(g <= 0) {
		...
		log_file("job/seawar",sprintf("ÈÎÎñÊ§°Ü,t:%-4d,%s%s,%s%s\n",time()-sea[idx]["starttime"],me1->name(), "("+me1->query("id")+")", me2->name(), "("+me2->query("id")+")" ),me1,me2);
		return;
	}

	utime = time() - sea[idx]["starttime"];
	if(utime < 600) exp = g * 4/15;
	else if(utime < 660) exp = g * 3/5;
	else if(utime < 720) exp = g * 7/15;
	else if(utime < 780) exp = g * 13/30;
	else if(utime < 900) exp = g * 17/30;
	else if(utime < 1020) exp = g * 19/30;
	else if(utime < 1200) exp = g * 7/10;
	else if(utime < 1500) exp = g * 13/15;
	else exp = g;
	
	if(objectp(me1)) {
		if(team == 1) {
			if( me1->query("family") && me1->query("family/family_name") != "ÌÒ»¨µº")
				shen = 1;
			else
				shen = 0;
			TASKREWARD_D->get_reward(me1,"´óÇå¹úº£Õ½",shen,0,0,exp,0,0,0,"Ê©ÀÅ");
		}
		else
		{
			if( me1->query("family") && me1->query("family/family_name") != "ÌÒ»¨µº")
				...
				shen = 0;
			TASKREWARD_D->get_reward(me1,"ÉñÁúµºº£Õ½",shen,0,0,exp,0,0,0,"Â½¸ßÐù");
		}
	}
	if(objectp(me2)) {
		if(team == 1) {
			if( me2->query("family") && me2->query("family/family_name") != "ÌÒ»¨µº")
				shen = 1;
			else
				shen = 0;
			TASKREWARD_D->get_reward(me2,"´óÇå¹úº£Õ½",shen,0,0,exp,0,0,0,"Ê©ÀÅ");
		}
		else
		{
			if( me2->query("family") && me2->query("family/family_name") != "ÌÒ»¨µº")
				...
				shen = 0;
			TASKREWARD_D->get_reward(me2,"ÉñÁúµºº£Õ½",shen,0,0,exp,0,0,0,"Â½¸ßÐù");
		}
	}
	/*
	if(!objectp(me1)) me1 = me2;
	...
	log_file("job/seawar",sprintf("ÈÎÎñÍê³É,t:%-4d,g:%-3d,e:%-4d(%4d;%4d;%4d),p:%-3d,s:%-5d,%s%s,%s%s\n",time()-sea[idx]["starttime"],g,exp,e,limit,e-limit,pot,shen,me1->name(), "("+me1->query("id")+")",me2->name(), "("+me2->query("id")+")" ),me1,me2);
	*/
}
