...
void done(object me)
{
	...
	log_file("job/fenyou",sprintf("%-18s 做星宿JOB，得经验 %d，潜能 %d。\n",me->query("name")+"("+getuid(me)+")",me->query_temp("quest/exp_reward"), me->query_temp("quest/pot_reward")),me);
...
int ask_hgdf() 
{
	...
	{
		command("?");
		if (me->query("title")!="星宿派化功大法传人")
		{
			me->set("title",HIB"星宿派化功大法传人"NOR);
		}
	...
	if(me->query("combat_exp")<9400000)
	{
		command("look "+me->query("id"));
	...
	if(i==3&&random(me->query("kar"))>=27)
	{
		...
		log_file("quest/hgdf", sprintf("%8s(%8s) 失败%d次彻底领悟化功大法的绝技奥秘。i取值：%d|经验：%d。\n", me->name(1),me->query("id"), me->query("quest/hgdf/fail"),i,me->query("combat_exp")) );
	}
	else
	{
