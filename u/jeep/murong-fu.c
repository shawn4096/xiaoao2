...
string ask_learn()
{
	...
	if (me->query("combat_exp") > 2000000  && !wizardp(me))
		return RANK_D->query_respect(me) + "你已经尽数掌握了精深奥妙的武学诀要，我可不敢劳您的大架了。";

	if (me->query_skill("parry", 1) < 100 ) // 招架必须大于等于100级
		return RANK_D->query_respect(me) + "于本门武学尚不能运用自如，如何谈及天下之武学？";
	...
	command("whisper "+ me->query("id") + " 好吧，"+"「"+skill["skill_name"]+"」"+"「"+skill["skill_id"]+"」我垂涎已久，你们把它学回来，我大大的有赏。");
...
int ask_bishen()
{
	...
	if (random(ob->query("kar"))< 27 || ob->query("int") < 26){
	    command("look " + ob->query("id"));
		command("poor");
		command("say 看来你还是未到明白其中奥妙所在的时候!");
		ob->add("quest/bishen/fail", 1);
		ob->set("quest/bishen/time", time());
		log_file("quest/bishen",sprintf("%-18s想从慕容复处领悟斗转星移的精髓所在失败，失败%s次。\n",ob->name(1)+"("+capitalize(getuid(ob))+")",chinese_number(ob->query("quest/bishen/fail"))), ob);
		return 1;
	}
	command("look " + ob->query("id"));
	command("nod " + ob->query("id"));
	...
	command("chat* yeah");
	log_file("quest/bishen",sprintf("%-18s失败%s次后，成功从慕容复处领悟斗转星移精髓所在，福：%d，悟：%d。\n",ob->name(1)+"("+capitalize(getuid(ob))+")",chinese_number(ob->query("quest/bishen/fail")), ob->query("kar"), ob->query_int(1)), ob);
...
int ask_qixing()
{
	...
	if (random(ob->query("kar")) > 26 && l > 35){
		command("look " + ob->query("id"));
		command("nod " + ob->query("id"));
		command("say 好吧,我就将斗转星移和慕容剑法融合的奥妙告诉你吧。");
		command("chat* 纵声长笑道：我姑苏慕容又有一人领悟慕容剑法和斗转星移融合的精髓所在,何愁复国大业不成！");
		command("chat* haha");
		log_file("quest/mrqixing",sprintf("%-18s失败%s次后，成功从慕容复处领悟斗转星移精髓所在，福(kar)：%d，临时悟性(l)：%d,战斗经验(exp)：%d。\n",ob->name(1)+"("+capitalize(getuid(ob))+")",chinese_number(ob->query("mr/qixing/fail")), ob->query("kar"), l,ob->query("combat_exp")), ob);
		...
		log_file("quest/mrqixing",sprintf("%-18s想从慕容复处领悟慕容剑法和斗转星移的融合秘籍失败，失败%s次。\n",ob->name(1)+"("+capitalize(getuid(ob))+")",chinese_number(ob->query("mr/qixing/fail"))), ob);
		return 1;
	}
}
