...
int ask_yjj()
{
   ...
   if (me->query("family/family_name")!="少林派")
	...
	{
	   command("say 你不是已经习得我少林寺最高武技么，为何还来如此骚扰老僧？\n");
	   me->set("title",HIR"少林派"HIC"易筋经神功传人"NOR);
	   return 1;	   
	}
	if (time()-me->query("yjj/time"))
	{
	   command("say 你来的也太勤快耶？\n");
	   return 1;	   
	}
   if (me->query_temp("yjj/givemask") && present("mian ju",environment(me)))
   {
      command("sigh ");
    ...
    if (!me->query_temp("yjj/menghuan"))
     {
	  ...
	  command("say 有道是，修佛即为修心，修心即为修身，又有谁能明白时”时常勤拂拭，勿使惹尘埃“的真意呢?。\n");
	  return 1;
