...
void kill_ob(object ob)
{
   ...
   && (!me->query_temp("special_die") || !ob->query_temp("special_die")))
	...
	//if (userp(me) && userp(ob)) {
		...
		&& strsrch(file_name(environment(me)),"/cmds/leitai") != 0 ){//去掉 /sjsz 嘿嘿
...
void reset_action()
{
	...
	else if ( sizeof(prepare) == 2)
	{
		...
		{
			if (query("env/主攻") == (keys(prepare))[i]) 
				type=(keys(prepare))[i]; 
		}
		}     				
