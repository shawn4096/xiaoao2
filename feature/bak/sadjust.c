...
*/
	...
	if (sizeof(skl)) {
		...
		switch (ob->query("family/family_name")) {
			...
			case "少林派":
                if (skl["buddhism"] > 80) {
					ob->add("max_jing",(skl["buddhism"]/2-10)*(skl["yijing-jing"]/10));
					ob->add("max_qi",(skl["buddhism"]/2-10)*(skl["yijing-jing"]/10));
			    }
				if (skl["buddhism"] > 150 && skl["yijin-jing"] > 350)
			...
			case "古墓派":
				...
				if (skl["suxin-jue"] > 150 && skl["yunu-xinjing"]>350)
				{
					...
					ob->add("max_jing",(skl["suxin-jue"]/2-40)*10);
			...
			default:
				...
				//百姓神照增加气和精血
				if( skl["shenzhao-jing"] ){
					ob->add("max_qi", skl["shenzhao-jing"] * 6);
					ob->add("max_jing", skl["shenzhao-jing"] * 6);
				}
			break;
		}
	}
// 加临时效果
...

 {
        ...
        zs_armor = total*3/2+random(10);
		zs_defense = total*3/2+random(10); //新增defense ,强调参悟后防御
