...
//	add("mud_time", (int)gap/2);	// 此为实际连线时间 yuj@SJ 2001-09-26 减少岁数调整为gap/2 
	add("mud_time", (int)gap/3);	// 此为实际连线时间 让玩家看起来更年轻些/3
	if (query("mud_age") / 6 > query("combat_exp"))	// 经验增长慢的年龄增长也慢 yuj@SJ 2001-09-26
		add("mud_age", -gap/2);
	
	//minute = add("mud_age", gap) / 60;
	minute = add("mud_age", gap) / 180;//减少岁数增长by sohu again 120->180
