...
void create()
{
        set_name (HIY"神眼"HIW"(年卡以上专供)"NOR, ({ "shenyan","eye" }));
...
int do_check(string arg)
{
		...
		if (!me->query("y-card-vip")&&!me->query("buyvip"))  return notify_fail("你还不是年卡以上等级的贵宾，暂时不能使用这个物品。\n");
		if (!restrict()&& !me->query("buyvip")) {
