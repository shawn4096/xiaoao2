...
void create()
{
	set_name (HIM"贵宾卡(荣誉终身贵宾卡)"NOR, ({ "vipcard"}));
	set("long","这是一张书剑专用的"HBRED"荣誉终身贵宾充值卡"NOR"，他可以用来给书剑的玩家进行充值("HIY"credit vip"NOR")。\n");
...
int do_credit(string arg)
{
	...
	me->set("vip/vip_time",5*365*24*3600);
	me->add("SJ_Credit",50000);
	me->add("balance",20000000);
	//me->add("balance",20000000);
	//me->set("y-card-vip",1);
	write(HIY"\n你将"HIW"荣誉终身贵宾充值卡"NOR+HIY"往书剑自动收款机里一刷\n"NOR);
