...
string ask_finish()
{
        object me = this_player(), *team = me->query_team();
        object xy_letter;
        int i,j,z,exp,shen,pot,SJCredit,gold,gold2;
        ...
        {         
                ...
                {	   
		          j = team[i]->query_temp("was_job/floor");
                        exp = (j*300 + (j-1)*j*80 + random(100))*z/250;
                     if(wizardp(me)){
		          ...
		          team[i]->set("job_name", "Íò°²ËÂÓª¾È");
                        ...
                        "µã[1;34mÇ±ÄÜ[43;1mµÄ½±Àø¡£\n"+HIR+"\n[45;1mÍ¬Ê±¶îÍâ»ñµÃÁË"+ CHINESE_D->chinese_number(SJCredit)+"¸öÊé½£Í¨±¦\n"NOR, team[i]));
		    			...
		    			team[i]->add("SJ_Credit", SJCredit );
                                   log_file("job/was", sprintf("%s %s(%s)%d´ÎÍò°²ËÂÓª¾ÈÈÎÎñ%d²ã,µÃµ½¾­Ñé%d£¬Ç±ÄÜ%d£¬Í¨±¦%d¡£\n",
                                  ctime(time())[4..19],team[i]->name(1),team[i]->query("id"),(int)team[i]->query("job_time/Íò°²ËÂÓª¾È"),j,exp,pot,SJCredit));
        }
                                  team[i]->delete_temp("was_job");
                ...
                if (me->query_temp("was_job/asked")){
                     ...
                     tell_object(me,sprintf(HIW"Äã»ñµÃÁË"+CHINESE_D->chinese_number(exp)+
                        ...
                        "µãÇ±ÄÜµÄ½±Àø¡£\n"+HIR+"\nÍ¬Ê±¶îÍâ»ñµÃÁË"+
                        CHINESE_D->chinese_number(SJCredit)+"¸öÊé½£Í¨±¦\n"NOR, me));
                                   me->add( "potential", pot );
                                   me->add("shen", shen );
                                   me->add("SJ_Credit", SJCredit );
                                   log_file("job/was", sprintf("%s %s(%s)%d´ÎÍò°²ËÂÓª¾ÈÈÎÎñµÃµ½¾­Ñé%d£¬Ç±ÄÜ%d£¬Í¨±¦%d¡£\n",
                                  ctime(time())[4..19],me->name(1),me->query("id"),(int)me->query("job_time/Íò°²ËÂÓª¾È"),j,exp,pot,SJCredit));
