...
int do_eat(string arg)
{
	...
	if((me->query_condition("medicine") > 0) && (me->query("combat_exp")>150000)) //renlai设置 150k以内药物无cd
	...
	if( worktime>0 && stringp(me->query_temp("drug_apply/name")) && (me->query("combat_exp")>150000))
		return notify_fail("你上次服用的"+me->query_temp("drug_apply/name")+NOR"药劲儿还没过呢，等会再服用吧。\n");
	
	lvl = query("lvl");
	busy = DRUGS[yao]["busy"];
	if( busy > 20 )
		me->apply_condition("medicine",busy);
	else {
