...
string ask_limit();
int ask_liaoshang();
nosave string yao_path = "/clone/zhiye/yao";
...
void create()
{
	...
	*/
	...
	carry_object("/clone/zhiye/yaochui")->wield();
	add_money("coin",99);
}
//for newbie
int ask_liaoshang()
...
string ask_job()
{
	...
	if (j)
		command("whisper "+me->query("id")+" 快帮我把丹药送给"+me->query("worker/djob_target")+"，他在"+me->query("worker/djob_oroom")+"周围方圆"+CHINESE_D->chinese_number(me->query("worker/djob_far"))+"里之内。" );
	else
		command("whisper "+me->query("id")+" 快帮我把丹药送给"+me->query("worker/djob_oroom")+"的"+me->query("worker/djob_target")+"。");
...
string ask_reward()
{
	...
	tell_object(me, HIC"药房掌柜仔细向你讲解提炼和炼丹的诀窍，你的提炼技术提升了"+CHINESE_D->chinese_number(improve1)+"点，你的炼丹技术提升了"+CHINESE_D->chinese_number(improve2)+"点，你获得了"+CHINESE_D->chinese_number(exp)+"点经验！\n"NOR);
