...
int do_answer(string arg)
{
	...
	if( me->query("combat_exp") > 4000000 && !random(me->query("kar")) && !random(me->query("per")) && !random(me->query("pur")) )
		i = 4;        	
	else if( me->query("combat_exp") > 3000000 && random(me->query("kar")) > 20 && random(20) > 15 )
		i = 3;
	else if(  me->query("combat_exp") > 1000000 && random(me->query("kar")) > 10 && random(15) > 11 )
		i = 2;
...
在出售自己制造的武器之外，他也做收购兵器的生意。");
	...
	carry_object("/clone/zhiye/tiechui")->wield();
	add_money("coin",99);
...
string ask_job()
{
	...
	if (j)
		command("whisper "+me->query("id")+" 快帮我把宝剑送给"+me->query("worker/job2_target")+"，他在"+me->query("worker/job2_oroom")+"周围方圆"+CHINESE_D->chinese_number(me->query("worker/job2_far"))+"里之内。" );
	else
		command("whisper "+me->query("id")+" 快帮我把宝剑送给"+me->query("worker/job2_oroom")+"的"+me->query("worker/job2_target")+"。");
