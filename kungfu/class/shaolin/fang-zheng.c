	...
	&& file_name(environment()) == "/d/hengshan/baiyunan") {
		...
		for(i=1;i<=teams;i++) {
			...
			if(ob1 && present(ob1)) {
                ...
                exp=exp+exp;
				exp =exp*9/10;//增加人数，下调exp
				exp=exp/2;

				if(ob1->query("buyvip") )
					exp = exp + exp * 20/ 100;
				else if(ob1->query("y-card-vip"))
					exp = exp + exp * 10 / 100;
				else if(ob1->query("m-card-vip"))
					exp = exp + exp * 5 / 100;

				if (me->query("job_time/少林救援")>2000)
				 exp=exp/5;
				...
				//增加了双倍奖励控制内容
				/*
				if( ob1->query("relife/exp_ext") ){ 
					ext = ob1->query("relife/exp_ext");
					ob1->add("combat_exp", exp * ext /100 );
					tell_object(ob1,HBGRN"你仔细回想着海战的过程，又额外增加了"+chinese_number(exp*ext/100)+"点实战经验！\n"NOR);
				}
				if( ob1->query_condition("db_exp") ) {   
					ob1->add("combat_exp", exp);
					tell_object(ob1,HBGRN"双倍经验奖励期间，你额外地增加了"+chinese_number(exp)+"点实战经验！\n"NOR);
				}
*/
				...
				&& random(ob1->query("job_time/少林救援")) > 1000
				&& ob1->query("combat_exp") > 5000000
				&& random(ob1->query("combat_exp")) > 2000000
