...
int ask_cangjinglou()
{
	...
	{
		return notify_fail("你不是我少林弟子，来此作甚？\n");

	}
	if ( me->query_skill("yijin-jing",1)<100)
	{
		return notify_fail("你的易筋经不足100，少林武技全靠这易筋经支持，你去了也学不了什么.\n");

	}
	book=new("d/shaolin/obj/allow-letter");

	book->move(me);
	message_vision("\n$N拿出一个手谕，交给$n后说道，“你如此辛苦修炼我少林易筋经，老衲就特许你上藏经楼去遍览群书。”\n");
	return 1;
...
int ask_job()
{
	...
	if (sizeof(me->query_team()) < 2 && (me->query("family/family_name") != "少林派" )) {
...
int ask_yjj()
{
   ...
   if (me->query("family/family_name")!="少林派")
	{
	   command("say 你不是我少林派，打听易筋经，所为何来？\n");
	   return 1;	   
	}
    if (me->query("yjj/powerup"))
	{
	   command("say 你不是已经解开了我大少林最高武技么，为何还来如此骚扰老僧？\n");
	   me->set("title",HIY"易筋经神功传人"NOR);	  
	   return 1;	   
	}
	if (time()-me->query("yjj/time")<86400)
	{
	   command("say 你来的也太勤快耶？\n");
	   
	   return 1;	   
	}
   if (me->query_temp("yjj/givemask") && present("mian ju",me))
   {
      command("sigh ");
	  command("say 看来你已经明白了这当中一些恩怨情仇，我佛门本是淡泊名利，但却又是何其难也。\n");
	  command("say 易筋经乃是我少林第一武技，可以说整个少林的武技都是从易筋经中所化而来。\n");
	  command("say 易筋经非核心弟子不传。\n");
	  command("think");
	  me->delete_temp("yjj/givemask");
	  me->set_temp("yjj/askci",1);	  
	  return 1;
   }
   else {
	   command("say 你找老衲有何事？\n");
...
int ask_mh()
{
   ...
   if (!me->query_temp("yjj/askci"))
   {
	   command("say 你想干什么？\n");
	   return 1;	   
	}
      command("smile ");
	  command("say ”一切有为法，如梦幻泡影，如露亦如电，当作如是观。。“\n");
	  command("think");
	  me->delete_temp("yjj/askci");
	  me->set_temp("yjj/menghuan",1);	  
	  return 1;
    ...
    if (!me->query_temp("yjj/menghuan"))
     {
	  ...
	  command("say ”一切有为法，如梦幻泡影，如露亦如电，当作如是观。。“\n");
	  command("think");
	  me->delete_temp("yjj/menghuan");
	  me->set_temp("yjj/rushi",1);
	  command("say 也罢，既然你有此心，那我就送你一场因缘，能否获得就看你的机缘了。\n");
	  command("say 最近少林寺藏经阁那面铜镜可是好久没有人擦拭了，不知道谁能勤快点。\n");
	  command("say 有道是，修佛即为修心，修心即为修身，又有谁能明白时”时常勤拂拭，??谷浅景！暗恼嬉饽?。\n");
	  return 1;
