...
int do_beidouzhen(string arg)
{
	...
	if ( me->query_skill_mapped("sword")!="quanzhen-jianfa" 
		...
		return notify_fail("需要激发全真剑法和招架才能施展「天罡北斗阵」。\n");
	if ( !weapon||weapon->query("skill_type")!="sword" )
		return notify_fail("你手中无剑，不能施展「天罡北斗阵」。\n");
...
int do_beidou(string arg)
{
	...
	if ( me->query_skill("quanzhen-jianfa",1) <60 || 
...
void check_leaving(object me)
{
	...
	if (userp(me) && me->query_temp("beidou/ticktime"))
	{	
		do_victory(me);
		//只计算一次就可以
		me->delete_temp("beidou/ticktime");
	}
...
void do_victory(object me)
{
	int t,exp,pot,shen,jobs;

	if (!me) return;
	t=time()-me->query_temp("qz/jobtime");
	exp=me->query_temp("qz/jobexp");
	pot=me->query_temp("qz/jobpot");
	if( (int)me->query("potential") < me->query_potential_limit() )
		me->add("potential", pot);
	else me->set("potential", me->query("max_pot"));
	//每回合加点数
	...
	message_vision(HBGRN+HIW"$N本次大阵演练，共耗时"+chinese_number(t)+"秒，共获得"+chinese_number(exp)+"经验和"+chinese_number(pot)+"点潜能！\n"NOR,me);
		//带出附属条件，，
		//大道诀参悟条件
		//练阵次数增加，
	me->add("job_time/全真练阵",1);
	//me->add("party/gongji",100);
	me->set("job_name","全真练阵");
//25的倍数,后天悟性40
	jobs=me->query("job_time/全真练阵");

	if (random(me->query("job_time/全真练阵"))>1000
			&& random(me->query("kar"))>26
			&& random(me->query_int(1))>40
			&& !me->query("quest/qz/qzjf/jianjue/pass")
			&& jobs % 25 ==0	
			&& me->query("quest/qz/dichuan"))
	{
			
				message_vision(HIC"赵志敬在远处对$N竖起了大拇指，说道，“你如此辛苦演练天罡北斗阵，真是不错”！\n"NOR,me);
				command("say "+me->query("name")+"，我就将全真剑法中关于七星剑诀的信息透漏给你！\n");
				command("chat* thumb "+me->query("id"));
				command("rumor "+me->query("name")+"机缘巧合，在练阵过程中成功领悟七星剑诀！\n");
				//command("chat "+me->query("name")+"成功领悟七星剑诀！\n");
				me->set("quest/qz/qzjf/jianjue/pass",1);/**/
	}
	return;
}
...
//奖励
	...
	if (me->query_skill("tiangang-beidouzhen",1) < me->query("max_pot")-100)
	{
		me->improve_skill("tiangang-beidouzhen", me->query_int() / 5 +
                                random(me->query("int") + me->query_skill("tiangang-beidouzhen",1) / 8));
	}
...
//	me->add( "combat_exp", bonus*3/2);
	...
	if(!random(15))
	{
		...
		//调出杀手
		if (!objectp(enemy)&& !present("jinbing tanzi",environment(me)))
		{
			enemy=new("/d/quanzhen/npc/bdattacker");
			enemy->set_temp("target",me->query("id"));
			enemy->set("type_speical",1);
