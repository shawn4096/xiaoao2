...
void create()
{
	...
	set_skill("dodge", 400);
	set_skill("wudang-quan", 400);
	set_skill("tiyunzong", 400);
	set_skill("taoism", 180);
	set_skill("strike", 400);
	set_skill("zhenshan-mianzhang", 400);
	set_skill("parry", 400);
	set_skill("literate", 100);
	set_skill("xuanxu-daofa", 400);
	set_skill("sword", 400);
...
int ask_me6()
{
	...
	if(ob->query_temp("wd/wd_job_lost") && !ob->query_temp("wd/wd_jobok") ) {
		...
		ob->add("bonus/武当",-3);
		ob->apply_condition("job_busy", 30);
		return 1;
	}
	...
	if(ob->query_temp("wd/wd_jobok") ){        	
		...
		{
			case 0..5:
				TASKREWARD_D->get_reward(ob,"武当",shen,0,0,1 + random(2),0,0,0,"宋远桥");                           
			break;
			case 6..7:
				bo = bo + 6;
				TASKREWARD_D->get_reward(ob,"武当",shen,0,0,bo,0,0,0,"宋远桥");                         
			break;
			case 8..9:
				bo = bo + 10;
				TASKREWARD_D->get_reward(ob,"武当",shen,0,0,bo,0,0,0,"宋远桥");    
		...
		//message_vision(CYN"\n宋远桥满面笑容，对$N笑道：大侠如此本事，我这里有点散碎银两不成敬意，你拿去买酒喝吧！\n"NOR,ob);
		//发点钱 by renlai
		...
		{      
			...
			command("say "+ob->query("name")+",今日是师傅九十寿辰之日,往常都是我们七个师兄弟集齐向师傅拜寿。\n");
			command("say 只是你三师兄俞岱岩前几日去南方诛灭一巨盗,迟迟不见回来。你不妨去山脚下迎迎,莫要误了时辰。\n");
		}
		return 1;
	}
	else {
...
string ask_me2()
{
	...
	for (i=0;i<sizeof(living);i++){
		...
		if( check(living[j]) ){
			...
			//这里再检测一次，否则玩家移动速度快，会改变room by renlai
			
			if(!mapp(room->query("exits")) || sizeof(room->query("exits"))<1 ||!checkroom(room) ) continue;
			...
			if( get_place(base_name(room))!="" ) {
				...
				j = 1+random(4);//改得好找些:)
				for(k=0;k<j;k++) {
					if (oroom->query("exits"))
					{
					exits = values(oroom->query("exits"));
					dirs = keys(oroom->query("exits"));
					b=0;
					b = sizeof(exits);
                    a=0;					
					a=random(b);//这里提前计算，防止溢出数组 by renlai
					t =0;
					while( a>0 && b>1 && t<10 ) {
						t++;
	...
	me->apply_condition("wd_job",20+random(10));
	...
	ob->setparty(me->query("bonus/武当")+1,me->query("max_pot")-100,me->query("combat_exp"));
	//这里加进去状态
	...
	if (strlen(str)<=1) 
	   ...
	   {
		if (objectp(ob) && objectp(me) && stringp(ob->query("title")) && stringp(me->query_temp("wd/wd_job_place")) && stringp(ob->query("name",1)))
		{
...
string ask_me3()
{
	...
	if(ob->query_temp("wd/wd_job")){
		ob->delete_temp("wd/wd_job");
		ob->delete_temp("wd/wd_job_place");
        //这里删除完成标记，避免出现杀死npc得到标记，然后放弃又能ask完成的bug。   by renlai 2016.10.03
	    ob->delete_temp("wd/wd_jobok");	
        ob->clear_conditions_by_type("wd_job");
		ob->apply_condition("wd_job", 5);//放弃，重设wd的busy by renlai
	 if (ob->query_temp("wd/ob")) 
	 {
		 destruct(ob->query_temp("wd/ob"));  //消除wd npc
		 ob->delete_temp("wd/ob");
	 }
...
string ask_me5()
{
	...
	if (strlen(str)<=1) {
		tell_object(me, GRN"宋远桥在你的耳边悄声说道：你要杀的恶贼正在\n"
			+ CHINESE_D->font(str) + GRN + "附近捣乱呢，你还不马上去巡视一圈。\n");
...
string ask_me7()
{
	...
	if (me->query_temp("quest/yttlg/asksong"))
	{
		...
		command("say 你方才不是问过我一次了么？\n");
		command("heng "+me->query("id"));
		command("say 赶紧到山下去看看，别耽误了大事。\n");
		return "这个弟子很不靠谱，现在就开始老年病了。\n";
	}
	if (!me->query_temp("quest/yttlg/story"))
		return "没看我在忙着么？";
	me->delete_temp("quest/yttlg/story");
	me->set_temp("quest/yttlg/asksong",1);
	tell_object(me,HIC"我三师弟应该快来了，赶紧到山脚下去迎接他看看，千万注意，小心歹人触摸。\n"NOR);
	return "宋远桥又嘱咐了"+me->query("name")+"一番，一副忧心冲冲的样子，似乎要发生什么事情一样。\n";
}
...
int check(object ob)
{
	...
	|| room->query("outdoors") == "梅庄"
	|| room->query("outdoors") == "武当后山"
	|| room->query("outdoors") == "蝴蝶谷"   //add by renlai for not enter zone
	|| room->query("short") == "地下黑拳市"	
	|| room->query("short") == "武僧堂"
	|| room->query("short") == "毒虫谷"
	|| room->query("short") == "观日峰"
	|| room->query("short") == "长江渡船"
	|| room->query("short") == "黄河渡船"	
	...
	|| strsrch(room_name, "/d/mingjiao/hdg/") == 0
...
int checkroom(object room)
{
	...
	|| strsrch(room_name, "/d/baituo/") == 0)
	{
	  log_file("job/wd_jobblack", room->query("short") +" "+ room->query("outdoors") +" "+ room_name +"\n");	
		return 0;
	}
	return 1;
}
