...
string ask_daodejing()
{
	...
	if (!(fam = me->query("family")) || fam["family_name"] != "武当派")
		return RANK_D->query_respect(this_player())+"讲笑话了，道德经心法乃我道家之事，你还是请回吧！！";
	if (me->query_skill("yinyun-ziqi",1)<200)
	{
		return RANK_D->query_respect(this_player())+"你的氤氲紫气太低了不足200，拿此重宝会被抢劫的！！";
	}
	if (!me->query("quest/yttlg/pass"))
	{
		return "这倚天屠龙刀中到底蕴藏什么样的秘密，五弟下山多时，至今未归，我心中甚是忧虑。\n"+
			"想要道德经可以，你先替我把倚天屠龙功学会了，我就给你。\n";
	}
	if (this_object()->query("ddj")<1)
	{
		return "这本经书我刚送人了，你待会再来。\n";
	}
	message_vision(HIY"$N长叹一口气说道，说道，“宝刀屠龙，倚天不出，谁与争锋？原来竟然是这么一回事.”\n"NOR,this_object(),me);
	command("say 好吧，看在你替我解了这个迷惑的面子上，我就给你一本祖师亲手修订的道德经全集。\n");
	command("say 这本全集乃是当初师傅修道时研习之物，虽然破旧了点，但上面有师傅修道的整个心得。\n");
	command("say 你要收好了，别弄丢了。\n");
	book=new("d/wudang/obj/daodejing");
	book->move(me);
    this_object()->add("ddj",-1);
	return "";
