...
void create()
{
        ...
        set_skill("xuanxu-daofa", 200);
        set_skill("blade", 200);
        set_skill("wudang-quan", 200);
        set_skill("parry", 200);
        set_skill("cuff", 200);
        set_skill("sword", 200);
        set_skill("taoism", 100);
		set_skill("strike", 200);
		set_skill("zhenshan-mianzhang", 200);
...
string ask_me()
{
	...
	if (query_skill("yinyun-ziqi",1) < 100)
		return "你的氤氲紫气太低，不足100，拿了也看不懂。";
	if (query_skill("taoism",1) < 100)
		return "你的道德经太低，不足100，拿了也看不懂。";
	add("book_count", -1);
