...
string ask_finish()
{
	...
	if (me->query("xs/girl_super")) {
        TASKREWARD_D->get_reward(me,"雪山",-1,0,0,19+ random(6),0,0,0,this_object());
 //random(3)调整为6
                                                          }
    else
		TASKREWARD_D->get_reward(me,"雪山",-1,0,0,9+random(5),0,0,0,this_object());//6调整为9
	...
	if ((string)me->query("family/family_name")=="血刀门"
	  && me->query_skill("huanxi-chan",1) < 200)
		  me->improve_skill("huanxi-chan", me->query_int()*2);
                //me->improve_skill("huanxi-chan", me->query_skill("huanxi-chan",1));
    
	me->delete("xs/girl_place");
	//宝象触发血刀刀法绝世武技,50的倍数,富源,先天悟性40
	total_job_times=me->query("job_time/雪山");
    i = 20;
	...
	if (random(me->query("job_time/雪山")) >5000
		&& random(i) == 4
        && random(me->query("combat_exp"))>10000000
		&& !me->query("quest/xd/xddf/pizhi/pass")
		&& !me->query_temp("quest/xd/xddf/pizhi/start")
    ...
    {
		    //me->set_temp("mb", time());
		    me->add("quest/xd/xddf/pizhi/start", 1);
			me->set_temp("quest/xd/xddf/pizhi/start",1);
		    log_file("quest/xdxsquest",sprintf("%8s%-10s第%4d次雪山任务时，获得血刀老祖指点的机会，雪山job：%4d，随机数：%4d，随机福：%2d，次：%d。\n",
			me->name(1),"("+capitalize(me->query("id"))+")",me->query("job_time/雪山"), me->query("job_time/雪山"),
			i,k,me->query("quest/xd/xddf/pizhi/start")),me);
		    command("say "+me->query("name")+"你为你老祖爷爷抢了不少美女，老祖爷爷让我通知你去找他有要事。\n");
     }
...
string ask_fail()
{
	...
	me->delete("xs/girl");
	me->delete("xs/girl_time");
	me->delete_temp("xs/finish");//修复bug
	...
	girl=me->query("xs/girl_ob");
	guard=me->query("xs/guard_ob");
	if(girl) destruct(girl);
	if(guard) destruct(guard);
	
	
	me->delete("xs/girl_ob");
	me->delete("xs/guard_ob");
...
{
	object ob;
	//if (!objectp(ob=present("bao xiang",environment(this_object()))) && !query_temp("living"))
	if ( !query_temp("living"))
	{
		ob = new("/kungfu/class/xuedao/baoxiang");
