...
void create()
{
	...
	set("eff_jingli",8000);
	set("jiali",250);
	set("quest/sld/dldf/pendu/pass",1);
	set("quest/sld/dldf/dulong/pass",1);
	set("quest/sld/dldf/wudi/pass",1);
	set("quest/sld/tlbf/beauty/pass",1);
	set("quest/sld/clgf/canglang/pass",1);
	set("quest/sld/hgmz/huagu/pass",1);

    set_skill("leg", 500);

	set_skill("shenlong-tuifa", 500);
	...
	set_skill("huagu-mianzhang", 550);
	set_skill("hand", 500);
	set_skill("chansi-shou", 500);
...
void attempt_apprentice(object ob)
{
	...
	if(ob->query("shen",1) > -2000000) {
		command("say 你还不够心黑手辣,满足了2m负神再来找我，再去多杀几个白道中人吧。");
		return;
	}
	if(ob->query_skill("dulong-dafa",1) < 350 ) {
		command("shake "+ob->query("id"));
		command("say 你毒龙大法不足350，继续努力去吧。");
		return;
	}
	if(ob->query_temp("marks/hant")) {
...
int do_canglang()
{
   ...
   if(!myfam || myfam["family_name"] != "神龙教"||myfam["master_id"] != "hong antong")
           return notify_fail("洪安通说道：“你是哪路英雄,我怎么不认识你？”\n");
 
   me->set_temp("clgf/askhong",1);
   write(HIC"这个地方很适合你去体悟这沧浪钩法的意境，我最近很忙，你先去问问夫人，让他帮你!\n"NOR);
   return 1;
}
//42章经
int do_book()
...
int do_ask()
{
   ...
   if ( me->query("job_name") == "神龙教" && me->query("combat_exp") >= 100000){
	...
	{
		command("say 你的经验值太小了，哪里敢派你去送死？\n");
		return 1;       
	}
	//随机生成任务
	//索命mapping get_quest(object me, int shen, string type, string family, string gender)

	if( random(2) == 1 ) 
	{
		write("sm1");
		if(mapp(quest=TASK_D->get_quest(me, -1, "杀", "神龙教","女性")))
		{
			write("sm11");
	...
	//招魂
	write("zh");
		if(mapp(quest=TASK_D->get_quest(me, 0, "请", "神龙教","女性")))
		{
				write("zh1");
                if(checkvaild(quest) && 
