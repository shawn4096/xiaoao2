...
void create()
{
	...
	set_skill("force", 430); // 基本内功
	set_skill("huntian-qigong", 430); // 混天气功
	set_skill("hand", 430); // 基本拳脚
        set_skill("suohou-shou", 430);
	set_skill("dodge", 430); // 基本躲闪
        set_skill("xiaoyaoyou", 430); // 千里行乞
	set_skill("parry", 430); // 基本招架
        set_skill("stick", 100);
