...
string ask_me()
{
	...
	position = (int)applicant->query("gb/bags");
	// num of bags
...
int promotion_checking(object who, object ob)
{
	...
	*/
	...
	chinese_number(who->query("gb/bags")) + "袋弟子，好好干！\n");
	who->set("title",sprintf("丐帮%s袋弟子", chinese_number(who->query("gb/bags"))) );
...
void gb_apprentice(object ob)
{
	...
	if ( prev_family_name != "丐帮")
		ob->set("gb/bags", 1);
	ob->set("title",sprintf("丐帮%s袋弟子", chinese_number(ob->query("gb/bags"))) );
}
