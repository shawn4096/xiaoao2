...
int kanglong(object me, object ob)
{
	...
	if ( random((int)me->query_con()) > 25 ){
		command("say 我也不能白吃了你的菜，看你这小子根骨还不错，这样吧。");
...
int ask_wugou()
{
	...
	if ( me->query_temp("noget_wugou") && !wizardp(me)) {
		...
		if (me->query_temp("noget_wugou") > 8) {
			//command("say 你目前的功夫太稀松，天下无狗是学不了的，看你有这份心思的份上，我就指点你几招！ ");
			//command("say 天下无狗乃是我丐帮绝世武技：既然你有心修习,那我就先传你【棒掌双绝】！ ");
			command("say 这就是打狗棒法里我丐帮中极为杀伤性的一招：【棒掌双绝】,你要好好体味！ ");
