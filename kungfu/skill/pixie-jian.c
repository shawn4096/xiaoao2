...
mapping query_action(object me, object weapon)
{
    ...
    if (me->query("quest/pixie/super2/psss")&&random(10)>7)
	{
			return ([
				"action": HBMAG+HIW"$N身形陡然加快，形如鬼魅如闪电般欺到$n"HBMAG+HIW"身前，相距不过一尺，手中$w"HBMAG+HIW"已经刺到$n"HBMAG+HIW"的$l。"+NOR,
		...
		{
			return ([
				"action": HIC+replace_string(replace_string(msg[random(sizeof(msg))], "$w", "$w"), "$w", "$w"HIM)+NOR,
	...
	skill=me->query_skill("pixie-jian",1);
		...
		if (!weapon) return;
     	if(((string)weapon->query("id")=="xiuhua zhen" 
			...
			victim->receive_wound("qi", me->query_str(1)*(1+random(100)),me);
			message_vision(HIR "只听「嗤」一声，一缕血丝从"+victim->name()+"被刺的针孔处喷出！\n" NOR,me,victim);
		}
		if (wizardp(me))
		{
			tell_object(me,"damage_bonus="+damage_bonus+"\n");
		}
		if (me->query("quest/pxj/super2/pass")&& random(me->query_skill("pixie-jian",1))>victim->query_skill("parry",1)/2&&random(10)>5)
		{
			switch (random(3))
			{
				case 0:
					message_vision(RED"$N身形陡转，围绕$n轻飘飘转了一圈，形如鬼魅，但手中的剑招却全是从不可能的角度刺向$n周身！\n"NOR,me,victim);
					message_vision(RED"$n顿时被刺出数个血口，汩汩而流，血流不止！\n"NOR,me,victim);
					victim->receive_damage("qi", damage_bonus, me);
					victim->receive_wound("qi", damage_bonus/2, me);
					break;
				case 1:
					message_vision(HBRED+HIY"$N揉身而上，连出数招，招招迅捷且诡异无比！$n攻击顿时失去方向!\n"NOR,me,victim);
					/*me->add_temp("apply/sword",skill/4);
					me->add_temp("apply/damage",skill/4);
					COMBAT_D->do_attack(me,victim, me->query_temp("weapon"),userp(me)?3:1);
					me->add_temp("apply/sword",-skill/4);
					me->add_temp("apply/damage",-skill/4);*/
					victim->add_temp("lost_attack",3+random(3));
					break;
				case 2:
					message_vision(HIC"$n觉眼前一花，似乎见到$N身形一幌，但随即又见$N回到原地，却似从未离开！\n"NOR,me,victim);
					if (!victim->query_temp("block_msg/all")&&random(me->query_dex())>victim->query_dex()/2)
					{
						message_vision(RED"$n忽觉眼前一黑，双目一阵剧痛传来，显然被$N方才刺瞎！\n"NOR,me,victim);
						victim->add_busy(2);
						victim->receive_wound("qi",damage_bonus,me);
						victim->set_temp("block_msg/all",1);
						if (objectp(victim))
							call_out("pxjcm_back",3, victim);
					}					
					break;
...
int ob_hit(object ob, object me, int damage)
{
		...
		if (me->is_busy()) 
        {
           message_vision(MAG "$N忽然一声尖叫，身形飞了起来，如一朵红云将$N的身形掩没，竟然摆脱了攻击！\n" NOR, me);
           me->start_busy(-1);
        }
