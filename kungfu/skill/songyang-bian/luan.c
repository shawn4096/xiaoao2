...
string perform_name() {return YEL"乱鞭诀"NOR;}
...
int perform(object me, object target)
{
        ...
        if( (int)me->query_skill("songyang-bian", 1) < 100 )
      ...
      //  me->add_temp("apply/dexerity", tmp);
        ...
        i = me->query_skill("songyang-bian");
       // if (wizardp(me))
        ...
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), random(3)?1:3);
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), random(3)?1:3);
		if (me->query_skill_mapped("whip")=="songyang-bian"
			...
			&&me->query_skill_prepared("hand")=="songyang-shou")
		{
					...
					weapon->unequip();
					message_vision(HIR"$N忽然将鞭子收回腰间，瞬间一掌"HIW"大嵩阳手"HIR"的掌力拍向$n！\n"NOR,me,target);
...
int help(object me)
{
        write(HIY"\n嵩阳鞭法之「乱鞭诀」："NOR"\n\n");
        write(@HELP
	嵩阳鞭法是嵩山派一门外派功法，是掌门人门下弟子邓八公
	的家传所学，后来带艺投师，进入嵩山派，经左冷禅梳理后
	此门鞭法可以作为嵩山弟子初期使用的一门武功。
	乱鞭诀就是运起浑身功力，瞬间甩动三招，形成鞭影纵横的
	趋势使得对手陷入一篇鞭影之中，无迹可寻的招式。
	
	指令：perform whip.luan
