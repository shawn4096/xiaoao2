...
{
	object wp,wp1, victim,target,*people;
	string msg;
	int i,l,p, j, skill, neili, neili1, exp, exp1, xu, qi_wound,damage2;
	...
	if (me->query("mjqkdny/pass")) me->add_temp("fanzhen", 1);
...
// yin
	...
	if( random(me->query_int(1)) > ob->query_int(1)/4
		 && random(me->query("combat_exp")) > ob->query("combat_exp")/3) {
			...
			//message_vision(HIY"$n眼见自己所施展的招式莫名其妙转了方向，竟然攻向自己，心中顿时慌乱！\n"NOR,me,ob);
			me->add("jing",-50);
			me->add("jingli",-50);
			me->add("neili",-120);
			
			//公共化学伤害
			damage2=COMBAT_D->do_chemis_attack(me,ob,1,damage/2,HIG"$N运起乾坤大挪移引字诀，$n的攻势不知如何攻向了$n自己！");		
			//ob->receive_damage("qi",damage, ob);
			//ob->receive_wound("qi",damage/2, ob);
		...
		else if (random(me->query_int(1)) > ob->query_int(1)/4
			...
			people = all_inventory(environment(me));
             ...
             {
				victim = people[i];				
                if (victim != me && victim != ob && victim->is_character() && living(victim) && !wizardp(victim)) {
				message_vision(HIR"$N见$n招式使来，乾坤大挪移随心而起，劲力顺着$n的攻势竟然指向了"+victim->query("name")+"\n"NOR,me,ob);
                //message_vision(HBRED"$N灵机一动，一牵一引，把$n拉转到身前，迎向了"+ob->query("name")+HBRED"的招式！\n"NOR,me,victim);
                ob->remove_all_enemy();
                me->remove_all_enemy();
				ob->offensive_target(victim);
				victim->offensive_target(ob);
                COMBAT_D->do_attack(ob, victim, ob->query_temp("weapon"), random(3)?1:3);
                return -damage;
				}
             }
        }				
...
// yi
	if( me->query_temp("qkdny/yi") && living(me) && !me->is_busy() ) {
        ...
        COMBAT_D->do_attack(ob, target, ob->query_temp("weapon"), 3);
		//自己打自己的反击
		if(target->query_skill("dodge") >= damage)
			...
			//化学伤害公共函数
			damage2=COMBAT_D->do_chemis_attack(ob,target,1,damage);	
			//target->receive_damage("qi", damage, ob);
			p = target->query("qi")*100/target->query("max_qi");
			msg = COMBAT_D->damage_msg(damage2, "伤害");
			msg += "( $n"+COMBAT_D->eff_status_msg(p)+" )\n";
		}
	...
	if( me->query_temp("qkdny/xu") ) {
		if( me->query_temp("qkdny/xu") > 1000
		 || me->query_temp("qkdny/xu") > me->query("neili")/4 ) {
			...
			damage2=COMBAT_D->do_chemis_attack(me,ob,1,qi_wound);	
			
			if( random(2) ) {
				p = (ob->query("eff_qi")+ob->query_temp("apply/qi"))*100/(ob->query("max_qi")+ob->query_temp("apply/qi"));
				msg += COMBAT_D->damage_msg(damage2, "震伤");
				msg += "( $n"+COMBAT_D->eff_status_msg(p)+" )\n";
			}
			else {
		...
		ob->add_busy(1+random(2));
		if( !me->is_busy() &&me->query_skill("qiankun-danuoyi",1)<450)
...
//没有各种标志时的乾坤大挪移
	if( me->query("jiali")
	 ...
	 && !ob->query("env/invisibility") ) {
		if( me->query("mjqkdny/pass")?random(5):random(2)) me->add_temp("fanzhen", 1);
		     me->add("neili",  -(random(10)));
		//双方兵器时判断
		
		if ( me->query("mjqkdny/pass")&&me->query_skill("qiankun-danuoyi",1)>450&&random(10)>6)
		{
			message_vision(HBRED+HIY"$N已经深悟乾坤大挪移第七层神功精妙，乾坤大挪移神功随心而已，将$n的攻击反击了回去。\n"HIC,me,ob);
			ob->add_busy(1);
			ob->add_temp("apply/attack",me->query_skill("qiankun-danuoyi",1)/2);
			COMBAT_D->do_attack(ob, ob, ob->query_temp("weapon"), 3);
			ob->add_temp("apply/attack",-me->query_skill("qiankun-danuoyi",1)/2);
		}		
		...
		if( wp && wp1 ) {
			...
			if( neili >= neili1*2 && !me->is_busy() ) {
				if( random(me->query_str(1)) > ob->query_str(1)/3 && skill > 150 ) {
					...
					if (ob->query("max_qi")<j) j=ob->query("max_qi");//防止反击太强 by renlai
					//化学伤害公共函数			
			       damage2=COMBAT_D->do_chemis_attack(me,ob,1,j/2);					
				   p = ob->query("qi")*100/ob->query("max_qi");
					msg += COMBAT_D->damage_msg(damage2, "伤害");
					msg += "( $n"+COMBAT_D->eff_status_msg(p)+" )\n";
					j = -5000;
			...
			else if( neili >= random(neili1)*3/2 ) {
				msg += "结果基本上卸掉了$n的力道。\n";
				j = -5000;
			}
			else{
			...
			if( neili >= neili1*2 && !me->is_busy() ) {
				...
				//化学伤害公共函数			
			      damage2=COMBAT_D->do_chemis_attack(me,ob,1,j/3);						
				//ob->receive_damage("qi", j/3, me);
				//ob->receive_wound("qi", j/6, me);
				p = ob->query("qi")*100/ob->query("max_qi");
