...
int valid_enable(string usage) { 
     if (this_player()->query("quest/tonglao/pass") || this_player()->query("family/family_name")=="逍遥派")		
...
int valid_learn(object me)
{
	if (!me->query("quest/tonglao/pass"))		
		return notify_fail("天山折梅手只能通过童姥指点后方可学习。\n");
...
{
    if (!me->query("quest/tonglao/pass")||me->query_skill("liuyang-zhang",1)<220)
                return notify_fail("只有受到童姥指点方可修习天山六阳掌。\n");
...
mixed hit_ob(object me, object target, int damage_bonus)
{
	...
	if (me->query_temp("bhg/duzun") && me->query("bhg/biguan")>=1)
		i=i+me->query_skill("bahuang-gong", 1)/2;
    if(i>=450 )
	{
		switch (random(4))
		{
			case 0:
			   message_vision(HIR"这时$N内力生发，将「逍遥」二字发挥得淋漓尽致，姿势优雅美观，潇洒如意然而逍遥派武学的诸般狠辣招数\n"
				...
				j = i*(2+random(4));
			  //if( j>=target->query("qi") )  j=target->query("qi")-1;
			...
			case 2:
				...
				target->receive_damage("qi",i*4,me);
			break;
			case 3:
				message_vision(CYN"$N微微一笑，使出天山派絕招「天山折梅手」，左手向敌人手中的武器轻轻弹出，好像是要弹去梅花上面的露珠！\n"NOR,me,target);
				message_vision(RED"$n只觉一股阴柔至极的劲力袭入自己经脉，四肢百骸如油入水中,炙热难当！\n"NOR,me,target);
				target->receive_damage("jing",i*3,me);
				//if( i>=target->query("qi") )  i=target->query("qi")-1;
