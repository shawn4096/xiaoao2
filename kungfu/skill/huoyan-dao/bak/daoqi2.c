...
int perform(object me, object target)
{
	...
	if((int)me->query_skill("huoyan-dao",1) < 350)
    ...
    if( me->query_skill_mapped("parry") != "huoyan-dao")
		return notify_fail("你必须使用火焰刀，才能使用「火焰刀气」的绝技！\n");
    ...
    message_vision(RED"\n$N左掌斜斜劈出，掌上已蓄积真力，化掌为刀，劲力弥漫，宛如实质，使出的正是“火焰刀”中的刀气绝技。\n"NOR, me, target);

	switch((int)me->query_temp("hyd/pfm")) {
		case 0:
			lvl1 = me->query_skill("huoyan-dao",1) + me->query_skill("longxiang-boruo",1)/2;
			...
			if( lvl1/2 > random(lvl2) ) {
				...
				msg += HIC"$n完全看不懂$N的刀路，只的收招紧守门户。\n却发现前后左右都被$N的刀气封死了，攻击防守都不免大受牵制！\n"NOR;
				me->set_temp("hyd/pfm",1); //准备第二招
				if (me->query_skill("huoyan-dao",1)<349)
					me->start_busy(1);					
				
				if (me->query_skill("huoyan-dao",1)<449)
				    me->start_perform(2,"火焰刀气");
				else me->start_perform(3,"火焰刀气");
				...
				msg += HIC"却不料$n完全不顾$N所劈出的刀气，奋力往中庭急进，$N只得回掌御敌。\n"NOR;
				me->start_busy(2);
				if (me->query_skill("huoyan-dao",1)<449)
				   me->start_perform(2,"火焰刀气");
				else  me->start_perform(3,"火焰刀气");
			...
			break;
		case 1:
			msg = HIR"\n$N龙象般若功随心而起，全身骨骼一阵爆响，随即又向$n的东南，东北，西南，西北连劈四刀，企图完全封死$n的去向！\n"NOR;
			lvl1 = me->query_skill("huoyan-dao",1);
			if( me->query_temp("xs/longxiang") &&  me->query_temp("apply/attack")>0 )
				lvl1 += me->query_temp("apply/attack");
			...
			if( lvl1/2 > random(lvl2) ) {
				...
				me->set_temp("hyd/pfm",2); //准备第二招
				if (me->query_skill("huoyan-dao",1)<449)
				    me->start_perform(2,"火焰刀气");
				else  me->start_perform(4,"火焰刀气");
...
void check_pfm(object me,object target,int count)
{
	if( !objectp(me) || !objectp(target) || !me->is_fighting(target) || objectp(me->query_temp("weapon")) || count==0 ||
	  me->query_skill_mapped("strike")!="huoyan-dao" || me->query_skill_mapped("force")!="longxiang-boruo" ) {
		...
		if(me && target) 
			message_vision(HIC"\n$N感觉丹田一滞，却是内力消耗过多，再也不能维持封住$n的刀气。\n"NOR, me , target);
		else 
...
int help(object me)
{
	write(HIR"\n火焰刀秘技「刀气」："NOR"\n");
	write(@HELP
       火焰刀「刀气」绝技,能提高在战斗中火焰刀的威力，乃辅助技能。
       ，
	   到龙象波若功达致大成，在战斗中运用龙象之力更是能够
	   君临天下无往不利。500级有飞跃。
	要求：	最大内力 9000 以上；
		    内力     3500  以上；
		    龙象般若功等级 350 以上；
