...
void die()
{
        ...
        if( me ) {
			...
			me->add_temp("apply/attack",-me->query_skill("xiangfu-lun",1)/5);
		    if (weapon->query("id") == "fenghuo lun" )
			{
              me->add_temp("apply/attack", -me->query_skill("xiangfu-lun",1)/10);
              me->add_temp("apply/damage", -me->query_skill("xiangfu-lun",1)/10);
			  me->add_temp("apply/hammer",-me->query_skill("xiangfu-lun",1)/10);
			}
