...
int perform(object me,object target)
{
	...
	//	if (target->query_temp("dazhuan"))
    	...
    	if ( weapon->query_amount() < 5 && weapon->query("hammer_count") < 5 && weapon->query("id") != "fenghuo lun")
    	...
    	message_vision(HIY "\n$N顺手甩出手中的"+weapon->query("name")+HIY"，立时呜呜、嗡嗡、轰轰之声大作，金光闪闪，银光烁烁，五只轮子从五个不同方位飞袭$n！\n"+
		HIY"					\n\n※※※ "HBYEL+HIR"五 轮 大 转 "NOR+HIY"※※※ \n\n"NOR, me,target);
    	target->set_temp("dazhuan",1);
		
		if (me->query_skill("xiangfu-lun", 1) < 449)
		{
			me->start_busy(1);
            me->start_perform(1 + random(3), "「五轮大转」");
		}
		...
		if( obj->move(environment(me)) )
		{
			...
			me->set_temp("buff_lvl",lvl);//保存等级，以便以后消除buff by renlai
