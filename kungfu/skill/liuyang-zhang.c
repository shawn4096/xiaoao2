...
int valid_learn(object me)
{
	...
	if (!me->query("quest/tonglao/pass"))		
		return notify_fail("天山六阳掌只能通过童姥指点后方可学习。\n");
		...
		if(level > action[i-1]["lvl"]) {
			j = NewRandom(i, 20, level/5);
				...
				return ([
					...
					"damage": t>0?120+random(100):80+random(100),
					"damage_type": "割伤",
					"dodge": random(35),
					"parry": random(35),
			...
			if ( me->query_temp("ryd/xiaoyao")){
				return ([
					...
					"damage":t>0?150+random(100):120+random(100),
					"damage_type": "割伤",
					"dodge": random(35),
					"parry": random(35),
					"force": 200+random(300),
            ...
            if ( me->query_temp("lyz/jiutian")){
				return ([
					...
					"damage": t>0?200+random(100):150+random(100),
					"damage_type": "割伤",
					"dodge": random(35),
					"parry": random(35),
					"force": 200+random(300),
		   ...
		   return ([
			"action":msg,
					"damage": t>0?250+random(100):120+random(100),
...
mixed hit_ob(object me, object target, int damage_bonus)
{
	    ...
	    j=i+me->query_skill("beiming-shengong",1);
	  //非北冥神功或八荒功不出生死符
	...
	//生死符        
		if(userp(me)
			...
			&&random(10)>3) 
      	 {
            message_vision(WHT"$N右掌运"HIR"阳刚"WHT"之气，左掌运"HIB"阴柔"WHT"之力，化水為冰，凝于掌中，以第二种法门急拍，輕飄飄地拍向$n周身大穴！\n"NOR,me,target);
      ...
      //北冥吸
		if (random(me->query_con(1))>target->query_con(1)/2
			...
			&&me->query_skill_mapped("force")=="beiming-shengong")
      	   ...
      	   {
      	   	message_vision(HIR"$n一惊之下，急运内力挣扎，突觉内力自膻中急泄而出，全身便似脱力一般，更是惶恐无己！\n"NOR,me,target);
      	   	target->add("neili",-j);
