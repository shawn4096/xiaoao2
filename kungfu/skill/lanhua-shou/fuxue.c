...
// 符合 清              
        if (me->query("eff_qi")>=me->query("max_qi") / 2)
        {
                ...
                if (me->query("gender")=="女性") cp=cp*me->query_per(1)/5;
                ...
                if( random(ap) > dp / 2 )
                {
                        ...
                        if(userp(me) && me->query("env/damage"))
							tell_object(me,WHT"你对"+ target->query("name") +"造成了"RED+random(cp)+ WHT"点攻击伤害。\n"NOR); 


						if(userp(target)&& target->query("env/damage"))
							tell_object(target,WHT""+ me->query("name") +"的攻击对你造成了"RED+random(cp)+ WHT"点伤害。\n"NOR); ;
...
int help(object me)
{
        write(HIC"\n兰花拂穴手之「兰花拂穴」："NOR"\n");
        write(@HELP
	兰花拂穴手乃是黄药师家传武技，非核心弟子不得外传。
	该绝技乃是以阴柔内力以手法方式施展，于轻柔美妙间
	伤人于无形，端的是厉害非常。女性使用有优势，和容
	貌有关系。
