...
int hit_ob(object me, object ob, int damage)
{
      if (me->query_skill_mapped("force")!="jiuyin-zhengong") return 0;
      if (random(5)>2&&me->query_temp("jiuyin/powerup"))
      {
        
		message_vision(HIR"$n被$N的九阴真气一震，内息大乱！\n"NOR,me,ob);
        ob->add("neili",-random(200));
        ob->add_busy(2);
		}
       return 0;
}
