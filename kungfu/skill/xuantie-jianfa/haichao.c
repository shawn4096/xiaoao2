...
int perform(object me, object target)
{
	...
	if ( me->query_skill("xuantie-jianfa", 1) > 250 ) {
                i = to_int(me->query_skill("xuantie-jianfa", 1)/4.0 * me->query_skill("xuantie-jianfa", 1)/275);
                j = to_int(me->query_skill("sword", 1)/4.0 * me->query_skill("sword", 1)/275);
	}
	...
	//屏蔽掉 set 玄铁剑法 汹涌 的情况 其实就是变相加强了haichao hehe,----瞎鸡巴乱扯
...
int perform2(object me,object target)
{
	...
	if( me->query("gender")=="男性" )
		p = me->query_skill("force") / 6+me->query_skill("xuantian-jianfa") / 6;
	else
...
void check_fight(object me,object target,object weapon, int p, int count)
{
	...
	if( ! me
	 ...
	 || count < 1){
		...
		target->add("neili", - random(me->query_skill("sword",1)));
