...
int is_pfm() {       return 1;}
string perform_name(){ return HIY"沾黏诀"NOR; }
...
{ 
	...
	if( userp(me) 
        ...
        if(me->query_skill("taiji-quan", 1) < 450 )
                return notify_fail("太极拳精要在于动急则急应，动缓则缓随，你太极拳功力不到，又如何能使用「沾黏诀」？\n");
...
int main(object me, object target)
{
			...
			||random(me->query_skill("force",1))>target->query_skill("dodge",1)/2)
		{
        	msg += CYN"但见这粘劲一下"RED"「粘」"CYN"在$n身上，无论$p怎么摆脱，就是脱不出"HIW"「"+str+"劲」"CYN"的包围，一个圆圈跟着\n"+"一个圆圈，大圈、小圈、平圈、立圈、正圈、斜圈，一个个太极圆圈发出，连绵不断，滔滔不绝！\n"NOR;
	            ...
	            target->receive_wound("jing", me->query_int(1)* 2+ me->query_str(1)*2 , me);
                me->start_perform(2, "「沾黏诀」");
...
int help(object me)
{
	write(WHT"\n太极拳法之"HIY"「沾黏诀」："NOR"\n");
	write(@HELP
	太极者，四两拨千斤之句，显非力胜；观耄耋御众之形，快何能为。
	立如秤准，活如车轮，偏沉则随，双重则滞。由招熟而渐悟懂劲，由
	懂劲而阶及神明。然非用力日久，不能豁然贯通焉。
	沾黏诀由太极推手练出，打斗中随敌而走，随敌而止，目的在于困敌
	极大损耗对方的精力，并有概率使得对方失去重心，无法形成有效攻
	击。该技能和臂力与战斗经验有关，且伤害与臂力与悟性有关。
	注意：若六合劲设置得当，太极拳威力倍增。

	指令：perform cuff.zhannian
