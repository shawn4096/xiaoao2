...
int is_pfm() {return 1;}
string perform_name(){ return WHT"阴阳诀"NOR; }
...
{ 
	...
	if( userp(me) 
        ...
        && me->query_skill_prepared("parry") != "taiji-jian") )
        ...
        if( me->query_skill("taiji-quan", 1) < 400 )
                return notify_fail("太极者，无极而生，阴阳之母也，你如此低级功力，又如何能使用「阴阳诀」？\n");
		 if( me->query_skill("yinyun-ziqi", 1) < 400 )
                return notify_fail("氤氲紫气功力太低，又如何能使用「阴阳诀」？\n");
        if( (int)me->query("neili", 1) < 2000 )
                return notify_fail("你现在真气不够，强行使用「阴阳诀」是发挥不了它的威力的！\n");
		if( (int)me->query("jingli", 1) < 1000 )
                return notify_fail("你现在体力不够，强行使用「阴阳诀」是发挥不了它的威力的！\n");       
...
int hurt(object me, object target, int damage, string str, string force, int jiali)
{
        ...
        if (target->query("neili") <= 1000){
        	message_vision(msg, me, target);
		    tell_object(target, "你发觉自己真气已然将近枯竭耗尽，根本无法相抗了！\n");
		...
		//反击啊，简直就是乾坤大挪移耶。。
		message_vision(BLU"$N运起氤氲紫气太极之意，瞬间阴阳转化，虚实互换，将$n的攻击反击了回去。\n"NOR,me,target);
        ...
        message_vision(HIR"$N太极之意圆转不绝，信手一击阳掌拍了出去。\n"NOR,me,target);
        COMBAT_D->do_attack(target, target, target->query_temp("weapon"),userp(me)?3:1);
...
int help(object me)
{
	write(WHT"\n太极拳法之「阴阳诀」："NOR"\n");
	write(@HELP
	太极拳经讲：太极者，无极而生，阴阳之母也。阴阳相济，
	方为懂劲。阴阳诀本身不带出手伤人之劲，但其内含阴阳，
	有力打无力，手慢让手快，是皆先天自然之能四两拨千斤
	也！故阴阳者，必拨敌之力，反制于敌。此招对付力大劲
	猛之敌最为有效，
	注意：若六合劲设置得当，太极拳威力倍增。
	
	指令：perform cuff.yinyang
