...
string exert_name(){ return HIM"氤氲内聚"NOR; }
...
int exert(object me)
{
        ...
        me->add("neili", -300);
        if (me->query_skill("yinyun-ziqi",1)< 450 ) me->start_busy(1+random(2)); //450后大飞
        return 1;
}
...
void checking(object me)
{
        if ( !me ) return;
				...
				me->add("neili", -50);
                me->receive_curing("qi", (me->query_skill("medicine",1)/3*5) + 10);
...
int help(object me)
{
	...
	英雄所向无敌，盖皆由此而及也。氤氲内聚，以
	先天一口真气护身，乃道家一绝也！450后大飞，
	取消busy状态。
