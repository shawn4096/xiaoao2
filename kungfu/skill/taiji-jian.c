...
mapping query_action(object me, object weapon)
{
	...
	if( random(50) >= 40 &&me->query("quest/wd/yyzq/taiji/pass")) me->add_temp("tjj/powerup", 1);
...
//三环
	...
	else if( me->query_temp("tjj/powerup") >= 1 )
	{
		...
		str =BCYN+HIW+msg[random(sizeof(msg))]+NOR;
		if( objectp(weapon) )
			str = replace_string(str, "$w", weapon->query("name")+BCYN+HIW);
		return ([
			"action":str,
	...
	if( me->query_temp("yyzq/taiji") || level > 350 ) 
	{
		//木质不断，太极专享
		if( weapon )
		{
...
//没有解密内功，太极剑的super出不来
	if( !userp(me) && !me->query("quest/wd/yyzq/taiji/pass") ) return 0;
...
int valid_learn(object me)
{
	...
	//if (me->query("quest/wd/tjj/pass"))
		//return notify_fail("太极剑乃张三丰晚年巅峰之作，你需要得到老人家的传承方可修习。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if( me->query("quest/wd/yyzq/taiji/pass")
		...
		if( random(me->query("int")) > victim->query("int")/3 ) {
			...
			message_vision(HIY"$N一招使出，真力运转不足，被"+weapon->name()+HIY"带着连转几个圈子，手中"+weapon1->name()+HIY"一震，险些脱手！\n"NOR, victim);
			victim->add_busy(2);
		}
	}
	else if( me->query_temp("yyzq/taiji") || i > 350 ) {
					...
					if( me->query("quest/wd/yyzq/taiji/pass")
						...
						if( random(i) > 260
						 && random(me->query_skill("sword")) > victim->query_skill("parry")/3 ) {
							...
							COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), random(5)?3:1);
							me->add_temp("apply/attack",  -i/5);
						}
						else
						if( random(i) > 250
...
int ob_hit(object ob, object me, int damage)
{
	...
	if( me->query_skill("yinyun-ziqi", 1) < 550
	 || me->query_skill_mapped("force") != "yinyun-ziqi"
	 || me->query_skill_mapped("parry") != "taiji-jian"
	...
	if( skill > 350 || me->query_temp("yyzq/taiji") ){
	...
	if( me->query_temp("yyzq/taiji") ) dp *= 2;
	//若解开太极神功，则增加反震效果
	if (me->query("quest/wd/yyzq/taiji/pass"))
	   me->set_temp("fanzhen", 2);
	else if( random(10) < 3 ) 
	...
	if( random( dp ) > ap && !me->is_busy() ) {
		...
		if( weapon ) {
			...
			if( random(me->query_skill("taiji-jian", 1)) > ob->query_skill("parry",1)/2){
