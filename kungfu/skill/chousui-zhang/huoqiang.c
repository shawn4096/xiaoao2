...
int perform(object me, string target)
{
	...
	if (wiz_level(me) < wiz_level(tar) && strsrch(file_name(environment(me)), "/cmds/leitai")!=0 )
    ...
    if (me->query("combat_exp")<5000000) //经验少于10M不许PK，防止大米干扰人家JOB！
		return notify_fail("对不起,只有大于5M才可以火墙。\n");
...
int check_pfm(object me, object target, object ob)
{
	...
	if( (i = random(ap)) > to_int(dp/2.5) ){
        ...
        if(userp(me) && me->query("env/damage"))
			tell_object(me,WHT"你对"+ target->query("name") +"造成了"RED+me->query_skill("force")*2+ WHT"点攻击伤害。\n"NOR);
		
if(userp(target)&& target->query("env/damage"))
			tell_object(target,WHT""+ me->query("name") +"的攻击对你造成了"RED+me->query_skill("force")*2+ WHT"点伤害。\n"NOR); 
		}
      	else {
