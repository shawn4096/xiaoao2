...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	int lvl = me->query_skill("hand",1);
	weapon=victim->query_temp("weapon");
	if( random(lvl) > 300 && !victim->is_lost() && random(2)) {
		message_vision(HIG"$N一阵紧密的攻击下，$n丝毫无法准确的攻击$N！\n"NOR, me, victim);
		victim->start_lost(2+random(2));
	}
	//带白金手套，则将对方武器打掉
	if(weapon && present("baijin shoutao",me) && random(10)>5) {
		message_vision(HIR"$N刷的一声带上白金手套，拿住了$n手中的"+weapon->query("name")+HIR"顺手扯飞！\n"NOR, me, victim);
