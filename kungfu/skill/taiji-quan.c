...
"$p眼见$P攻到，当即使出一招「揽雀尾」，右脚实，左脚虚，运起“挤”字诀，粘连粘随，\n右掌已搭住$P左腕，横劲发出。$N身不由主的向前一冲，跨出两步，方始站定。\n",
"不料$p双手一圈，如抱太极，一股雄浑无比的力道组成了一个旋涡，只带得$P在原地急转七八下。\n",
"$p连消带打，双手成圆形击出，随即左圈右圈，一个圆圈跟着一个圆圈，大圈、小圈、平圈、立圈、正圈、斜圈，\n一个个太极圆圈发出，登时便套得$P跌跌撞撞，身不由主的立足不稳。\n",
...
void skill_improved(object me)
{
	...
	if(skill > 100 && random(3) == 1){
		...
		me->improve_skill("parry", skill * me->query_int());
	}
...
mapping query_action(object me, object weapon)
{
	...
	parry = random(10);
//超级攻击
	if( me->query_temp("tjq/super_hit") )
		return ([
			"action" : HIG"$N一击不中，马上变招，左手捏出个剑指，" + super_msg[random(sizeof(super_msg))]+NOR,
			"force" :  500 + random(300),
			"dodge":   level,
			"parry":   level,
			"damage":  me->query_temp("yyzq/taiji")?250+random(100):200+random(50),
	...
	if( !me->query_temp("tjq/start") && me->query_temp("taiji")){
	 ...
	 else if ( me->query_temp("yyzq/taiji") || level > 350 ){
		if (random( level) > 400) {
	...
	if( objectp(victim = me->query_temp("tjq/zhannian")) ) {
		if( me->query("neili") < 400 || me->query("jingli") < 100 ) {
			...
			//添加化学函数伤害
			COMBAT_D->do_chemis_attack(me,ob,0,(me->query_int(1)+me->query_con(1) )* 2);
            COMBAT_D->do_chemis_attack(me,ob,2,(me->query_int(1)+me->query_con(1)));			
			
			//ob->receive_damage("jingli", -(me->query_int(1)+me->query_con(1) )* 2);

			me->receive_damage("neili", me->query("int"), "内力透支过度");
			me->receive_damage("jingli", me->query("int")/2, "精力透支过度");
			message_vision(RED"\n$n被$N「沾黏诀」带动，行动滞缓，汗透全身，精力和体力大为透支，简直无法出招！\n"NOR, me, ob);
	...
	if ( level > 180
	&& random(level) > 120
	&& (me->query_temp("yyzq/taiji") || level > 350 ))
		return ([
			"action" : msg,
...
{
   int taiji_damage,dm;
   string next_taiji,taiji_set;
   int cur_taiji;

   taiji_set = me->query("env/taiji");
   if (userp(victim)) dm=2;
   else dm=3;
	
	//if( me->query_temp("tjj/super_hit") ) return;
        if(me->query("combat_exp")< victim->query("combat_exp") * 1/3 ) return me->query_int(1); //以弱胜强，由2/3调成1/3
			...
			switch (cur_taiji) {	
			   ...
			   case 0: 
			      message_vision(WHT"$N缓缓念道：“"+HIW+"挤在手背，小心了！"+WHT+"”,便暗运氤氲紫气，太极"+HIW+"「挤」"+WHT+"字诀已使的出神入化！\n"+
                    ...
                    victim->add("jing",-to_int((me->query_int(1)+ me->query_con(1))* dm) );
                    victim->add_busy(1+random(2)); //busy效果
	         ...
	         case 1:
			        ...
			        me->add("neili", -300);
                   victim->apply_condition("no_exert",1+random(2)); //内功受伤，不能yun,闭气状态
                   victim->add("jing",-to_int((me->query_int(1)+ me->query_con(1))* dm) );				   me->add_temp("tjq/taiji",-1);
			     
			   break;
			  case 2:
			         message_vision(WHT"$N缓缓念道：“"+RED+"采在十指，小心了！"+WHT+"”,便暗运氤氲紫气， 顺势使出太极"+RED+"「采」"+WHT+"字诀！\n"+
					 RED"$n一被「采」到，立即感到一股罕见浑厚内劲袭来，$n哪里抵御的住，一口鲜血随即喷了出来！\n"NOR,me,victim);
			         taiji_damage = to_int(me->query_skill("taiji-quan",1)*((int)me->query_con()+(int)me->query_str())/10);
			         taiji_damage += random(taiji_damage);
			    if (wizardp(me)) write("damage is: "+ taiji_damage +"!\n");
                   //victim->receive_damage("qi", taiji_damage*4/5,me);
				   //化学攻击函数
				   COMBAT_D->do_chemis_attack(me,victim,1,taiji_damage*4/5);				   
			       message_vision(damage_msg(taiji_damage, "内伤")+"( $n"+eff_status_msg((int)victim->query("qi")*100/(int)victim->query("max_qi"))+" )\n",me,victim);
                   //victim->receive_wound("qi", taiji_damage*3/5,me);
                   //victim->add("jing",-(me->query_int(1)+ me->query_con(1))* dm );
			        me->add("neili", -800);
			        me->add("jingli",-300);
					me->add_temp("tjq/taiji",-1);					
               break;	
	...
	if( (me->query_skill("taiji-quan",1) > 250 && me->query_temp("yyzq/taiji"))
...
int taiji(object me, object victim, object weapon, int damage)
{
	...
	if( !me->query("yyzq/taiji") && userp(me)) return 0;
...
{
	object weapon;
	object pweapon = ob->query_temp("weapon");
	string msg;
	int ap, dp, j = 0, skill,cur_taiji;
    string taiji_set,next_taiji;
   ...
   if ((int)me->query_temp("tjq/taiji") > 0) {	   
	   ...
	   //开始set的随机
			...
			switch (cur_taiji) {	
			   ...
			   case 0: 
			      message_vision(WHT"$N缓缓念道：“"+HIC+"掤在两臂，小心了！"+WHT+"”,便暗运氤氲紫气，太极"+HIC+"「掤」"+WHT+"字诀随手套出！\n"+
                   ...
                   //ob->receive_damage("qi", damage*4/5,me);
                   //ob->receive_wound("qi", damage/3,me);
				   //化学伤害函数
				   COMBAT_D->do_chemis_attack(me,ob,1,damage*4/5);
                   me->add("neili", -(int)me->query("max_neili",1)/6);
                   if(me->query("neili")<0) me->set("neili",0);
                   ob->add("jing",-to_int((me->query_int(1)+ me->query_con(1))* 3) );
			       me->add("jingli",-300);
			       me->add_temp("tjq/taiji",-1);
			...
			if(ob->query_temp("weapon")&& random(1))
			{
              message_vision(WHT"$n一不小心，手中的"NOR+pweapon->query("name")+WHT"被$N拂中，掉在地上。\n"NOR,me,ob);
			  pweapon->unequip();
		      pweapon->move(environment(ob));
			}
			me->add("neili", -300);
			...
			me->add_temp("tjq/taiji",-1);
			  ...
			  case 2:
			        ...
			        me->set("env/taiji",next_taiji);
                     ...
                     ob->add("jing",-to_int((me->query_int(1)+ me->query_con(1))* 3.5) );
			         me->add_temp("tjq/taiji",-1);	
                    break;	
              case 3:
                     ...
                     ob->add("jing",-to_int((me->query_int(1)+ me->query_con(1))* 4) );
					me->add_temp("tjq/taiji",-1);  
					break;
			}
			return -damage/2;
	...
	if ( skill < 300 && !me->query_temp("yyzq/taiji") ) return 0;
...
int help(object me)
{
	...
	太极拳劲首要在似松非松，将展未展，劲断意不断，其借力打力为武林一绝。
