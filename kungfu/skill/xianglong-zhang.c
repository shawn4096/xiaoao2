...
/*
	...
	if( deads > m ) deads = m;*/
//履霜冰至
    if( me->query_temp("xlz/lushuang") ) {
		string msg;		
		switch(me->query_temp("xlz/lushuang")) {
			case 5: msg = RED"$N身形站定，丹田劲气涌出，降龙掌圆转如意，力由脚下起，双掌向外同时向$n的$l推去，劲力刚柔闪烁"NOR; break;
			case 4: msg = HIC"只听$N又左掌轻旋，一声龙吟，力发丹田，挟起掌风向$n的$l拍去，同时劲力吞吐，捉摸不定"NOR; break;
			case 3: msg = HIR"右掌一收，$n只觉得劲力汹涌，脚下站立不稳，只听$N一声大喝，看掌，刚猛无铸"NOR; break;
			case 2: msg = HIW"$N不等招式用老，回身上步，大喝一声，猛然一掌拍出，招式简单，但回劲无穷"NOR;break;
			case 1: msg = HIB"只听$N又一声龙吟，方才掌力还是刚猛无比，突然换招，一股阴柔之力陡生，拍向$n的$l"NOR; break;
			default: msg = HIG"一招「履霜冰至」一掌在前，柔劲闪烁，一掌在后，刚猛无比，连续两掌拍向$n的$l"NOR; break;
		}
		me->add_temp("xlz/lushuang", -1);
...
//如果是神龙摆尾，会出这招，带反击的。ob_hit中有
	if( me->query_temp("slbw") ) {
		...
		return ([
			...
			"damage":  me->query("gender")=="男性"?300:100,
			"damage_type" : "瘀伤",
		]);
	}
//亢龙有悔绝技
	else if( me->query_temp("xlz/hanglong")|| me->query_temp("xlz/sanzhang") ) { 
		me->delete_temp("xlz/hanglong");
        me->delete_temp("xlz/sanzhang");
		return ([
            "action": HIR"\n$N左腿微屈，身子轻微转动，右掌划了个圆圈，平推出去，正是降龙十八掌中的绝技「亢龙有悔」!"NOR,
			"dodge": -30 + 10 * tmp,
			"parry": -25 + 10 * tmp,
			"strike": 10 + 10 * tmp, 
			"force": 500 + 20 * tmp,
			"damage": me->query("gender")=="男性"?300+random(100):100+random(50),
...
int ob_hit(object ob, object me, int damage)
{
	...
	dp = (int)ob->query("combat_exp")/1000;
...
// slbw 对方空手有效，450后大飞，当前内力要高//迷提 
	if( random(me->query_str()) > ob->query_str()/3
	 ...
	 && random(3) ) {
		...
		me->set("slbw", jl);
		damage = -damage;
        //message_vision(HIY"$N使出一招"NOR,me,ob);
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if (random(me->query_str(1))>victim->query_dex(1)/2
		&& random(me->query_skill("xianglong-zhang",1))>victim->query_skill("parry",1)/2
		&& me->query("double_attack")
		&& objectp(victim)
		&& !random(1))
	  {
		
	         message_vision(HBRED"$N运使双手互搏绝技，左手施展出「"+action[random(sizeof(action))]["skill_name"]+"」掌力未消，右手随后施展「"+action[random(sizeof(action))]["skill_name"]+"」掌力又到，双掌叠加，威力大增。\n"NOR, me, victim);
	         //victim->set_temp("no_fight",1);
		     victim->receive_damage("qi",i);
		     victim->receive_wound("qi",i/2);
		     if (victim->query_condition("no_fight"))
		        victim->add_condition("no_fight",1+random(1));
             else victim->apply_condition("no_fight",1+random(1));
