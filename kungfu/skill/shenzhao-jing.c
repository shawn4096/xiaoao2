...
mixed hit_ob(object me, object ob, int damage)
{
	...
	if(!me->query("quest/shenzhao/diyun")) return;
	if(me->query_temp("szj_power"))
	{
	 ...
	 if(me->query_temp("combat_time")>10 && random(3)  )
	 {
		...
		if (ob->is_busy()&& (me->query("quest/shenzhao/diyun")||me->query("quest/shenzhao/laozu")))
		{
			message_vision(HIC"$N见到$n为神照经所制，趁其手忙脚乱之际，运起神照经又攻出一招。\n"NOR,me,ob);
			//me->add_temp("apply/damage",);
	 ...
	 && random(3)==1
		...
		me->set_temp("szj_power",5);
	 	...
	 	if(busy>5) busy=5;
	 	   ob->add_busy(busy);
		if (me->query("quest/shenzhao/laozu")>3)
...
mixed ob_hit(object ob,object me,int damage)
{
	...
	string msg;
	if(!me->query("quest/shenzhao/diyun")) return;
	if(me->query_skill("shenzhao-jing",1)<450
	 ||me->query_str()<40
	 ||me->query_con()<40
	 ||random(6)
	 ) return;
