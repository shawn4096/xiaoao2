...
mapping query_action(object me, object weapon)
{
	...
	if( me->query_skill("staff", 1) >= 250
	 && me->query_skill("fengmo-zhang", 1) >= 250
	 && me->query_skill("force",1) >= 100
	 && me->query("neili") >= 500
	 && random(me->query_str(1)) >= 25
	 //&& me->query("shen")+1000 < 0 
	 && random(2) ) {
		return ([
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	int j;
	object weapon;
	j=me->query_skill("fengmo-zhang",1);
	j=j+random(j);
	j=j*2;
	
	
	if( random(me->query("neili")) > victim->query("neili")/2 
	 && random(me->query_skill("fengmo-zhang", 1)) >= 100
	 && me->query("env/听风辨器")=="铁蒺藜"
	 && me->query("quest/bx/anqi/pass")
	 && !random(3) )
	{
 	    message_vision(HIB"$N手中疯魔杖法施展起来，霍霍有声，但心声一片安宁，顺手甩出几枚铁蒺藜，射向$n。\n"NOR,me,victim);
		victim->receive_damage("qi",j,me);
		victim->receive_wound("qi",j/3,me);
	}
