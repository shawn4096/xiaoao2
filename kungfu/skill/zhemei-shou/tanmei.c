...
/*
        ...
        lv = me->query_skill("zhemei-shou",1) + me->query_skill("bahuang-gong",1)/3;
        lv = lv /2;
        if(!userp(me)) lv = lv / 6;
        //if (userp(target)) lv=lv/2;
    
        me->add_temp("apply/attack", lv/2);
        me->add_temp("apply/damage", lv /4 );
        me->add_temp("apply/strength", lv /10 );
        me->add_temp("apply/hand", lv /4 );
        me->add_temp("apply/parry", lv /4 );


        me->set_temp("tanmei",1);
