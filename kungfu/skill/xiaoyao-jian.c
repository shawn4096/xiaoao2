...
int valid_learn(object me)
{
        ...
        if (me->query("family/family_name")!="明教")
                return notify_fail("逍遥剑乃是明教光明左使绝技，非明教人士不得修习。\n");
       // if (me->query("family/master_name")!="杨逍")
                //return notify_fail("逍遥剑乃是明教光明左使独门绝技，只能跟杨左使修习。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if (lvl < 350 
			|| lvl / 2 + random(lvl) < (int)victim->query_skill("dodge", 1) 
			|| me->query("neili") < 350 
			|| me->query_skill("dodge", 1) < 350 
			|| me->query_skill("force", 1) < 350 
			|| random(6)
			|| me->is_busy() )
            return 0;
        ...
        me->add("neili", -80);
        victim->add_busy(2 + (int)random(lvl / 100));
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("jingli") < 60)
                return notify_fail("你的精力目前没有办法练习逍遥剑法。\n");
