...
string perform_name(){ return HIC"缠绕诀"NOR; }
...
int check_fight(object me, object target, object weapon)
{
	...
	if( me->query_skill("taiji-jian", 1) > 450 ) i = 2;
        ...
        if(!objectp(wep = me->query_temp("weapon")) || wep->query("skill_type") != "sword"){ 
               ...
               // if(objectp(target)&&target->is_busy()) target->start_busy(1);
        } 
        else if(!target) 
                msg = HIY"$N将手中"+wep->name()+HIY"向后一摆，抱圆守一，撤回了散开的太极剑意．\n"NOR;
        ...
        else if(!living(target) || (!me->is_fighting() && !me->is_killing(target->query("id")))){
       	...
       	else if (userp(me) && (me->query_skill_mapped("sword") != "taiji-jian" 
...
int checking(object me, object target, object weapon)
{
       ...
       //大飞后效果增加
       	...
       	me->add("neili", -350);
       	me->add("jingli", -100);
		target->receive_damage("jing",100+random(500),me);
		target->receive_damage("jingli",100+random(500),me);
//消耗对方的精气，精
       	if( random(ap + dp) > dp 
			|| random( me->query_skill("taiji-jian",1))>target->query_skill("dodge",1)/2){          
                //开了太极神功，则效果加大
				if (me->query("quest/wd/yyzq/taiji/pass"))
                	target->add_busy(3+random(cost/10));
				else target->add_busy(2+random(3));
                msg += chaned[random(sizeof(chaned))];
                msg = replace_string(msg, "$W", weapon->name());
                me->set_temp("tjj/chan", 1);
		        //me->start_perform(1, "缠");//容易出现无法停止的bug
