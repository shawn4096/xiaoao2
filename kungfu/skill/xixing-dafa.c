...
void skill_improved(object me)
{
        ...
        if(skill >= 300 && !me->query("xxdf/300") && me->query("age")>30){
...
{
    int m_xxdf,t_force,i;
    m_xxdf = random((int)me->query_skill("xixing-dafa",1));
    t_force = (int)ob->query_skill("force",1);

    if((int)me->query_skill("xixing-dafa",1)>=350 && ob->query("yjj/powerup")!="pass")
    if(me->query_temp("xxdf/xixing")){
        if(m_xxdf > t_force*3/5 && !random(1)){
            i = - damage;
