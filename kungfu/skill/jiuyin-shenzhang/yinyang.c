...
int perform(object me, object target)
{
	...
	call_out("attack_1", 5,me,target);
	return 1;
}

void attack_1(object me,object target)
{
	...
	if (me->query("neili")<=0||target->query("neili")<=0) {
		...
		call_out("finish",1,me,target); 
	}
	else
		call_out("attack_2",5,me,target);
}

void attack_2(object me,object target)
{
	//object me = this_player();
	//object target = offensive_target(me);
	if (!me || !target) return;
	...
	if (me->query("neili")<=0||target->query("neili")<=0) {
		...
		call_out("attack_3",5,me,target);
}

void attack_3(object me,object target)
{
	//object me = this_player();
	//object target = offensive_target(me);
	if (!me || !target) return;
...
void finish_2(object me,object target)
{
	//object me = this_player();
	//object target = offensive_target(me);
	if (objectp(target)) message_vision(HIY"$N和$n各自大喝一声，停止了对掌，退开数尺。\n"NOR, me,target);
}
