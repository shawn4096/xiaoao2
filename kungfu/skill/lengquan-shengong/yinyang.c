...
#include <ansi.h>
string exert_name() {return HIB"阴"HIY"阳"HIC"无"HIW"常"NOR;}
int check(object me);
int exert(object me)
{
   ...
   me->add("neili", - 400);
   check(me);
   me->start_exert(1+random(2),"「阴阳无常」");
...
int check(object me)
{
   ...
   {
		return me->start_call_out( (: call_other, __FILE__, "remove_effect", me :), 1);
	}
	else {
		//若气受伤，自动疗伤
