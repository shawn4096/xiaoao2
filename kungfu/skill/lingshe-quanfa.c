...
mapping query_action(object me, object weapon)
{
        ...
        if( (me->query_temp("lsqf/ls") || me->query_temp("lszf/bb")) && s > 3 ) {
                ...
                return ([
                 "action": HIG "$N手臂宛然灵动如蛇,出拳的方位匪夷所思，怪招猝发！"NOR,
                  "force": 400 + random(300),
					"parry": 30,
					"dodge": 30,
...
mixed hit_ob(object me, object victim)
{
        ...
        int lvl = me->query_skill("cuff",1),attack,lb;
        attack=lvl/100;
        ...
        lb = random(10);
		//若没有灵蛇无骨，则无法发挥最大威力
		if (!me->query_temp("lsqf/wugu")) return;
		
        if( lb > 5 && !me->query_temp("lsqf/auto") )
          {
                message_vision(WHT "$N忽地欺进，左手快如闪电，两指一并点在$n胸口，正是一招"+HIW+"白蛇吐信"+WHT+"！\n$N紧跟又轰出一拳！" NOR,me,victim);
                    victim->add_busy(random(4));
						 ...
						 if (userp(me))
						  me->add_temp("apply/damage",  me->query_skill("lingshe-quanfa", 1)/2);
     ...
     else if( lb > 4 && !me->query_temp("lsqf/auto") )
     {
