...
string exert_name(){ return HBMAG+HIW"葵花神功"NOR; }
inherit F_SSERVER;
...
int exert(object me)
{
	...
	if( (int)me->query_skill("force",1) < 500 )
                return notify_fail("你的内功还不够火候，使不出「葵花」绝技。\n");  
                
       	if( (int)me->query_skill("kuihua-shengong",1) < 500 )
                return notify_fail("你的葵花神功还不够火候，使不出「葵花」绝技。\n");  
                
       	if( (int)me->query("max_neili",1) < 15000 )
                return notify_fail("你的最大内力修为还不够火候，使不出「葵花」绝技。\n"); 
...
void remove_effect(object me, int count,int dex,int con,int str)
{
        ...
        || count < 1)
        {
            ...
            me->add_temp("apply/parry", -dex/4);
           	...
           	tell_object(me,HIW"你的「"HIR"葵花神功"HIW"」绝技运行完毕，全身忽的一顿，现出整个身形。\n"NOR);
           	return;
        }
