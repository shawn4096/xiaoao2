...
if (skill>=351 && !userp(target) ) target->set_temp("must_be_hit",1);
	...
	weapon2->unequip();
   me->prepare_skill("hand");
	COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), 3);   //一掌定生死
 ...
 if(me->is_fighting(target)) {
	...
	weapon->wield();
	COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), 3);  //如意刀
...
COMBAT_D->do_attack(me, victim, me->query_temp("weapon"),3);         //折梅手
	                          }

 if(me->is_fighting(target)) {
	weapon2->wield();
    me->map_skill("sword", "tianyu-qijian");
	COMBAT_D->do_attack(me, victim, me->query_temp("weapon"),3);   //天羽剑
	weapon2->unequip();
	weapon->wield();
                                   }
