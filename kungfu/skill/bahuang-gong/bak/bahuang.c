...
string exert_name(){ return HIR"八荒六合诀"NOR; }
...
int exert(object me, object target)
{
  ...
  if (me->query("bhg/biguan")==2) 
	...
	call_out("remove_effect", 10, me, skill);
	message_vision(HBYEL+HIG "在$N八荒六合唯我独尊功运转之下，$N只觉四周精气不断向自己周身涌来！\n" NOR, me);
...
{
	int skill;
	if( objectp(me) && me->query_temp("bhg/bahuang") ) {
		/*if( count-- > 0 ) {
			//count=count-2;
    ...
    {		
		...
		tell_object(me, HIR"你的「八荒六合功」运行完毕，将内力缓缓收回丹田。\n"NOR);
	return;
	}
}
...
int help(object me)
{
	write(HIW"\n八荒六合唯我独尊功之「八荒六合诀」："NOR"\n\n");
        write(@HELP
	八荒六合唯我独尊神功乃是逍遥派三大镇派绝技之一，当初童姥修持此功
	和无崖子师兄的北冥神功、师妹刘秋水修炼的小无相神功相抗衡。八荒六
	合唯我独尊原本是阳刚之气强劲的功法，每三十年需要闭关一次，返老还
	童一次，每一次返老还童防御品性都会大幅上升。
	要求：
		最大内力  1500 以上；
		当前内力  500  以上；
		八荒六合唯我独尊功  100  级以上；
