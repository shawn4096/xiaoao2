...
string msg;
    ...
    me->add("neili", -350);
    target->start_busy(1);
        message_vision(HIR "陡然间$N" HIR "施出「" HIY "真" HIR "」字诀，双掌向$n"
              HIR "平平推去，招数朴实无华，毫无半点花巧可言。\n" NOR, me,target);
              
    COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg); 
    
    message_vision(HIB "\n紧接着$N" HIB "变招「" HIC "归" HIB "」字诀，霎"
               "时只见$N" HIR "双掌纷飞，化出漫天掌影笼罩$n" HIW "四面"
               "八方。\n" NOR, me,target); 
    COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
    
    message_vision(HIM "\n紧接着$N" HIM "变招「" HIY "璞" HIM "」字诀，霎"
               "时只见$N" HIR "双掌纷飞，化出漫天掌影笼罩$n" HIW "四面"
               "八方。\n" NOR, me,target); 
    COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
    
    message_vision(HIC "\n最后着$N" HIC "变招「" HIG "反" HIC "」字诀，霎"
               "时只见$N" HIR "双掌纷飞，化出漫天掌影笼罩$n" HIW "四面"
               "八方。\n" NOR, me,target); 
    COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
