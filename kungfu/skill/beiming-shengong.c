...
int valid_learn(object me)
{
	...
	if (me->query("family/family_name")!="逍遥派" && !me->query("quest/天龙八部/北冥神功/pass"))
		return notify_fail("你不得其法，闻名不知其意，难以领会高深的北冥神功。\n");
...
{
   if (me->query("family/family_name")!="逍遥派" && !me->query("quest/天龙八部/北冥神功/pass"))
		return notify_fail("你不得其法，闻名不知其意，难以练习高深的北冥神功。\n");
...
int ob_hit(object ob, object me, int damage, object target)
{
   ...
   // if( !target ) target = offensive_target(me);
    ...
    if((int)me->query_skill("beiming-shengong",1)>=300)
    	...
    	&&(me->query("quest/天龙八部/北冥神功/pass") || me->query("family/family_name")=="逍遥派"))
	{
    	
    	if(m_bmsg > t_force*9/10 && !random(3)){
            i = - damage;
            message_vision(HIC"$N神功陡发，将「北冥真气」聚集于全身，双掌贴将$n胸口过去，将毕生修为贯穿于$n各处经脉之中,$n内力顿时翻腾汹涌。\n"NOR,me,ob);
            me->add("neili",-1000);
    	 ...
    	 else if(m_bmsg > t_force*6/10 && !random(1))
		{
            ...
            ob->add("neili",-2000);
			target->receive_damage("jing",damage/3,me);
            message_vision(HBBLU"$N浑身精力为北冥真气扯动，$n感觉精力似乎有不断流失之状。\n"NOR,me,ob);
			target->add_busy(1);
