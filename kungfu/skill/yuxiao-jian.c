...
([
	"action" : "$N左足一点，身形后撤，忽然又纵身向前，萧意如音缈九天，一招「清箫万里」，$w轻飘飘地直逼$n",
	"skill_name" : "清箫万里",
	"lvl" : 120,
	"damage_type" : "割伤"
]),
([
	"action" : "$N跃起丈余，手中$w画了个弧线，斜斜划下，萧光匝地，一招「狂歌九天」，罩向$n的$l",
	"skill_name" : "狂歌九天",
	"lvl" : 140,
	"damage_type" : "割伤"
]),
([
	"action" : "$N横萧就唇，使出「凤曲长鸣」，$w轻颤有声，呜咽作响，忽然快如闪电般划向$n的右耳",
	"skill_name" : "凤曲长鸣",
	"lvl" : 160,
	"damage_type" : "割伤"
]),
([
	"action" : "$N回身拧腰，右手双指捏了个指诀，左手挥$w向后反撩而上，一式「箫史乘龙」，萧势如匹练般撩向$n的$l",
	"skill_name" : "箫史乘龙",
	"lvl" : 180,
	"damage_type" : "刺伤"
]),
...
int valid_learn(object me)
{
	...
	if (me->query("family/family_name")!="桃花岛")
		return notify_fail("你不是桃花岛弟子无法练玉箫剑法。\n");
	if (me->query("family/master_name")!="黄药师")
		return notify_fail("你不是黄药师弟子无法练玉箫剑法。\n");
...
void yuxiao(object me, object victim)
{
	...
	switch(random(3)){
		case 0 :
			msg = HIW"\n$N施展开奇门剑路，剑剑不依照常规！$n连连後退，全力招架，无力还招！\n"NOR;
			...
			break;
		case 1 :
			msg = HIY"\n$N脚下踩著奇门步法，一下子刺出好几剑！$n根本分不清楚那一剑是虚，那一剑是实，只好停下攻势，严守门户！\n"NOR;
			message_vision(msg, me, victim);
			me->set_temp("yuxiao_auto", 1);
			COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), 3);
			me->delete_temp("yuxiao_auto");
			...
			break;
		case 2 :
			msg = HIW"\n$N风驰电掣的绕著$n快跑，忽东忽西，简直像是足不点地的飞行一般！$n随着$N转来转去，弄得眼花缭乱，摇摇欲坠！\n"NOR;
			message_vision(msg, me, victim);
			me->set_temp("yuxiao_auto", 1);
			COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), 3);
			me->delete_temp("yuxiao_auto");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	&& random(j)> victim->query_skill("dodge",1)/3
