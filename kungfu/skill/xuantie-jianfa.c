...
{
	if (!this_player()->query("quest/xtj/pass"))
		return notify_fail("你尚未解密成功，无法激发玄铁剑法。\n");
	else return usage == "sword" || usage == "parry";
}
...
mapping query_action(object me, object weapon)
{
   ...
   if(level> 450 
	...
	{
		return ([
			    "action": HBRED"$N潜运内力将全身功力凝聚在$w"+HBRED"之上缓缓刺出一剑，只是一招挺剑击刺却是挡无可挡避无可避无坚不摧。"NOR,
...
//增加无招胜有招 无剑3【木剑】
	if(i> 150 
	   ...
	   {
		if(i<350)
			msg = HIB"$N狂啸一声，"+weapon->name()+HIB"顿时泛出青蓝色的光彩，一股蓝色剑芒激刺而出！\n"NOR;
		else 
		{
            ...
            if(random(4)==1) victim->add_busy(1);
			}
