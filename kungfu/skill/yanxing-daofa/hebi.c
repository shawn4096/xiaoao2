...
int perform(object me, string arg)
{
	...
	if(me->query_skill("force", 1) < 500 )
		return notify_fail("你的基本内功等级不够，不能使用「双刀合璧」。\n");       
	...
	if(me->query("max_neili") < 12500 )
		return notify_fail("你的内力修为太弱，不能使用「双刀合璧」。\n");

	if(target->query("max_neili") < 12500 )
...
string perform_name(){ return HIW"双刀合璧"NOR; }
