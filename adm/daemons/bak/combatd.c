...
//调整参悟后移穴效果
	if( damage > 0 ) {
        ...
        if((int)victim->query("relife/gifts/total",1) > 0 && random(120)<random(80+(int)victim->query("relife/gifts/total",1))  )
		{
           ...
           if( victim->query_condition("no_fight") || victim->query_temp("no_fight")){
      ...
      if(userp(me) && me->query("env/damage"))      tell_object(me,WHT"你对"+ victim->query("name") +"造成了"RED+ damage+ WHT"点攻击伤害。\n"NOR);    
      if(userp(victim)&& victim->query("env/damage"))      tell_object(victim,WHT""+ me->query("name") +"的攻击对你造成了"RED+ damage+ WHT"点伤害。\n"NOR);    //-------------------------------------------------------------
...
void killer_reward(mixed killer, object victim)
{
	...
	if (userp(victim)) {
		...
		do {
			...
			//终身vip掉落0.7%,上限为200k,增加终身的好处。
			if (victim->query("buyvip") || victim->query("rmb_used")>=500 ) {
				exp = victim->query("combat_exp")* 70 / 10000;
...
//玩家对NPC 伤害降低20%，这个值得商榷。。。4-》5
		   ...
		   { 
			    if (me->query("combat_exp",1)<1500000) damage=damage *1.3;
