...
mapping trusted_read = ([
	"/":				({ "(admin)", "(assist)", "(arch)", "(wizard)", "(apprentice)", "(immortal)", "(player)" }),
	"adm/etc":			({ "(apprentice)", "(immortal)", "(player)" }),
	"d/group":			({ "(all)" }),
	"clone/zhiye/signs":	({ "(all)" }),
...
int valid_read(string file, mixed user, string func)
{
	...
	if (func == "restore_object")
	{
		...
		if (base_name(user) == "/d/xiangyang/leitai/npc/leitaiuser" && sscanf(file, "/data/%*s"))
		{
			return 1;
		}
	}
...
int valid_write(string file, mixed user, string func)
{
	...
	if (sscanf(file, LOG_DIR + "%*s") && (func=="write_file" || func=="save_object" || func=="rename" || func=="mkdir"))
		return 1;

	// Let user save their save file
	if (func == "save_object") {
		if (sscanf(base_name(user), "/clone/%*s") && sscanf(file, "/data/%*s") && user->query_save_file()+SAVE_EXTENSION == file)
			return 1;
	}
