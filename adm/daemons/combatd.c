...
// By Jpei 整合参悟和天赋系统,修改状态报告适应message.c以便屏蔽信息
// Modified by Renlai 修复攻击溢出问题 2016/10/19
...
#ifndef INTERGER_MAXVALUE
#define INTERGER_MAXVALUE 2147483647
#endif
//设定最大值
#define MAX_POWER INTERGER_MAXVALUE
#define CRITICAL_OVERFLOW(p)    ( p < -100000000 )



public int last_damage(object me,object victim,int damage);

string humanreadable_number(int x){
	string orc = sprintf("%d",ABS(x));
	string rs;
	string* fx;
	
	if(strlen(orc)>9){
		fx = allocate(4);
		fx[0] = HIR+orc[0..<10]+NOR;
		fx[1] = HIM+orc[<9..<7]+NOR;
		fx[2] = HIC+orc[<6..<4]+NOR;
		fx[3] = orc[<3..<1];		
	}else if(strlen(orc)>6){
		fx = allocate(3);
		fx[0] = HIM+orc[0..<7]+NOR;
		fx[1] = HIC+orc[<6..<4]+NOR;
		fx[2] = orc[<3..<1];
	}else if(strlen(orc)>3){
		fx = allocate(2);
		fx[0] = HIC+orc[0..<4]+NOR;
		fx[1] = orc[<3..<1];
	}else{
		if(x<0)
			return "-"+orc;
...
void announce(object ob, string event)
{
	...
	switch(event) {
		...
		case "revive":
			...
			//break;//原来没有，尝试增加by sohu@xo,以防止错误
		//default:
			//break;
	}
	tell_room(env,"\n\n" + ob->name() + msg, ({ob}) );
...
varargs int skill_power(object ob, string skill, int usage)
{
	...
	if (!living(ob)) return 0; //如果昏倒，则跳出？是不是会导致杀不死？临时调整测试
	...
	if (usage == SKILL_USAGE_ATTACK) power = power * ob->query_str();
	else power = power * (skill == "dodge"?ob->query_dex():ob->query_int());
	//overflow check block by Luffy
	if (CRITICAL_OVERFLOW(power) ){
		string sskill = skill;
		if(sskill!="dodge" && sskill!="parry")
			sskill = "attack";
		power = MAX_POWER;
		//who exp skill parry
		log_file("combat_overflow",
			sprintf("%-19sEXP:%8d %-5s %s",ob->detail_name(),ob->query("combat_exp",1),sskill, ob->query_skill_mapped(skill)), 
			ob);
	}
	//end
	return power;
...
mixed force_hit(object me, object victim, int damage_bonus, int factor, int em)
{
	...
	if (!intp(factor) || !factor) factor=1;//防止加力异常
	...
	if (damage < 0) {
		...
		{
			damage = - damage;
			me->receive_damage( "qi", (int)damage, victim);
			me->receive_wound( "qi", (int)(damage/3), victim);
			if (damage < 100)
...
//添加个统一化学攻击 by renlai
//do_chemis_attack()
// attack_type 0精力 1气血 2，精血 3，气血+精血
varargs int do_chemis_attack(object me, object victim,int attack_type,int damage,string msg)
{
	int em,vexp,mexp,i,p;	
	...
	|| me->query("eff_jing")+me->query_temp("apply/jing") < 0) return 0;	
	
	if (living(victim) && (victim->query("qi") < 0 || victim->query("jing") < 0 || victim->query("jingli") < 0)) return 0;
	...
	if (mexp < 1000000)
	{
		if (damage>= 2000) damage=900+random(1000);
	}
	else if (mexp<10000000)
	   {
		  if (damage>= 6000) damage=4900+random(1000);
	   }
	   else if (mexp<50000000)
	   {
		   if (damage>= 10000) damage=8900+random(1000);
	   }
      else if (mexp<100000000)
	   {
		   if (damage>= 50000) damage=49900+random(1000);
	   } 
	 ...
	 switch (attack_type)
		 {
			 case 0:
			    victim->receive_damage("jingli", to_int(damage),me);		       
			 break;
			 case 1:
			    victim->receive_damage("qi", to_int(damage),me);
		        victim->receive_wound("qi", to_int(damage/5),me);
			 break;
			 case 2:			    
				victim->receive_damage("jing", to_int(damage),me);
		        victim->receive_wound("jing", to_int(damage/5),me);
			 break;			
			 case 3:
			    victim->receive_damage("qi", to_int(damage),me);
		        victim->receive_wound("qi", to_int(damage/5),me);
			 ...
			 break;
			 default:
                victim->receive_damage("jingli",10,me);			 
		 }
if (msg && (attack_type==1 ||attack_type==3))
		 {
			 message_vision(msg+"\n"NOR,me,victim);//化学描述
			 report_status(victim);//对手状态描述
		 }	 
...
varargs void do_attack(object me, object victim, object weapon, int attack_type)
{
	...
	mixed foo;
	int ap, dp, pp, em, damage, damage_bonus, brief,total,def,abs,lastaction,i;
	int apanddp,apandpp;
	int wounded = 0;
	int flag = 0;
	int skill = (me->query_skill("jingmai-xue", 1) + me->query_skill("medicine", 1)) * 3/4;
	...
	if (living(victim) && (victim->query("qi") < 0 || victim->query("jing") < 0 || victim->query("jingli") < 0)){
		//message_vision(HIC"请报告巫师，here is teset bug area\n"NOR,me);
		//tell_object(me, HIR "\n对方已经濒死，你不需要用大招了。\n" NOR);
		//me->remove_all_enemy();
	...
	if ( sizeof(prepare) == 2) {
		lastaction=me->query_temp("action_flag");
		if (lastaction<1) lastaction=1;
		attack_skill = (keys(prepare))[lastaction-1];
		if (lastaction==2) me->set_temp("action_last",2); //by renlai 互备交叉用招
		if (lastaction==1) me->set_temp("action_last",1); //by renlai 互备交叉用招
		
		if (me->query("env/主攻")) {
		for(i = 0; i<sizeof(prepare); i++)
		{
		  if (me->query("env/主攻") == (keys(prepare))[i]) attack_skill=(keys(prepare))[i]; 
		}
		}		
	...
	if (flag && !me->query_skill_mapped(attack_skill)) {
		...
		for(i = 0; i<sizeof(prepare); i++)
		{
		   if (me->query("env/主攻") == (keys(prepare))[i]) 
			   attack_skill=(keys(prepare))[i]; 
		}
		}	
		}
		if ( weapon ) attack_skill = weapon->query("skill_type");
	...
	if( !mapp(action) ) {
		CHANNEL_D->do_channel(this_object(), "err",sprintf("%O(%s): bad action = %O",me, me->query("id"), me->query("actions", 1)));
...
*/
	...
	apanddp = ap + dp;
	if ( CRITICAL_OVERFLOW(apanddp) ){
		apanddp = MAX_POWER;
	}
	if (random(apanddp) < dp && living(victim)) {
...
*/
		...
		if (victim->query_temp("must_be_hit"))
			...
			if (vweapon) {
				...
				if (sizeof(prepare) == 1)
					foo = (keys(prepare))[0];
				if (sizeof(prepare) == 2) {
					lastaction=victim->query_temp("action_flag");
					if (lastaction<1) lastaction=1;
					foo = (keys(prepare))[lastaction-1];
					if (lastaction==2) victim->set_temp("action_last",2);
					if (lastaction==1) victim->set_temp("action_last",1);
                  if (victim->query("env/主攻")) {
		for(i = 0; i<sizeof(prepare); i++)
		{
			if (victim->query("env/主攻") == (keys(prepare))[i]) 
				foo=(keys(prepare))[i]; 
		}
		}     				
		...
		apandpp = ap + pp;
		if ( CRITICAL_OVERFLOW(apandpp) ){
			apandpp = MAX_POWER;
		}
		if (random(apandpp) < pp && living(victim)) {
			...
			//添加was里免伤武器 by renlai

			  vroom=environment(me);
		if ( weapon && objectp(vroom) && !vroom->query("backroom")) WEAPON_D->weapon_cut(me, victim, weapon, vweapon, damage);
		...
		// Hit! modified by snowman
            ...
            damage = damage*2/3 + random(damage*2/3); //增加伤害测试早期性能
			if (action["damage"])
...
//							foo /= 2;
     ...
     //update by lsxk@hsbbs 敌方是玩家，大幅度降低super威力.  2007/10/29
			...
			if (!living(victim)) damage *= 6; // hehehehehehe....会不会溢出啊？暂时屏蔽下，反正也昏迷了，必死，2014/05/30
			...
			//添加was免伤
			vroom=environment(me);
		  if (objectp(vroom) && !vroom->query("backroom")) CLOTH_D->armor_cut(me , victim , weapon , damage);			

			damage -= random(victim->query_temp("apply/armor")) * 3;	// 让 armor 有一些效果,//加大效果，由2-》3
			...
			if (damage <= 0) damage = 0;
			else victim->receive_damage("qi", (int)damage, me );
			...
			if (
				...
				victim->receive_wound("qi", damage - victim->query_temp("apply/armor") * 3 , me); //armor由2->3,提高铠甲防御力
...
//调整参悟后移穴效果
//重新调整战斗移穴模式，每次只有一项选择，变相增加牵制类武技的威力
	if( damage > 0 ) {
		report_status(victim, wounded);
		if( victim->is_busy() ) {
		  //原来是因为转派，所以有120的叠加，现在没转派，基数为80
		  //if((int)victim->query("relife/gifts/total") > 0 && random(240)<(int)victim->query("relife/gifts/total",1) )
		   if((int)victim->query("relife/gifts/total",1) > 0 && random(100)<random(80+(int)victim->query("relife/gifts/total",1)) )
		   { 
			   message_vision(HIW"$N微觉行动受制，急忙默运真气，冲击受阻的经脉，移开了周身要穴！\n"NOR,victim);
               victim->start_busy(-1);
             }
			   // victim->interrupt_me(me);//暂时调整此处，中断对方怕有异常。有待于观察
		}	

        else if((int)victim->query("relife/gifts/total",1) > 0 && random(120)<random(80+(int)victim->query("relife/gifts/total",1))  )
		{
        ...
        else if( victim->query_condition("no_force") ){
               message_vision(HIC"$N发现自己内息不匀，急忙默运真气，调理内息！\n"NOR,victim);
               victim->clear_condition("no_force");
           }
        else if( victim->query_condition("no_fight") || victim->query_temp("no_fight")){
               message_vision(HIC"$N发现自己不能出招，急忙默运真气，使得招式逐渐连贯起来！\n"NOR,victim);
               victim->clear_condition("no_fight");
			   victim->delete_temp("no_fight");
           }
        else if( victim->query_condition("no_perform") ){
      ...
      if(userp(victim)&& victim->query("env/damage"))      tell_object(victim,WHT""+ me->query("name") +"的攻击对你造成了"RED+ damage+ WHT"点伤害(总计"RED+ total +WHT"，豁免"RED + abs + WHT")。\n"NOR);    //-------------------------------------------------------------
		...
		&& ((victim->query("qi")*2 <= victim->query("max_qi"))||!living(victim)))
			end_fight(me, victim);

//参悟后增加出狠招机会，
        if(  me->query("relife/gifts/total",1) > 0
             && living(victim)
			 && me->is_fighting(victim)
             && !me->is_busy()
             && 80< random(80+(int)me->query("relife/gifts/total",1))
			 && random(10) > 7 ){
                  message_vision(HIR"$N将自己平素领悟到的武学精微之处发挥出来，攻势更加凌厉！\n"NOR,me,victim);
...
void fight(object me, object victim)
{
	...
	if (!living(me) || me->query_temp("combat_yield"))
		return;
	/*if (me->query_temp("no_fight")) {
		message_vision(HIR"提示：你被设定了no_fight变量。\n"NOR,me);
		//发现杀不死的主要原因是有这个变量。。。
		return;
	}*/
...
//			|| (me->query("double_attack") && !me->query_temp("weapon"))
			...
			|| me->query_temp("double_attack")) {
				...
				//记录上次攻击的内容 by renlai
				me->set_temp("action_flag",2);
				if (me->query_temp("action_last")==2) me->set_temp("action_flag",1);
				do_attack(me, victim, weapon, TYPE_QUICK);
				me->delete_temp("action_flag"); //这里不能删除，否则不会切换 by renlai
			}
...
int last_damage(object me,object victim,int damage)
{
	...
	if(!userp(me) && userp(victim)){
		...
		if(victim->query_condition("job_busy"))
		{
		    damage = damage /5 ;
		    damage = damage *4 ;
			if(victim->query("combat_exp")>25000000) //原来都是<
			{
		     damage = damage /5 ;
		     damage = damage *4 ;
			}
			//弱化npc攻击
			if(victim->query("combat_exp")>20000000)
			{
		     damage = damage /5 ;
		     damage = damage *4 ;
			}
...
//pvp
		...
		{       
			if(me->query("combat_exp") < 5000000)  
				...
				//else damage = damage * 1 / 7;

                     }
//玩家对NPC 伤害降低20%，这个值得商榷。。。4-》5
		else 
		   { 
			    if (me->query("combat_exp",1)<1500000) damage=damage *1.2;
				//else if (me->query("combat_exp",1)<2500000) damage=damage *1.1;
				else if (me->query("combat_exp",1)<4500000) damage=damage ;
				//else if (me->query("combat_exp",1)<6400000) damage=damage ;
				else if (me->query("combat_exp",1)<9400000) damage=damage *0.9;
				else if (me->query("combat_exp",1)<15000000) damage=damage *0.8;
		        else damage = damage *0.6;
		   }	                                
	}
	else
	{
