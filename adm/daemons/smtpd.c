...
#define MAIL_NAME "bjarrower@126.com"
#define PUBLIC_MUD_MAIL "bjarrower@126.com"
#define SMTP_SERVER "115.28.138.173"
#define SMTP_PORT "25"
#define SMTP_CLIENT_VERSION "LPC SMTP Client v1.0"
#define DOMAIN_NAME "xojh.cn"
#define TIME_OUT 30
...
protected void time_out(int fd, int s)
{
	...
	if (objectp(usr = mailmsg->user)){
		...
		tell_object(usr,HIW"网址："+HIY+U+"http://www.xojh.cn"NOR+HIW+"留言，工作人员会手工发送。\n"NOR);
	}
	map_delete(queue,fd);
	socket_close(fd);
}
