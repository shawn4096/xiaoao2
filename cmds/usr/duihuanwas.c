...
string check(object me)
{
	...
	if (me->query("was_job/wasscore") < 500)
...
int main(object me, string arg)
{
    ...
    if (me->query("was_job/wasscore") < num)
			return notify_fail("你的积分没有这么多。\n");
		jobs=num /500;
		me->add("was_job/wasscore",-jobs*500);
		me->add("job_time/勇闯万安寺",jobs);
		write("您使用了"+jobs*500+"点万安寺积分兑换了"+jobs+"次勇闯万安寺任务。\n");
		write("您还有万安寺积分"+me->query("was_job/wasscore")+"点。\n");
		me->save();
	    log_file("static/wasscore",
		me->query("name")+"("+capitalize(geteuid(me))+")"
		+"兑换"+num+"点，增加勇闯万安寺job次数"+jobs+"次。\n",
		me,
		({ me })
	);
...
int help(object me)
{
	...
	write("	您目前拥有积分"+me->query("was_job/wasscore")+"点。\n\n");
	write("	若积分不足"+500+"则不能兑换。\n");
	write("	指令格式：\n");
	write("	duihuanwas 500\n");	
