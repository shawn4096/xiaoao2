...
// 显示武器未打造的机会次数
   ...
   /*    if(ob->query("registered")<3) temp =HBCYN+HIY"免费玩家"NOR;
	...
	if(ob->query("buyvip")) temp =HBRED+HIW "荣誉终身贵宾"NOR;
	else if(ob->query("y-card-vip")) temp =HBMAG+HIY"年卡贵宾"NOR;
	else if(ob->query("m-card-vip")) temp =HBCYN+HIY"月卡贵宾"NOR;
	else if(ob->query("registered")<3) temp =HBCYN+HIY"免费玩家"NOR;
