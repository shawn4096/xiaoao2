...
int main(object me, string arg)
{
	...
	//mneili=ob->query_skill("force")-ob->query_temp("apply/force"))*10 + my["age"]*20 + my["combat_exp"]/1000 + ob->query_temp("apply/neili"));
	//mjingli=
	...
	if (!intp(my["qi"]))
	{	
           log_file("hp_error",sprintf("%s %sHP获取qi错误%s。\n", ob->query("id"),ob->query("name"),my["qi"]), ob);
		   my["qi"]=0;
	}
	if (!intp(my["jiali"]))
	{	
           log_file("hp_error",sprintf("%s %sHP获取jiali错误%s。\n", ob->query("id"),ob->query("name"),my["jiali"]), ob);
		   my["jiali"]=0;
	}
	...
	write(sprintf("·气血·%s%5d / %5d %s(%3d%%)" NOR "  ·内力·%s%5d / %5d(+%d)\n" NOR,
		...
		//status_color(my["neili"], mneili), my["neili"], mneili,
		my["jiali"] ));
