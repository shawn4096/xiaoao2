...
// Add By River@SJ 2003.6.24
	...
	if ( userp(me) && target->query("combat_exp") < 10000000 )
		return notify_fail("你的嘴太黑了, 弱者(不足10m)你也骂？\n");
		
	if ( userp(me) && me->query("combat_exp") < 10000000 )
          return notify_fail("对不起,只有大于10M才可以叫骂玩家。\n");
...
/*
	if ( userp(me) && userp(target))
		if (me->query("family/family_name")==target->query("family/family_name"))
			return notify_fail("师傅说了同门要彼此提携，互帮互助，严禁同门叫骂！\n");
			*/
	if( me->is_busy() )
