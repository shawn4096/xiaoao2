...
//		return notify_fail(HIR"你的id属于接手别人的id，未cut转让时的exp不能杀害其他玩家！\n"NOR);
	...
	if (userp(obj) && me->query("combat_exp")<10000000) //经验少于12M不许PK，防止大米干扰人家JOB！		
		return notify_fail("对不起,只有大于10M才可以组队PK。\n");
...
*/
    ...
    //添加大还丹cd by renlai
	if (userp(obj) && userp(me)) {
			 me->add_condition("dhd_cd",30);
	}
			 
			 
	if (userp(obj) && userp(me)
		&& me->query("family/family_name")==obj->query("family/family_name"))
			return notify_fail("师傅说了同门要彼此提携，互帮互助，严禁同门偷盗！\n");
	...
	for(i=0;i<sizeof(t);i++)
	{
		...
		if (t[i]->query("family/family_name")==obj->query("family/family_name"))
			return notify_fail("你的队伍中有人和要杀害的玩家是同门，无法组队杀！\n");
