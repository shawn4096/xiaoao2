		...
		case "dls":												//设定大轮寺武功
			...
			switch(in_level)
			{
				...
				case 1:
					...
					{
						case 0:
							set_skill("xuedao-daofa",500);										//大轮寺高级兵器：血刀经
							map_skill("blade","xuedao-daofa");
							w_parry="xuedao-daofa";
							set("env/jixue",random(5)+1);                                       //pfm 祭血设置条件
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.jixue" :)});        	//pfm 祭血
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.bafang" :)});			//神刀穿心
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.chuanxin" :)});			//神刀穿心
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.liuxing" :)});			//神刀穿心
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.chilian" :)});			//神刀穿心
							u_w_pfm = u_w_pfm + ({(: perform_action, "blade.huaxue" :)});			//神刀穿心

							u_weapon="blade";
							set("f_w_skill","xuedao-daofa");
						break;
						case 1:
