					...
					switch(random(1))
					{
						case 0:
							...
							if(in_weapon)
							{
								...
								set("quest/mj/shlf/lianji/pass",1);
							}
							...
							//u_w_pfm = u_w_pfm + ({(: perform_action, "dagger.lianji" :)});
				...
				case 1:
					...
					switch(random(1))
					{
						case 0:
							...
							map_skill("strike","jiuyin-shenzhang");
							prepare_skill("strike", "jiuyin-shenzhang");
							set_skill("jiuyin-shenzhua",501);								//九阴高级空手：九阴神爪
							map_skill("claw","jiuyin-shenzhua");							
							...
							u_w_pfm = u_w_pfm + ({(: perform_action, "whip.xuanyin" :)});      //玄阴剑法pfm：玄阴诀 	
							u_w_pfm = u_w_pfm + ({(: perform_action, "whip.feilong" :)});			//斩字诀	
							u_weapon="whip";
							set("f_w_skill","yinlong-bian");
						break;
					}
