...
nosave string *weapon_list = ({"sword","axe","blade","brush","club","dagger","fork","hammer","hook","spear","staff","stick","xiao","whip","arrow"});
nosave string *armor_list = ({"cap","boot","coat","armor","mantle","belt","glove",});
nosave string *all_list = ({"sword","axe","blade","brush","club","dagger","fork","hammer","hook","spear","staff","stick","xiao","whip","arrow", "cap","boot","coat","armor","mantle","belt","glove",});
...
int accept_object(object who, object ob, object me)
{
	...
	message_vision(CYN "$N" CYN "把$n" CYN "递过去的" + ob->name() +CYN "推了回来，摇头道：无功不受禄。\n" NOR, this_object(), who);
	return 0;
}
...
int do_taojiao(string arg)
{
	...
	if (mapp(myskill)){
		...
		//duanzao由30调成20，解决一个id，双料工匠问题
		if (skill!="" && me->query_skill(skill,1) / 20 <= sizeof(vb) ){
