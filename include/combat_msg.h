...
string damage_msg(int damage, string type)
{
	...
	switch( type ) {
		...
		case "刀伤":
			...
			if( damage < 100 ) return "结果在$p$l划出一道细长的血痕。\n";
			if( damage < 260 ) return "结果「嗤」地一声，在$n身上划出一道伤口！\n";
			if( damage < 520 ) return "结果「嗤」地一声，在$n身上划出一道血淋淋的伤口！\n";
		...
		case "剑伤":
			...
			if( damage < 100 ) return "结果在$n$l刺出一个创口。\n";
			if( damage < 260 ) return "结果「噗」地一声刺入了$n$l寸许！\n";
			if( damage < 520 ) return "结果「噗」地一声刺进$n的$l，使$p不由自主地退了几步！\n";
		...
		case "击伤":
			...
			if( damage < 100 ) return "结果在$n的伤处造成一处瘀青。\n";
			if( damage < 250 ) return "结果一击命中，$n的伤处登时肿了一块老高！\n";
			if( damage < 320 ) return "结果一击命中，$n闷哼了一声显然吃了不小的亏！\n";
			if( damage < 440 ) return "结果「砰」地一声，$n退了两步！\n";
			if( damage < 560 ) return "结果这一下「砰」地一声打得$n连退了好几步，差一点摔倒！\n";
			if( damage < 680 ) return "结果重重地击中，$n「哇」地一声吐出一口鲜血！\n";
			if( damage < 1000 ) return "结果只听见「砰」地一声巨响，$n像一捆稻草般飞了出去！！\n";
			return "只听见$n一声惨叫，整个身体象断了线的风筝一样飞了出去！！\n";
		case "内伤":
			if( damage < 50 ) return "结果只是把$n打得退了半步，毫发无损。\n";
			if( damage < 100 ) return "结果$n痛哼一声，在$p的$l造成一处瘀伤。\n";
			if( damage < 250 ) return "结果一击命中，把$n打得痛得弯下腰去！\n";
			if( damage < 320 ) return "结果$n闷哼了一声，脸上一阵青一阵白，显然受了点内伤！\n";
			if( damage < 440 ) return "结果$n脸色一下变得惨白，昏昏沉沉接连退了好几步！\n";
			if( damage < 560 ) return "结果重重地击中，$n「哇」地一声吐出一口鲜血！\n";
			if( damage < 1000 ) return "结果「轰」地一声，$n全身气血倒流，口中鲜血狂喷而出！\n";
			return "结果只听见几声喀喀轻响，$n一声惨叫，像滩软泥般塌了下去！！\n";
		case "震伤":                
			if( damage < 200 ) return "结果$n受到$N的劲力一震，闷哼一声。\n";
			if( damage < 300 ) return "结果$n被$N以劲力一震，「嘿」地一声退了两步。\n";
			if( damage < 450 ) return "结果$n被$N以劲力一震，胸口有如受到一记重锤，连退了五六步！\n";
			if( damage < 550 ) return "结果$N的劲力一下震得$n连退了好几步，差一点摔倒！\n";
		...
		case "拉伤":
			...
			if( damage < 100 ) return "结果在$n的$l抽出一道细长的血印。\n";
			if( damage < 260 ) return "结果「啪」地一声$w将$n的$l皮肤扯开，形成一道尺许长的伤口！\n";
			if( damage < 380 ) return "只听$n「啊」地一声吐出一口鲜血，原来$p$l内的骨头已经被$w的劲力抽裂！\n";
			if( damage < 1000 ) return "结果随着「咔」地一声，$w已将$n$l处的骨头硬生生地抽断，断骨从肉里冒了出来！\n";
			return "结果$n一声惨嚎，$w已从$p$l处撕下了一大片血淋淋的皮肉，鲜血溅得满地！！\n";
		case "砸伤":
		case "挫伤":
			if( damage < 50 ) return "结果只是轻轻地碰撞到了$n一下，没有造成什么伤害。\n";
			if( damage < 100 ) return "结果$w只将$n的$l砸出一块瘀红。\n";
			if( damage < 260 ) return "结果「啪」地一声，$w打中$n的$l，登时肿了一块老高！\n";
			if( damage < 420 ) return "结果$w重重得击中$n的$l，打得$p倒退数步，「哇」地吐出一大口鲜血！\n";
			if( damage < 1000 ) return "结果只听见「硼」地一声巨响，$w打在$n身上，将$p像一捆稻草般击飞出去！\n";
			return "结果$w一下打在$n的身上，几声骨碎声中，$p象散了架似的瘫了下去！！\n";
		case "抓伤":
			if( damage < 50 ) return "结果只是在$n$l处轻轻刮过，没有什么伤害。\n";
			if( damage < 100 ) return "结果$n皱了皱眉，$p的$l已经被拉出了一道浅浅的血痕。\n";
			if( damage < 220 ) return "结果拉下来$n$l的一点皮肉，疼得$p「哇哇」怪叫了起来！\n";
			if( damage < 390 ) return "结果「唰」地一声，$n的$l上顿时被抓出五道血痕，鲜血流了出来！\n";
			if( damage < 460 ) return "结果$n疼得大叫一声，$p$l上被抓出了几道深深的血沟，鲜血直流！\n";
			if( damage < 580 ) return "结果只听见$n一声惨嚎，$N的手爪已在$p的$l处刺出了五个血肉模糊的窟窿！\n";
			if( damage < 1000 ) return "结果「啊」地一声惨叫，$n身上$l处被$N抓下了一大片皮肉，鲜血横飞满地！！\n";
			return "结果$n一声凄惨的嘶叫，$l处被连皮带肉扯下一大块，露出了白森森的骨头！！\n";
                case "摔伤":
                case "撞伤":
			if( damage < 100 ) return "结果$n脚下不稳，在地上擦破了点儿皮。\n";
			if( damage < 380 ) return "结果「啪」地一声，$n在地上摔了个屁礅。\n";
			if( damage < 460 ) return "结果$n一不小心，被$N重重摔倒在地！\n";
			if( damage < 520 ) return "结果$n被一下摔倒在地，「噗地」一声喷出了一大口鲜血！\n";
		...
		default:
			...
			if( damage < 700 ) str = "结果对$n造成相当严重的";
			if( damage < 600 ) str = "结果对$n造成颇为严重的";
			if( damage < 530 ) str = "结果对$n造成一处严重";
			if( damage < 300 ) str = "结果对$n造成一处";
			if( damage < 240 ) str = "结果对$n造成轻微的";
			if( damage < 100 ) str =  "结果对$n只是勉强造成一处轻微";
			return str + type + "！\n";
	}
}
