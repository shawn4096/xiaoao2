...
//	me->start_busy(random(3) + 2);
	...
	if (!newroom->query("no_fight") && !query_temp("s_time") && random(10) < 7) {
		call_out("delete_temp", 15, "safe");
		set_temp("s_time", 1);
		set("fei_time", i);

		jf = new(__DIR__"cateran1");
      ...
      //  tell_room(newroom,HIR"突然路边跳出一群劫匪！\n"NOR);
		...
		if (random(5)) {       
		 ...
		 jf->do_copy(maxplayer);
		 jf->move(newroom);
		 //jf->set("title",HIR"土匪帮主"NOR)
		 add("cateran",1);
         tell_room(newroom,HIR"紧接着突然路边又跳出一个劫匪头子！\n"NOR);
		 tell_room(newroom,HIR+jf->query("name")+"恶狠狠地说道：想我这边过，留下买路钱！\n"NOR);
        }
		else {
		jf = new(__DIR__"cateran1");
		jf->set_temp("target", cart); //要抢劫的目标
...
int do_finish(string arg)
{
	...
	if (j > 0) {
		...
		tb=2+random(2);
       // exp= step * 60 + cateran * 30 ;
			
		exp= step * 60 + cateran * 100+ query("cateran1")*50 ; //杀死超级劫匪增加奖励by sohu..
		...
		if (step < 1) step = 1;
        if (exp > 7500  ) exp =7000 + random(500);

		if (exp < 2500 ) exp= 2000+ random(500);
    
		exp=exp*0.5;//统一把护镖下调一半;
	    //为了压制效率和参悟时间再统一下调一般;
		//增加vip的奖励
		exp=exp*2/3; //为了压缩发展速度，再次下调
		...
		if (team[i] && team[i]->query_temp("protecting")==cart) {
			...
			if(team[i]->query("buyvip") )
				exp = exp + exp * 20/ 100;
			else if(team[i]->query("y-card-vip"))
				exp = exp + exp * 10 / 100;
			else if(team[i]->query("m-card-vip"))
				exp = exp + exp * 5 / 100;
		  ...
		  if (team[i]->query("combat_exp")>25000000&&team[i]->query("combat_exp")<35000000)
		     jl=jl/2;
          if (team[i]->query("combat_exp")>35000000&&team[i]->query("combat_exp")<75000000)
			 jl=jl/10;
		  if (team[i]->query("combat_exp")>75000000) jl=0;
...
//                        team[i]->add("SJ_Credit", tb);
            ...
            team[i]->apply_condition("hb_job_busy", 40);
        ...
        if( team[i]->query_condition("db_exp") )
		{
		  ...
		  //dbexp = jl/2 + random(jl/2); //恢复double
			dbexp = jl;
...
//双倍exp
		...
		{ 
            ...
            if (target)
			    ...
			    tell_object(team[i],HIY"你被奖励了"+chinese_number(jl)+"点经验，"+chinese_number(j)+"点潜能，"+chinese_number(gold->query("amount"))+"两黄金！\n"NOR);
...
//精英之书奖励
	...
	for(i=0; i<sizeof(obj); i++) {
		if(obj[i]->query_temp("protecting")==cart) {
			if (userp(obj[i]))
			{
				obj[i]->delete_temp("protecting");		
			}else destruct(obj[i]);
