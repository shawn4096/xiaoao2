...
int do_search(string arg)
{
  ...
  if(me->query(QUESTDIR5+"shuku")>3 && me->query("registered")<3)
  {
        tell_object(me,HIR"\n因为非VIP身份，你只有三次机会。\n"NOR);
 ...
 // if( me->query(QUESTDIR5+"shuku")>=4||random(2)||present("zhi tuan",me))
  //身上有纸团或第四次挖就不出现纸团
  if( me->query(QUESTDIR5+"shuku")>=4||present("zhi tuan",me))
  {
  ...
  obj=new("/d/xingxiu/baozang/obj/danwan");
...

  {
	  ...
	  obj=new("/d/xingxiu/baozang/obj/zhituan");
	  if(!clonep(obj)){
