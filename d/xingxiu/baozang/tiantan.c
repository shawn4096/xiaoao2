...
int i;
  ...
  {
     tell_object(me,YEL"突然你听到一个声音道：“宝藏已经被" + ob->query("busy") + "抢先一步了！”\n"NOR); 
     me->move("/d/xiangyang/damen");  
     me->delete_temp(QUESTDIR5+"search_ok");
      me->delete_temp(QUESTDIR5+"search");
     tell_object(me,YEL"你抬眼一看，这里竟然是武馆大门，刚才的一切像梦一样虚幻。\n"NOR); 
  ...
  //这里设置定时器
  me->apply_condition("baozang_time", 60);
  remove_call_out("check");
  call_out("check",1,me);
}
int valid_leave(object me, string dir)
{
        if ( dir == "up" && userp(me) && me->query(QUESTDIR5+"start"))
		{	
		   this_object()->delete("busy");
		   me->clear_condition("baozang_time");
		}
