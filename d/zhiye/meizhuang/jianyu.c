	...
	set("item_desc", ([
		...
		"门"   : "囚门似乎以前有人动过什么手脚。\n",
		    ...
		    "d/meizhuang/npc/renwoxing" : 1,
				...
				set("item_desc", ([
                "man": "这个人如同稀泥一般，双目涣散，没有一点神采。整个人似乎只剩下了一个干枯的躯壳，\n然而却还有一口气，如果不是铁镣锁着，也许可以把他搬下床来。\n",
        ]));
...
{       
        ...
        if (me->query_temp("get_xxbook"))
                return notify_fail("你没兴趣理这个半死不活"
                                   "的人，现在只想快点离开这里。\n");
        if (time()-me->query("quest/xxdf/time")<86400)
		{
			write("你解密冷却时间还不到，请满足一天后再来。");
            me->move("d/hz/baidi");
        }
			...
			&& random(i)>=27 )
       {
            ...
            log_file("quest/xixing",sprintf("%8s%-10s在梅庄地牢成功获得吸星大法拓本，复兴：%4d，雪山：%4d，失败：%2d，福：%2d。\n",
			me->name(1),"("+capitalize(me->query("id"))+")",me->query("job_time/日月复兴"), 
			me->query("job_time/雪山"), me->query("quest/xxdf/fail"),me->query("kar")),me);
            return 1;
       }
        else
        {
            write(HIY"你走上前去，却发现锁人的铁镣上面有一个细细的缺口，而且看上去前不久有人曾经扭开过，\n不禁有些迷惑，不知道有什么机关，索性就不再理床上那人。\n"NOR);
            write(HIG"你在囚室里转悠了许久，也没有发现什么特别的地方，于是就推开囚门，转身走了出去。你觉得\n此地甚是诡异，不可久留， 于是就不告而别，当下一个人悄悄出了梅庄。待到白堤，仔细回想，忽觉得那人床下石板似有玄机，可惜已经错过，不禁大感懊悔。\n"NOR);
            me->delete_temp("quest/xxdf/start");
