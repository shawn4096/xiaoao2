...
void init()
 {
     ...
     if(me->query_skill("tiezhang-zhangfa",1)<500
		 && me->query(QUESTDIR+"combat_exp") )
		 //&& me->query("combat_exp")-me->query(QUESTDIR+"combat_exp")<100000
       { 
		message_vision( HIW "$N看到兵器架上的武器，忽然若有所思。\n"NOR,me);
		message_vision( HIW "$N想了半天，但终因技能不足（500）级，没想起什么来，不由得摇了摇头。\n"NOR,me);
...
int thinking(object me)
{
  ...
  if(!me) return 0;
  if (time()-me->query("quest/tzautopfm/time")<86400) 
  {
	  message_vision(HIC"你来的太勤快了！\n"NOR,me);	  
	  return 0;
  }

  if(me->query_temp("try_auto")<(15+random(3)))
  ...
  else
   {
     ...
     if(i = 1 && random(me->query("kar"))>26)
     {
       tell_object(me,HIG"\n你一掌挥出，去势轻灵，宛若天成！却是一招柳叶刀法！\n"NOR);
       tell_object(me,HIW"\n你一掌挥出，中途忽然变为离魂斩的招式，！\n"NOR);

       message_vision(HIC"\n$N「哈哈哈」大笑几声，说道：“原来如此，原来如此。”\n"NOR,me); 
       tell_object(me,HIR"恭喜你，历经千辛万苦，你成功的把铁掌斧法和刀法武技融入了铁掌掌法中！\n"NOR);
       me->set("quest/tzauto/over",1);
	   me->set("quest/tzautopfm/time",time());
       log_file("quest/tzautopfm", sprintf("%s(%s) 失败%d次成功融合了铁掌武技。富源：%d；经验：%d。\n", me->name(1),me->query("id"), me->query(QUESTDIR+"fail"),me->query("kar"),me->query("combat_exp")) );
     }
    else
    {
       tell_object(me,HIG"\n你沉思许久，不得要领。\n"NOR);
       ...
       log_file("quest/tzautopfm", sprintf("%s(%s) 融合铁掌武技失败：%d次。富源：%d；经验：%d。\n", me->name(1),me->query("id"), me->query(QUESTDIR+"fail"),me->query("kar"),me->query("combat_exp")) );
    }
  }
