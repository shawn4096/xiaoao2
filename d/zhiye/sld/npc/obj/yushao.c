...
void create()
{
	set_name(HIW"玉哨"NOR, ({ "yu shao","shao" }) );
	set("long",HIG"这是一只用来训练神龙岛特定宝蛇的玉制哨。\n"+
		"主要指令有：blowshao(吹哨寻找蛇)\n"
	+"yinshe(引蛇)\n"
	+"fangshe(放下蛇)\n"
	+"nashe(拿起来蛇)\n"
	+"chushe(放蛇出洞)\n"
...
void init()
{
	add_action("do_findshe","blowshao");
	add_action("do_yinshe","yinshe");
...
int do_findshe(string arg)       
{
	...
	if (me->query_skill("qusheshu",1)<200)
		return notify_fail("你的驯蛇术不足200，无法驯养自己的宠物。\n");
...
{ 
	...
	ob->set("long",ob->query("long")+HIC"这是玩家"+me->query("name")+HIC"所驯化的宠物蛇！\n"NOR);
