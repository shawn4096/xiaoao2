...
string ask_job()
{
	...
	if (me->query("combat_exp")<10485761) return "你还是抓紧时间练功吧。";
	
        //return ("最近大海风浪比较大，暂时无法航行，请今天晚上在来。")
	if(!DZD_D->query_mode() && !me->query_temp("ask_dzd")) return 0;
	if(DZD_D->query_stat()=="ingame")
		return "你来晚了，已经开始了。";
	if(DZD_D->query_stat()=="overbusy")
		return "大智岛正在重置呢，等会再说。\n";	
	if( !DZD_D->query_mode() && ( me->query_condition("job_busy") ||  me->query("job_name")==DZD_D->query_name())) return "您现在这么忙，还想尝试什么啊？\n";
	if(!DZD_D->query_mode() && mapp(me->query_conditions_by_type("poison"))){
		command("hmm "+getuid(me));
		return "您身中剧毒，怕是承受岛上煎熬。\n";
	}
	if(!DZD_D->query_mode() && mapp(me->query_conditions_by_type("hurt"))){
		command("hmm "+getuid(me));
		return "想占这个现成便宜？\n";
	}//over back I will clear_condition so return of poison for forbid player use bug	
	
	if(!DZD_D->query_mode() && me->query_condition("killer")) return DZD_D->query_name()+"可不想跟官府扯上什么关系。\n";
	if(!DZD_D->query_mode() && me->query("combat_exp") <20000) return "您的武功未成，不去也罢。\n";
   if(!DZD_D->query_mode() && wizardp(me) && getuid(me)!="anger") return "您老就别来捣乱了。\n";
	switch(time = DZD_D->add_player(me,1))
	{
		case -3:command("wawa");
			return "船已经开走啦。\n";
		case -2:command("tm3");
		...
		case 0:command("sorry");//debug
			return "工作人员说了，"+DZD_D->query_name()+"怕是得要修整几天了。\n";
	}
	...
	command(sprintf("whisper %s %s请与%s前到佛山港口，自有人接待贵客上船。"NOR,getuid(me),HIW+"",CHINESE_D->chinese_date(time)));
	//XKD_D->demon_tell(2,me,sprintf(HIW"您看这铜牌后面刻有：%s到小渔村，自有人接待贵客到侠客岛。\n"NOR,CHINESE_D->chinese_date(time)) );
	return RANK_D->query_respect(me)+"乃当时英雄，便请到"+DZD_D->query_name()+"一行，生死各安天命。\n";
}
