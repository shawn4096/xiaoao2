...
//	因为gold不缺，目前设置成只加pot，等darken的药品系统完成后再改回加gold。
	...
	me->add("potential", value/100);
	if (wizardp(me)) 
		tell_object(me,"这块矿石的价值是：" + value + "。\n");
	message_vision("$N将一" + obj->query("unit") + obj->name() + "交给$n。\n", me, ob);
	tell_object(me, "你得到了" + CHINESE_D->chinese_number(value/100) + "点潜能。\n");
...
//随机奖励种类。
	...
	if (me->query("potential") > me->query("max_pot") * 2)
	{

		tell_object(me,"可惜很遗憾，受经验所限，你损失了"+chinese_number( value / 100 - (me->query("max_pot") * 2 - pot))+"点潜能。\n");
