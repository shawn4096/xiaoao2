...
void init()
{
	...
	if (!query_temp("leitai") && base_name(me) == "/d/xiangyang/leitai/npc/leitaiuser") {
		me->quit(me);
		return;
	}
...
int do_ok()
{
	...
	if (!me->query_temp("leitai") || base_name(me) != "/d/xiangyang/leitai/npc/leitaiuser")
...
int do_tui(string arg)
{
	...
	if (!ob->query_temp("leitai") || base_name(ob) != "/d/xiangyang/leitai/npc/leitaiuser")
...
int load_user(object player)
{
	...
	|| base_name(player) == "/d/xiangyang/leitai/npc/leitaiuser")
		return 0;

	seteuid(arg);
	user = new("/d/xiangyang/leitai/npc/leitaiuser", arg);   // 去掉了save()，擂台中的一切和本身无关。
	player->save();
	if (!user->restore()) {
		destruct(user);
...
int do_load(string arg)
{
	...
	|| base_name(ob) == "/d/xiangyang/leitai/npc/leitaiuser")
		return notify_fail(ob->name()+"正在别的擂台比武中。\n");

	seteuid(arg1);
	user = new("/d/xiangyang/leitai/npc/leitaiuser", arg1);   // 去掉了save()，擂台中的一切和本身无关。
	if(!user->restore()) {
		destruct(user);
