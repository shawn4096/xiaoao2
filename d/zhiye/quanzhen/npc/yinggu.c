...
int ask_fox()
{
	...
	if (me->query_temp("quest/shendiao/大雪风陵渡/askheal"))
	{
		...
		me->set_temp("shendian/大雪风陵渡/askying",1);
	}else {
...
int ask_nqgjj()
{
	...
	ob = this_object();
	me = this_player();
	if (!me->query("quest/shediao/nqg/over")&&!me->query("shendiao/大雪风陵渡/yingok"))
	{
		return notify_fail("瑛姑不愿意搭理你，嫌你太懒了,一点贡献都不愿意为她去做！\n");
	}
	...
	if (me->query("quest/shediao/nqg/pass"))
	{
		 ...
		 return notify_fail( "你已经彻底领悟了这门绝学，在这里啰嗦什么？");
    }
	if (time()-me->query("quest/shediao/nqg/time")<86400)
	{
  		command("nod "+me->query("id"));
		return notify_fail( "你来的也太勤快了，烦不烦！？");
    }
