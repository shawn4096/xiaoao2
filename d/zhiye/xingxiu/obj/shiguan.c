...
上角，以屈下颊至；又从耳後入耳中，出走耳前，交颊。三路合于一处！\n"NOR);
        ...
        if(period==times){
               ...
               // tell_room(environment(me), HIY+me->name()+"缓缓收回双掌，笼罩着全身的青气也随之消散了。\n"NOR, ({ me }));
                message_vision(HIY+me->name()+"缓缓收回双掌，笼罩着全身的青气也随之消散了。\n"NOR,me);
...
int do_open(string arg)
{
	...
	{
		return notify_fail("你不是星宿派弟子，来此作甚？");
	}
	if (me->query_skill("huagong-dafa",1)<200)
	{
		return notify_fail("你的化功大法不够200，推不开这个棺盖。\n");
	}
	if (me->query_skill("poison",1)<100)
	{
		return notify_fail("你的毒计水平不够100，推不开这个棺盖。\n");

	}
	if (query("book")<1)
	{
		return notify_fail("书籍已经被人拿走了。\n");

	}
	message_vision(HIC"$N运起化功大法，屏住呼吸，运劲于掌，将石棺盖打开。\n"NOR,me);
	message_vision(HIY"\n\n$N凝神一看见棺材内封藏了一本破旧的书籍，急忙拿将出来揣入怀中。\n"NOR,me);
	book=new("d/xingxiu/obj/dujing_3");
	book->move(me);
	add("book",-1);
	return 1;
