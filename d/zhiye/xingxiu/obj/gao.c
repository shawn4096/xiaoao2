...
int do_chang(string arg)
{
	...
	if( this_object()->query("xx/owner")!=who )
		return notify_fail("这不是你亲自由毒物炼制的药膏，你也敢尝？\n");
	...
	//设置嫡传标记
	if (random(who->query("job_time/星宿熬膏"))>800 && random(who->query_kar())>25&& !who->query("quest/xx/dichuan"))
	{
		command("say 真不错，老仙跟我说了，让我好好发现门中的好苗子，就是你了！\n");
		command("pat "+who->query("id"));
		who->set("title","星宿派嫡传弟子");
		who->set("quest/xx/dichuan",1);
	}
	destruct(me);
	return 1;
}
