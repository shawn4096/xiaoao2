...
void create()
{
	set_name("竹笛", ({ "duan di", "di",}));
	set("weight", 100);
	if(clonep()) set_default_object(__FILE__);
	else{
		set("unit", "只");
          	set("long", "这竹笛短得出奇，只不来七寸来长，普通的一个牧童常用的竹笛。\n" );
	  	set("value", 50000); 
          	set("no_get", 1);
			set("no_give", 1);
          	set("di_number", 0);
...
void done(object me)
{
	...
	///TASK_D->give_reward(me, me->query_temp("quest"), -1); //屏蔽
	//TASK_D->give_reward(me, me->query_temp("quest"), -1, "丁春秋" ,"星宿");
	//me->add("job_time/星宿", 1);
    
	TASKREWARD_D->get_reward(me,"星宿",-1,0,0,20+random(2),0,0,0,this_object());
