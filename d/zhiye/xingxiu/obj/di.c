...
void init()
{
		...
		add_action("fail_task", "shibai");
		add_action("fail_task", "fail");
		add_action("fail_task", "sorry");
...
int fail_task()
{
	...
	if(!me->query_temp("quest/id")) 
		return notify_fail("你现在还没有接收到任何任务,无需放弃。\n");

	if(me->query_temp("quest/type") == "杀")		
	{
		write(me->query("name")+"，你放弃了此次杀"+me->query_temp("quest/name")+"的任务！\n");

	}else {

		write(me->query("name")+"，你放弃了此次为老仙寻宝"+me->query_temp("quest/name")+"的任务!\n");

	}
	me->delete_temp("quest");
	me->apply_condition("job_busy", 10+random(10));
	return 1;
...
int set_task(string arg)
{
	...
	if(me->query_temp("quest/family") == "丐帮")
		return notify_fail("老仙最讨厌臭叫化子，先扔掉你的丐帮身份再说！\n");
...
int mark_success(string arg)
{
	...
	me = this_player();
	//增加防止bug 判断
    if (me->query_temp("quest/killover")) return notify_fail("你已经完成杀人任务了，赶紧回去跟老仙交付任务去吧。\n");
	...
	if(me->query("family/family_name") == "星宿派")
	{
		...
		me->set_temp("quest/corpse",target->query("id"));
	...
	//call_out("done", 1, me);  
	message_vision(HIG"$N已经将尸体烧焦，任务完成，可以回去跟老仙交付任务了。\n"NOR,me);
	me->set_temp("quest/killover",1);
