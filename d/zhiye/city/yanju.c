...
{	
	...
	if ((time()-query("shihui/time")>(86400*7)) && (query("hui")==0)) 
	{
		set("hui",5);
		}
}
...
int do_zhua(string arg)
{
	...
	if (time()-query("shihui/time")<(86400*7)) return notify_fail("此地石灰已用完！\n");
	...
	ob1->move(ob);
	add("hui",-1);
	if (query("hui")==0) set("shihui/time",time()); //记录抓完时间，一周后刷新 by renlai
