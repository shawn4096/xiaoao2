...
//							u_pfm = u_pfm + ({(: perform_action, "sword.hebi" :)});        	//两仪剑法pfm：双剑合壁 
		...
		case "9y":
			...
			set_skill("jiuyin-zhengong",501);												//九阴内功：九阴真功	
			set_skill("jiuyin-shenfa",501);													//九阴轻功：九阴身法	
			set_skill("daode-jing",200);
            set_skill("yinlong-bian",501);	
            set_skill("whip",501);			
			map_skill("force", "jiuyin-zhengong");
			map_skill("dodge", "jiuyin-shenfa");
			u_pfm=u_pfm + ({(: exert_function, "yihun" :)});   
			...
			switch(in_level)
			{
				case 0:
					switch(random(2))
					{
						case 0:
                            ...
                            set("quest/jiuyin2/jybgz/pass",1);//设置标记							
						break;
						case 1:
					...
					switch(random(1))
					{
						case 0:
							...
							map_skill("claw","jiuyin-baiguzhua");
							prepare_skill("claw", "jiuyin-baiguzhua");
						    set("quest/jiuyin2/jybgz/pass",1);
							set("quest/em/zzr/pass",1);
				...
				case 1:
					...
					{
						case 0:
						    set("quest/jiuyin1/pass",1);
							set_skill("jiuyin-shenzhang",501);                              //九阴低级空手：九阴神掌
							map_skill("strike","jiuyin-shenzhang");
							prepare_skill("strike", "jiuyin-shenzhang");
							set_skill("jiuyin-zhengong",501);                              //九阴低级空手：九阴神掌
							map_skill("force","jiuyin-zhengong");
							set_skill("jiuyin-shenfa",501);                              //九阴低级空手：九阴神掌
							map_skill("dodge","jiuyin-shenfa");
							set_skill("yinlong-bian",501);									//九阴高级兵器：银龙鞭法	
							map_skill("whip","yinlong-bian");
							w_parry="yinlong-bian";
							u_pfm = u_pfm + ({(: perform_action, "whip.chanrao" :)});					
							u_pfm = u_pfm + ({(: perform_action, "whip.xuanyin" :)});      //pfm：玄阴诀 	
							u_pfm = u_pfm + ({(: perform_action, "whip.feilong" :)});			//feilong诀
...
if (nw_parry=="")
{
	switch(random(2))
	{
		...
		case 1:
			...
			u_pfm = u_pfm + ({(: perform_action, "cuff.hengsao" :)});
