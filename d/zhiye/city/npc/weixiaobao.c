...
int ask_fight()
{
	...
	command("claw");
	//int i = random(5);
	//if(!environment(ob)->query("no_fight")) return 0;
...
int accept_fight(object ob){
        command("cry2");
		if (ob->query_temp("give_wei_money_gived"))
		{
			ob->set_temp("quest/baibian",1);
		    kill_ob(ob);
		}
        return 0;
}
...
//"quest/sld/yysf/baibian/pass"
//神龙岛专属游龙身法的百变quest by renlai
string ask_baibian()
{
  object me;
        me=this_player();
	     ...
	     if ((int)me->query_skill("youlong-shenfa",1) < 350) {
                command("say 你的游龙身法修为不够！");
                return "\n你的修为不够。\n";
        }	    
        
	   //增加时间和经验间隔限制
	   if (me->query("quest/sld/yysf/baibian/pass"))
		{
			command("heng ");
			command("say 你不是已经解开百变了么？还来此跟我啰嗦什么？\n");
			return "你已经解开百变了。";
		}
	   
	    if(me->query("quest/sld/yysf/baibian_time")&& (time()-me->query("quest/sld/yysf/baibian_time",1))<86400)
	    {
		   command("shake "+me->query("id"));
		   if(random(2)) return "今天很忙了，"+RANK_D->query_respect(me) +"先行休息去吧！";
		   else return "学习游龙百变也不用急在一时，"+RANK_D->query_respect(me) +"先行休息去吧！";
	    }
		
        me->set("quest/sld/yysf/baibian_time",time());	    
		
		if (random(me->query("kar")) > 28)
	     { 
            command("haha");
		    command("pat "+me->query("id"));
		    message_vision(HIY"韦小宝附耳在你的耳边悄声说了一番道理.....\n"NOR,me);
		    me->set("quest/sld/yysf/baibian/pass",1);
			log_file("quest/youlong",sprintf("%-18s失败多次后，从韦小宝处学到游龙百变，福：%d。\n",
              			me->name(1)+"("+capitalize(getuid(me))+")",me->query("kar")), me);
		    command("pat "+me->query("id"));
			return "恭喜你，你已经解开游龙百变。";
		 }	 
		 return "‘看你面带晦气，我心情不好，改日再来吧！’\n";
