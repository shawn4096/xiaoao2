...
void create()
{
       ...
       set("max_neili", 10000);
       set("jiali", 210);
       set("combat_exp", 3180000);
       set("no_get",1);
       set("no_bark",1);
       set_skill("force", 260);
        set_skill("yinyun-ziqi", 260);
       set_skill("dodge", 260);
...
int ask_shenpian()
{
	...
	if(!me->query("quest/雪山飞狐/解药篇/over"))
	{
		...
		command("say 你连解药篇都没搞完，我凭什么给你这本书？");
		command("angry ");
		command("heng ");
		return 1;
	}
	command("say 好吧，既然你这么厉害，我就把这本书传给你，但你不能凭此祸害江湖，否则我一定用七星海棠毒死你。\n");
	command("heng");
	book=new("d/miaojiang/obj/yaowang_shenpian");
	book->move(me);
	command("rumor "+me->query("name")+"机缘巧合，得到了一部《药王神篇》!\n");
	return 1;	
