...
void init()
{
  	...
  	if (me->query_temp("thd/tupu")||me->query_temp("qmbg/qimendunjia"))
        {
...
int do_fan(string arg)
{
	...
	if (arg=="书架" || arg == "shujia"|| arg == "奇门遁甲")
	{
        ob = unew(__DIR__"obj/qimendunjia");
                
		if(!clonep(ob))
                	return notify_fail("你来晚了，《奇门遁甲》已经被人拿走了。\n");
        	if(clonep(ob) && ob->violate_unique()){
           		destruct(ob);
                	return notify_fail("你来晚了，《奇门遁甲》已经被人拿走了。\n");
                }
                write("你仔细的翻了翻书案，找到了一本关于《奇门遁甲》精义的书!\n");
                write("你如获至宝，赶忙捡了起来。\n");
                ob->move(me);
                me->delete_temp("qmbg/qimendunjia");
                return 1;
      }
	...
	if (arg=="乐理" || arg == "音乐")
	{
        	ob = unew("d/music/obj/flute-book2.c");
                
		if(!clonep(ob))
                	return notify_fail("你来晚了，萧技提高篇已经被人拿走了。\n");
        	if(clonep(ob) && ob->violate_unique()){
           		destruct(ob);
                	return notify_fail("你来晚了，萧技提高篇已经被人拿走了。\n");
                }
                write("你仔细的翻了翻书案，找到了一本关于萧技提高篇的书!\n");
                write("你如获至宝，赶忙捡了起来。\n");
                ob->move(me);
               //me->delete_temp("thd/tupu");
                return 1;
