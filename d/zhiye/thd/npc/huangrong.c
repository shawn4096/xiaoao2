...
int ask_job_time()
{
	...
	if (random(i)>1200)
	{
		command("say 别贪玩，你再继续努力去?\n");
		command("say 既然你如此勤奋，辛苦守墓，我就将我爹爹的萧技高级篇作为奖励给你!\n");
		ob=new("d/music/obj/flute-book3.c");
		write("你仔细的翻了翻书案，找到了一本关于萧技高级篇的书!\n");
		write("你如获至宝，赶忙捡了起来。\n");
		//return 1;
...
int ask_h7g()
{
	...
	command("say 洪七公乃是我的恩师，他老人家虽然生性滑稽，但却是食客中的状元。\n");
...
int ask_xlz()
{
	...
	if (me->query_skill("xianglong-zhang",1)<450) {
		command("say 你的降龙掌还没大飞，就想学绝招？");
		return 1;
	}	
	if (me->query_temp("slbw/askhuang1"))
	{	
	command("say 降龙十八掌乃是我丐帮绝学，非核心弟子不得传授。\n");
	command("say 不过看在你是我师弟的份上，我偷偷告诉你一个秘密。\n");
	command("say 当初之所以郭靖哥哥能拜师并得以传授降龙十八掌正是因为吃了叫花鸡。\n");
	me->delete_temp("slbw/askhuang1");
	me->set_temp("slbw/askhuang2",1);
	return 1;
	}
	else command("say 这是我丐帮绝学，不可轻易传于人。\n");	
	return 1;
}
int ask_guojing()
{
	...
	{	
	command("blush ");
	command("say 郭靖哥哥乃是我的夫君。。，。\n");
	command("say 不过他当时虽然被七公传授了十五掌，另外三掌并没有传授，降龙十八掌只有学完才会融汇贯通，威力绝大。\n");
...
int ask_slbw()
{
	...
	if (time()-(int)me->query("xlz/slbw/time")<86400) {
		command("say 你真够勤快的，时间不到，别来烦我？\n");
		return 1;
	}
	if (me->query("slbw/pass")) {
		command("say 你不是已经解开了么，还过干嘛？\n");
		command("angry ");
		command("say ");
		return 1;
	}
	if (me->query_temp("slbw/askhuang3"))
	{
	  command("consider ");
	//command("say 郭靖哥哥乃是我的夫君。。，。\n");
	  command("say 那最后三招乃是时乘六龙、履霜冰至、神龙摆尾，其中神龙摆尾乃是最为厉害的一招。\n");
	  command("say 不过七公传授给我的是打狗棒，降龙掌只传给了靖哥哥，你可以向他详细询问此事。\n");
	...
	{
        command("say 神龙摆尾乃是我丐帮绝学，轻易不会传与别人。\n");
	    return 1;
	}
}