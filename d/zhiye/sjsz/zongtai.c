...
int valid_leave(object me,string dir)
{
   ...
   if ( !me->query_temp("get_in_sjsz") && dir == "enter") //&& !wizardp(me)
           return notify_fail(HIR"\nÄãÃ»ÓĞ»ñµÃĞí¿É£¬»¹ÊÇ²»ÒªÓ²´³µÄºÃ¡£\n"NOR);
    if(dir=="enter" && me->query_temp("get_in_sjsz"))
        me->apply_condition("sjsz_time",15);


	if ( me->query_condition("killer") && dir == "enter")
		return notify_fail("É±ÈË·¸ÎŞ·¨½øÈëÊÔ½£É½×¯¡£\n");
...
int do_set2(string arg)
{
   int time1, time2, time3;
	//object me = this_player();
	mixed *local;
	
	//userp(me)&&!present("shijian ling", me)&&
...
void sjsz_reset()
{
	...
	if ( local[2]%3 < 1 && local[2] > 8 && sjsz["find_time"]+3600 < time()) {
		...
		for (i=0 ; i < j; i ++) {
			if ( !room = find_object("/d/sjsz/"+(file[i])) )
				room = load_object("/d/sjsz/"+(file[i]));
		...
		message("channel", HIW"\n\t"+CHINESE_MUD_NAME+HIW"µÄÊÔ½£É½×¯ÄÚÌıËµÓĞÄÜ¸Ä±äÈİÃ²ÔöÇ¿¹¦Á¦µÄÉñÆæÒ©ÎïÒÔ¼°¿ÉÒÔ´òÔì³ö¼«Æ·ÎäÆ÷×°±¸µÄÓñÊ¯£¡\n\n"NOR, users());
