...
int ask_feidao()
{
   ...
   {
  	  message_vision(YEL"$N笑了下，飞刀乃是我纵横江湖之物，你先解完这个谜题我再给你。\n"NOR,this_object()); 
	  command("say 别偷懒。");
	  command("think");
    ...
    if (query("feidao")<1)
	  {
		  command("say 你来晚了，飞刀已经送人了。\n");
		  return 1;
	  }
	  add("feidao",-1);
	  new("d/huanghe/obj/feidao")->move(this_object());
	  command("give feidao to "+me->query("id"));
	  command("say 好吧，这套飞刀就给你了，好自为之。\n");
	return 0;
...
int ask_hujiadaofa() 
{
  ...
  //非vip玩家，只能失败三次
  if((me->query(QUESTDIR4+"hujiadaofa_fail")>=3 || QUEST_D->queryquestfail(me,"hujiadaofa")>=3) && me->query("registered")<3)
  {
	  command("shake "+me->query("id"));
	  command("say 现在时间紧急，这刀法还是以后切磋吧。");
	  return 1;
	...
	//注意这个标记和苗家剑法使用同一个标记。
	if((me->query(QUESTDIR4+"time") && time()-me->query(QUESTDIR4+"time")<86400) || QUEST_D->queryquesttime(me,"hujiadaofa"))
	{
	  command("shake "+me->query("id"));
	  command("say 今天先切磋到这里吧，明天吧。");
	  return 1;
  }
   if(me->query(QUESTDIR4+"combat_exp") 
	   && me->query("combat_exp")-me->query(QUESTDIR4+"combat_exp")<500000 
	   &&!me->query("cw_exp"))
	{
	...
	for ( i = 0; i < 6+random(6);i++)
	  {
         ...
         else
         {
            ...
            COMBAT_D->do_attack(ob, me, ob->query_temp("weapon"), random(3)?3:1);
  ...
  power_condition(me,ob,350+random(200));
  COMBAT_D->do_attack(ob, me, ob->query_temp("weapon"), 1);
  if(random(2)) COMBAT_D->do_attack(ob, me, ob->query_temp("weapon"), 3);
  if(random(2)) COMBAT_D->do_attack(ob, me, ob->query_temp("weapon"), 3);
  start_condition(ob);
	command("sneer");
	return 1;
...
int thinking(object me, object ob)
{
  ...
  if(me->query_temp(QUESTDIR4+"thinking")<(3+random(3)))
    ...
    //如果有特殊通宝物品，略微提高成功几率。  by lsxk@hsbbs 2007/7/20
	  ...
	  //quest/雪山飞狐/武功/
	  if (QUEST_D->easy_questing(me,10,"hujiadaofa"))
      {
       tell_object(me,HIG"\n你瞬间脑中电闪雷鸣，瞬间明悟其中，突然间明白了一切。\n"NOR);
