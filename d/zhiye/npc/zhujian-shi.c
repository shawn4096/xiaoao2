...
int do_answer(string arg)
{
  ...
  //  if(wizardp(me) ) return notify_fail("你老人家不是一直用SMASH的么？\n");
	if(!me->query_temp("zj_answer")) return 0;
	if(!arg) return notify_fail("你想说什么？\n");
	//提示下：
	if (arg=="lun") notify_fail("轮已经取消打造。");
...
在出售自己制造的武器之外，他也做收购兵器的生意。");
	...
	set("vendor_goods", ({
		(["name": BINGQI_D("sword/sword"),"number":50]),
		(["name": BINGQI_D("brush"),"number":50]),
		(["name": BINGQI_D("qiang"),"number":50]),
		(["name": BINGQI_D("blade/blade"),"number":50]),
		(["name": BINGQI_D("hammer/chui"),"number":50]),
		(["name": BINGQI_D("club/gun"),"number":50]),
		(["name": BINGQI_D("staff/gangzhang"),"number":50]),
		(["name": BINGQI_D("whip/whip"),"number":50]),
		(["name": BINGQI_D("hook"),"number":50]),
		(["name": BINGQI_D("stick"),"number":50]),
		(["name": "d/city/obj/tiejili","number":50]),
        (["name": BINGQI_D("axe"),"number":50]),
        (["name": BINGQI_D("dagger"),"number":50]),
	}));
...
string ask_reward()
{
	...
	me->add("job_time/送剑任务",1);
	jobs=me->query("job_time/送剑任务");
	//me->set("job_name","送剑任务");
	
	if (random(jobs)>100
		&&random(me->query("kar"))>20
		&&me->query_skill("baduan-jin",1)<220
		&&!me->query("family"))
	{
		command("pat "+me->query("id"));
		command("say 你这娃娃真不错!\n");
		command("say 我曾在江湖流浪之际曾偶遇华山一位叫做神剑轩辕的老前辈，他传给我一本叫做[八段锦]的功夫！\n");
		command("say 我一直修习至今，从没间断过，对改善体质很有帮助，今日有缘就送给你好了！\n");
		
		ob=new("clone/book/baduanjin");
		me->set("quest/baduanjin/pass",1);

		ob->set("owner",me->query("id"));
		ob->move(me);
		message_vision(HIY"铸剑师送给$N一本神秘的武功秘籍\n",me);
	}
...
string ask_baduanjin()
{
	...
	if( !me->query("quest/baduanjin/pass"))
		return "你没事到我这儿来瞎扯什么？\n";
	if (me->query_temp("baduanjian/give")&&me->query_skill("baduan-jin",1)<220)
	{
		return "我已经给你一本了，真是贪得无厌！\n";
	}
	command("consider ");
	ob=new("clone/book/baduanjin");
	ob->set("owner",me->query("id"));
	ob->move(me);
	me->set_temp("baduanjian/give",1);
	
	command("say 既然你如此努力我就把我手头上这本书送给你了，望你好自为止。\n");
	return "以后不要再将秘籍弄丢了！\n";
...
string ask_bdjmm()
{
	...
	if (me->query_skill("baduan-jin",1)>400)
	{
		me->set_temp("baduanjian/quest",1);
		return "传说八段锦和太祖拳渊源颇深，而太祖拳本为宋太祖赵匡胤所创，他的后人应该会知道些什么吧。\n";
	}
	else
		return "你八段锦造诣如此之浅薄，问之无益。";
}
