...
int do_tong(string arg)
{
	...
	if ( arg =="fengchao"){
		...
		if ( !wizardp(me) && me->query_skill("qufengshu",1) > 200 )
			return notify_fail("你驱蜂术武功已经够高，采蜂蜜的事就让小弟子们去做吧。\n");
		if ( me->query_temp("gm_fengmi") < 10 + random(5)){
			...
			if ( j > 2 || random(10) > i || me->query("combat_exp") < 10000 ){
				...
				me->add_busy(1);
				me->receive_damage("jing",5+random(5),"被蜜蜂蛰");
				me->receive_damage("qi",5+random(10),"被蜜蜂蛰");
				if (me->query_skill("qufengshu",1)<200)
				{
					me->improve_skill("qufengshu",(int)me->query_int());
				}
