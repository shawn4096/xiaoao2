...
void create()
{
	set_name(GRN"『琴技入门(一)』"NOR, ({ "art's book", "book" }));
	set_weight(500);
	if( clonep() )
...
int do_du(string arg)
{
	...
	if( j < 50 )
		return notify_fail("你的读书写字等级不够50，无法理解书中记载的「乐理琴技」。\n"); 
	...
	me->receive_damage("jing", ob->query("jing_cost"),"精力耗尽而"); 
	me->improve_skill("art", me->query_int()/5 + 3);
