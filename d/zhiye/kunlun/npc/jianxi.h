...
void kill(object ob)
{
	command("say 你一直跟我这么多天，既然躲无所躲，大爷今天就超渡你！\n");
	kill_ob(ob);
}
...
void kill_ob(object ob)
{
	if (ob->query("id") != query("owner")) 
	{
		...
		tell_object(ob, "这不是昆仑何掌门让你追杀的奸细。\n");
		return;
	}
	::kill_ob(ob);
}
...
//	if (!living(ob)) {
		...
		//return;
	//}
	//if (count >= 5) command("say 老子终于逃出生天，昆仑弟子真懒！");
	//else command("say 他奶奶的，这次真是命大，老子算是跟昆仑派结下梁子了！");
	write(name()+"哼着小曲，一身轻松地离开。\n");
