...
#define SKILLS_LEVEL            120

#define KNOWLEDGE_LEVEL         300

#define GIVE_EXP                175000

#define MAX_NEILI               3000

#define MAX_JINGLI              3000
...
void create()
{
    set_name(HIW"书剑礼物使者(初级)1"NOR, ({ "shizhea" }));
...
string ask_xingxiu()
{
	...
	this_player()->set_skill("zhuihun-goufa",SKILLS_LEVEL);
...
string ask_xuedao()
{
    ...
    if (!this_player()->query("shizhe_newbie")) return "你必须要先ask shizhe about newbie！？\n";
    message_vision(HIY "使者"+ NOR "一挥手，一阵狂风将$N吹得无影无踪了。"+ NOR "\n",this_player());
    this_object()->create_family("血刀门", 2, "弟子");
    this_object()->recruit_apprentice(this_player());
    this_object()->delete("title");
    // this_player()->move("/d/xingxiu/xxh");
  //   this_player()->set("job_time/雪山",500);
