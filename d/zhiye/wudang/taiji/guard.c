...
string *title2 = ({ "蒙面人", "黑衣人", "贴身护卫", "护院", "护卫"});
...
int do_copy(object target,int mode)
{
	...
	if(mode) {
		t = mode+random(5);
		set("long",query("long")+"最近治安不太好，这个保镖是有人特别从京城请来的，据说还曾经是大内高手呢！\n");
	...
	//exp = exp * (100+random(50))/100;
    
	me->set("max_qi", (int)ob->query("max_qi")*t/10 );
	me->set("eff_qi", (int)ob->query("max_qi")*t/10 );
	me->set("qi", (int)ob->query("max_qi")*t/10 );
