...
int do_kanshou()
{
       ...
       if(!present("yu daiyan",  environment(ob1) ) )
                return notify_fail("什么？\n");

       me = present("yu daiyan",  environment(ob1) );
       fam = ob1->query("family");
       exp = ob1->query("combat_exp");

	   if (query("dan_busy")) {
		        ob1->add_busy(2);
	            return notify_fail("稍等。\n");
	   }   
	   
	   
       if(ob1->is_busy()) return notify_fail("你现在正忙着呢。\n");
       
	   
	   if (ob1->query_temp("wd_job1"))
       ...
       if(ob1->query_condition("wdks_job") > 0 )
             return notify_fail("你还是先去歇息片刻再来吧。\n");

       if(ob1->query("job_name") == "看守铜鼎")
		{
            message_vision(CYN"俞岱岩说道："+ob1->query("name")+"，你刚做完守鼎任务，还是先去歇息片刻再来吧！\n"NOR,ob1);
            return 0; 
		}
	   if(ob1->query_condition("job_busy") >= 1){
            message_vision(CYN"俞岱岩说道："+ob1->query("name")+"，你正工作忙碌中，还是先去歇息片刻再来吧！\n"NOR,ob1);
            return 0; 
       }
/*
...
*/
		...
		else call_out("do_attack",120+random(80),ob);
...
int do_leave(object ob)
{
        ...
        ob->apply_condition("wdks_job",16);// add by caiji@SJ
        //busy must > 15,because of a bug
...
int do_attack(object ob)
{
        ...
        exp = ob->query("combat_exp")+10000;
    //    lv = ob->query("max_pot") - 90 + random(10);
        lv = ob->query("max_pot") - 99;
       /*
...
int give_reward(object ob)
{
	...
	if(ob->query_condition("wdks_job")||  //add by caiji@SJ
	...
	if (random(ob->query("job_time/看守铜鼎"))>500 && random(ob->query("kar"))>25 && !ob->query("quest/wd/dichuan"))
	{
		
		message_vision(HIC"俞岱岩对你竖起了大拇指，说道，“经过考验，我接受你为武当核心弟子”\n"NOR,ob);
		ob->set("title","武当派嫡传弟子");
		ob->set("quest/wd/dichuan",1);
	}
	...
	tell_object(ob, GRN"你此次看守共得到"+CHINESE_D->chinese_number(e)+"点经验，"+CHINESE_D->chinese_number(p)+"点潜能,"+CHINESE_D->chinese_number(shen)+"点正神！\n"NOR);
