...
//    me->set("disable_type","<静坐中>")
          call_out("do_wait1", 5+random(5), me);       
          }
}
...
\n"NOR);
          me->disable_player("<静坐中>");
          call_out("do_wait2", 5+random(5), me);       
          }
}
...
\n"NOR);
          me->disable_player("<静坐中>");
          call_out("do_wait3", 5+random(5), me);       
          }
}
...
//          me->clear_condition();
          me->disable_player("<静坐中>");
          call_out("do_wait4", 5+random(5), me);       
          }
}
...
message_vision(HIY"$N已经和大自然容为一体，过了片刻，精神抖擞的站了起来。\n"NOR, this_player());
		 ...
		 //提升大乘佛法
		 if (me->query_skill("dacheng-fofa",1)<200)
		 {
