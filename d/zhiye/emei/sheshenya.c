...
int do_jump(string arg)
{
	...
	if (me->query("buyvip"))
		j=(me->query("relife/exp_ext")+me->query("relife/gifts/total"))*1600000;
	else if (me->query("y-card-vip"))
		j=(me->query("relife/exp_ext")+me->query("relife/gifts/total"))*1700000;
	else if (me->query("m-card-vip"))
		j=(me->query("relife/exp_ext")+me->query("relife/gifts/total"))*1800000;
	else j=(me->query("relife/exp_ext")+me->query("relife/gifts/total"))*2000000;
	...
	if(me->query("int")<40)
	{
		return notify_fail("你的先天悟性不足40，跳下去有死无回？\n");
	}
	...
	//增加经验值判断
	if ((me->query("quest/jiuyin2/fail") >= (int)((me->query("combat_exp") - 1000000)+j)/100000)
		||me->query("combat_exp",1)<5000000
		|| (time() - me->query("quest/jiuyin2/time")) < 86400)
	{
		...
		if(local[2] > 13 && local[2] < 15 )
		{
			if (random(me->query("kar")) > 23
				&& random(me->query_dex(1)) > 50 && me->query("family/family_name")=="峨嵋派")
			{
				call_out("do_wait", 2, me);
			}
			//else if ( me->query("hanshui_food") && random(10) == 3 && me->query("family/family_name")=="峨嵋派" )
			//{
				//call_out("do_wait", 2, me);
			//}
			else 
...
{
	tell_object(me, HIR"你只觉得耳边呼呼风声作响，身体飞快的向下缀去....\n"NOR);
