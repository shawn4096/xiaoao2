...
int do_move(string arg)
{
	...
	//!me->query("hanshui_food") &&
	if( random(dex) < 18 && random(2) ) {
...
//原来只有random(30)>5 增加绝世的难度


    ob->add("count", -1);
	if( random(me->query("kar")) < 20 || random(30) < 10) {
        ...
        write("只听见轰隆之声不绝，你吓得赶紧爬出了墓穴。\n", me);        
		me->move("/d/chengdu/tulu1");		
		return 1;
	}
	if (ob->query("count") > 0) {
		switch(random(2)) {
