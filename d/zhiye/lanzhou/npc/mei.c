...
string jybgz()
{
	...
	if( QUEST_D->queryquest(me,"jiuyin2/jybgz") )
	{
		...
		{
		//me->set("title",HIB"铁尸·白骨"NOR);
		//me->set("mytitle/quest/jybgztitle",HIB"铁尸·白骨"NOR);
		QUEST_D->setmytitle(me,"jybgztitle",HIB"铁尸·白骨"NOR);
  ...
  if (!wizardp(me)) {
	...
	if (QUEST_D->queryquesttime(me,"jiuyin2/jybgz"))
	...
	//改用公共解密函数 by renlai
	  ...
	  // 1，代表真人人品暂不参与，80代表四维属性随机基数，0代表不限制该quest在线人数，后面标记是判断在线人数的
	   ...
	   if (QUEST_D->questing(me,3,80,0,"jiuyin2/jybgz")) {		
			...
			log_file("quest/jiuyinbaiguzhua",sprintf("%-18s失败多次后，从梅超风处学到九阴白骨爪真意，福：%d。\n",
