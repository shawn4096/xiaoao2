...
void create()
{
	...
	set_skill("sword", 530);
	set_skill("cuff",480); 
	set_skill("taxue-wuhen",480);
	set_skill("zhentian-quan",480);
	set_skill("lengquan-shengong",500);
	set_skill("miaojia-jianfa",530);
    ...
    set("chat_msg_combat", ({
	   ...
	   (: perform_action, "sword.baoyue" :),
       (: perform_action, "sword.ci" :),
		(: perform_action, "sword.piaojian" :),
     }));
...
int ask_miaojiajianfa() 
{
  ...
  if((me->query(QUESTDIR4+"miaojiajianfa") || QUEST_D->queryquest(me,"miaojiajianfa")) && me->query("potential")<= 5)
  {
	  command("? "+me->query("id"));
    return 1;
  }

  if(me->query(QUESTDIR4+"miaojiajianfa") || QUEST_D->queryquest(me,"miaojiajianfa") )
  {
  ...
  if(me->query(QUESTDIR4+"miaojiajianfa_fail")>=3 && me->query("registered")<3)
  {
	...
	//注意这个标记和刀法使用同一个标记。
	if ((me->query(QUESTDIR4+"time") && time()-me->query(QUESTDIR4+"time")<86400) || QUEST_D->queryquesttime(me,"miaojiajianfa"))
	{
	  command("shake "+me->query("id"));
...
int thinking(object me, object ob)
{
  ...
  if(me->query_temp(QUESTDIR4+"thinking")<(3+random(3)))
   ...
   if(me->query_temp("sj_credit/quest/public/mjjf"))
    ...
    if(QUEST_D->easy_questing(me,10,"miaojiajianfa")) { 
		
       tell_object(me,HIG"\n你明悟其中，突然间明白了一切。\n"NOR);
       message_vision(HIC"\n$N「哈哈哈」大笑几声，说道：“原来如此，原来如此。”\n"NOR,me); 
       tell_object(me,HIR"你听了苗人凤的指点，对苗家剑法的奥妙似乎有些心得。\n"NOR);
	   if(me->query(QUESTDIR4+"hujiadaofa") ||QUEST_D->queryquest(me,"hujiadaofa"))
...
int ask_hujiadaofa() 
{
  ...
  if ((me->query(QUESTDIR4+"hujiadaofa") && me->query(QUESTDIR4+"miaojiajianfa") )|| (QUEST_D->queryquest(me,"hujiadaofa") && QUEST_D->queryquest(me,"miaojiajianfa")))
  {
  	  //非vip玩家，只能失败三次
     if((me->query(QUESTDIR4+"ronghe_fail")>=3 || QUEST_D->queryquestfail(me,"daojianronghe")>=3) && me->query("registered")<3)
     {
	      command("shake "+me->query("id"));
	   ...
	   //武功学习之间需要间隔一天，经验需要间隔1M
	   //注意这个标记和刀法使用同一个标记。
	   if ((me->query(QUESTDIR4+"time") && time()-me->query(QUESTDIR4+"time")<86400)||QUEST_D->queryquesttime(me,"daojianronghe"))
	   {
	     command("shake "+me->query("id"));
	     command("say 今天先切磋到这里吧，明天吧。");
	     return 1;
     ...
     if(me->query(QUESTDIR4+"combat_exp") && me->query("combat_exp")-me->query(QUESTDIR4+"combat_exp")<500000&& !me->query("cw_exp"))
  	 ...
  	 command("nod "+me->query("id"));
  	 if(me->query(QUESTDIR4+"ronghe") || QUEST_D->queryquest(me,"daojianronghe"))
  	 {
      ...
      me->set(QUESTDIR4+"combat_exp",me->query("combat_exp"));
        ...
        if (QUEST_D->easy_questing(me,20,"daojianronghe"))
      {
...
int ask_chuangwang() 
{
  ...
  if(me->query(QUESTDIR3+"over")&& !present("baozang tu",me))
  {
    ...
    command("smile");
    tell_object(me,HIR"\n苗人凤交给你一张宝藏图。\n"NOR);
	CHANNEL_D->do_channel(this_object(), "rumor",sprintf("有人看到%s从苗人凤手中弄到了一张%s！", me->name(1), obj->name()));
    command("addoil");
