...
int check(object ob)
{
	...
	if(!objectp(ob)
	 ...
	 || strsrch(room_name, "/d/motianya") == 0
	 || strsrch(room_name, "/d/huashan/hamasuperquest") == 0
     || strsrch(room_name, "/d/emei/houshan") == 0	 
...
//        	return 1;
//        }
        if (sizeof(filter_array(children(__DIR__"obj/cart"), (: clonep :))) > 10) {
...
*/
	...
	if( totalexp < 20000000 || (maxexp-minexp) > 2000000 ){
...
//	endname = get_place(base_name(place))+place->query("short");
	...
	biaotou->set_temp("level", maxplayer->query("max_pot") - 200);
