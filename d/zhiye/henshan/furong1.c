...
int do_think(string arg)
{
        ...
        if ((int)me->query_skill("spear", 1) > 120)
                return notify_fail("你发现墙上所记载的枪法过于肤浅，自"
                                   "己无法领会到任何东西。\n");
        ...
        if (me->can_improve_skill("spear"))
                me->improve_skill("spear", add);

        me->start_busy(random(2));
        message_vision(HIY "\n$N" HIY "聚精会神的参详墙上所记载的枪法，"
                       "似有所悟。\n" NOR, me);
        write(HIC "你对「基本枪法」有了新的领悟。\n" NOR);
