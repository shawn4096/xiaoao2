...
void create()
{
	...
	set_skill("dodge",450);
	set_skill("force", 450);
	set_skill("canhe-zhi", 450);
	set_skill("murong-jianfa",450);
	set_skill("shenyuan-gong", 450);
	set_skill("yanling-shenfa", 450);
	set_skill("xingyi-zhang",450);
	set_skill("douzhuan-xingyi", 450);
	set_skill("parry", 450);
	set_skill("sword", 450);
	set_skill("literate", 300);
	set_skill("murong-daofa", 450);
	set_skill("blade",450);
	set_skill("xingyi-jianzhen",450);
...
{
	if (ob->query_skill("parry", 1) < 200) {
		command("say 我姑苏慕容对招架要求甚高。");
		command("say " + RANK_D->query_respect(ob) + "的功力不够，还是请回吧。");
		return;
	}
	if (ob->query_skill("douzhuan-xingyi", 1) < 200) {
		command("say 我姑苏慕容以彼之道、还施彼身对斗转星移的要求甚高。");
		command("say " + RANK_D->query_respect(ob) + "的功力不够，还是请回吧。");
		return;
	}
	if (ob->query_skill("shenyuan-gong", 1) < 200) {
...
string ask_fuxing()
{
	...
	//增加玩家身上没有bad标记且没有去走援救大辽的线路，则可以给次杀萧峰机会重新赋值bad。
	
	if( !ob->query(QUESTDIR3+"bad")&&!ob->query(QUESTDIR3+"good"))
	{
		command("say 如果"+ RANK_D->query_respect(ob) +"愿意帮我复国，那就把萧峰给我赶出中原！");
		ob->set_temp(QUESTDIR3+"askfuxing",1);
		return "萧峰居然阻我复国之路，每次都让我寝食难安！\n"; 		
	}
...
string ask_yuxi()
{
	...
	command("whisper " + ob->query("id")+" 我已经在燕子坞布下天罗地网，刚才探子回报，叛徒很可能在曼陀罗山庄"+room->query("short")+"出现过！");
...
string ask_ge()
{
	...
	if (me->query("mr/qinchuan"))
	{
		command("laugh "+me->query("id"));
		command("say 好，看你这么勤奋，为我大慕容偷得如此多的武技，我就让你去密室参悟一番。");
		command("say 哪儿非我慕容核心弟子不得入内,你切要珍惜。");
		me->set_temp("mr/askfu",1);
		return "";
	}
		command("angry "+me->query("id"));
		command("heng "+me->query("id"));
		command("say 你为我大慕容做了多大贡献？凭什么要将慕容绝世武技传授于你？");
		command("say 若想修得我南慕容的绝世心法和武技，还要多为我大慕容多做贡献才行.\n");
	...
	//	set("max_neili", 4500);
		...
		while ((obj->query("qi")*100 / (int)obj->query("max_qi")) > 30)
		{
			if (!present(obj, environment()))
				return 1;
			i--;
			COMBAT_D->do_attack(obj, me, obj->query_temp("weapon"), 3);
...
int ask_fuming(string arg)
{
	...
	//此处添加终极绝世武技斗转星移的 绝技判断
	/*
	if (random(me->query("job_time/慕容"))>400&&random(me->query("kar"))>25&& !me->query("mr/qinchuan"))
	{
		message_vision("\n\n$N微微一笑，对$n说道，看你如此辛苦为慕容世家效力，经过考核我正式通知你成为门派的核心亲传弟子。\n",this_object(),me);
		command("chat 从今日起，"+me->query("name")+"将成为我慕容世家的核心亲传弟子，还望江湖各位同道多加照顾!\n");
		me->set("mr/qinchuan",1);
		me->set("title","慕容世家嫡传弟子");
	}*/
...
int accept_object(object who, object ob)
{
   ...
   //接到慕容复杀萧峰的指令，然后给了人头，赋予坏人标记，完善天龙story
   if(ob->query("id") == "shouji" 
	   && ob->query("kill_by")
	   && who->query_temp(QUESTDIR3+"askfuxing")
	   && ob->query("kill_by")==who
	   && ob->query("victim_id")=="xiao feng")
   {
		command("look "+ob->query("id"));
		command("laugh ");
		command("say 好！好！好!\n");
		command("say 你替我干了一件我朝思暮想的大好事！\n");
		command("say 有了这件事，我的复国大业就有望了！\n");
		command("say 从此你就是我慕容复的挚交好友！\n");
		who->set(QUESTDIR3+"bad",1);
		return 1;
   }
//投名状的撒
   if(ob->query("id") == "shouji" 
	   ...
	   && (ob->query("victim_id")=="zhang sanfeng"
		   ...
		   ||ob->query("victim_id")=="miejue shitai"
		   ||ob->query("victim_id")=="xuanci dashi"
		   ||ob->query("victim_id")=="mu renqing"
		   ||ob->query("victim_id")=="hong qigong"
...
int ask_bishen()
{
    ...
    if ( ob->query("combat_exp") < 4500000) {

		command("shake "+ ob->query("id"));
		command("say 这位"+RANK_D->query_respect(ob)+"你的武功过于浮躁,连4.5M都不到，现在还不是告诉你的时候。");
		return 1;
	}
	if (random(ob->query("kar"))< 25 || ob->query("int") < 26 ){
	    command("look " + ob->query("id"));
		command("poor");
...
int ask_qixing()
{
	...
	time = time() - ob->query("mr/qixing/time");
	...
	if (ob->query("mrqixing/super")=="pass") {
		command("? " + ob->query("id"));
		command("say 你不是已经明白剑转七星的奥妙了吗？");
		return 1;
	}
	if (ob->query("mr/qixing/fail") >=3 && ob->query("registered") < 3){
		command("hmm");
		command("say 你受天资所限,看来此生无望了！");
		return 1;
	}
	if ( ob->query("mr/qixing/time") && time < 86400 ) {
		command("shake");
		command("say 你解迷时间未到,请稍后再来。");
		return 1;
	}
	
	if ( ob->query_skill("murong-jianfa",1) < 350) {
		command("shake "+ ob->query("id"));
		command("say 这位"+RANK_D->query_respect(ob)+"你的剑法武功过于浮躁,现在还不是告诉你的时候。");
		return 1;
	}
    //l=random(ob->query_int(1));
	l = 20;
      if(ob->query("m-card-vip")) l =  18;
      if(ob->query("y-card-vip")) l =  15;
      if(ob->query("buyvip"))     l =  10;
	i=random(l);
	if (random(ob->query("kar")) > 23 && i <5){
	
	command("look " + ob->query("id"));
	command("nod " + ob->query("id"));
	command("say 好吧,我就将剑转七星的奥妙告诉你吧。");
    command("chat* 纵声长笑道：我姑苏慕容又有一人领悟慕容剑法之剑转七星的精髓所在,何愁复国大业不成！");
    command("chat* haha");
	log_file("quest/mrqixing",
		sprintf("%-18s失败%s次后，成功从慕容复处领悟剑转七星精髓所在，福(kar)：%d，临时悟性(i)：%d,战斗经验(exp)：%d。\n",
			ob->name(1)+"("+capitalize(getuid(ob))+")",
			chinese_number(ob->query("mr/qixing/fail")), 
			ob->query("kar"), 
			i,ob->query("combat_exp")), ob
	);
	ob->set("mrqixing/super", "pass");
	ob->set("mr/qixing/exp", ob->query("combat_exp"));
	return 1;
	}
	else {
		...
		command("poor");
		command("say 看来你还是未到明白其中奥妙所在的时候!");
		ob->add("mr/qixing/fail", 1);
		ob->set("mr/qixing/time", time());
		log_file("quest/mrqixing",
			sprintf("%-18s想从慕容复处领悟剑转七星秘籍失败，失败%s次。\n",
				ob->name(1)+"("+capitalize(getuid(ob))+")",
				chinese_number(ob->query("mr/qixing/fail")) 
			), ob
		);
