...
void die()
{
	...
	if (!stringp(ob->query_temp("last_damage_from"))) //如果是特殊死亡则不进入剧情    
	{
    
	if(objectp(me) && !present("yu guangbiao", environment(ob))&& me->query(QUESTDIR1+"start") && !me->query(QUESTDIR1+"wuliangover")&& !me->query_temp(QUESTDIR1+"jianying") || me->query_temp("marks/xiaoyao/gotowyz")  )
    {
      ...
      //message_vision(HIG"吴光胜仰天长叹，为什么啊，为什么，$N竟然如此狠心。\n"NOR, me, this_object());
	  message_vision(HIG"吴光胜仰天长叹，为什么啊，为什么，$N竟然如此狠心。\n"NOR, me);
