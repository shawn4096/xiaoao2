...
{      
  ...
  if(present("duan yu",this_object())) return 0;
	...
	{
		tell_object(me,"你无法做到阴阳调和，此生与你无缘了。 \n");		
		return 0;
	}
	...
	if( me->query_temp(QUESTDIR1+"ketou_trigger")==16 ) {
	   ...
	   obj->move(me);	
       me->set_temp("getdodge/ob",obj);//添加删除标记
...
int do_yanjiu(string arg)
{
    ...
    if (me->query("gender") == "无性") 
	{
		return notify_fail("你阴阳不调，北冥此生与你无缘。\n");
	}	
	
	if (me->query("quest/天龙八部/北冥神功/pass"))
	{
		me->set("title",HIC"北冥神功传人"NOR);
		me->set("mytitle/quest/bmtitle",HIC"北冥神功传人"NOR);
...
*/
  ...
  if( random(me->query("kar"))>=28
	   && me->query("kar") <31 
       && random(me->query("pur") )>=28
	   && random(me->query("int"))>40
	   && me->query("pur") <31
	   //&& me->query("int")>100   //暂时屏蔽掉，等统一接口
	   && ( (!me->query("buyvip")&& random(6) ==1) ||( me->query("buyvip")&& random(3)==1 ) )
) {
...
int valid_leave(object me, string dir)
{
	...
	{
    if ((inv[i]->query("id") == "bo juan") || (inv[i]->query("id") == "bojuan")) 
      if (me->query_temp("getdodge/ob"))
      {		  
		  destruct(me->query_temp("getdodge/ob"));
		  me->delete_temp("getdodge/ob");
	  }
	}
