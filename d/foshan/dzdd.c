...
nosave int min_users = 4;  //By lsxk 同一ip 2个人，假设3个玩家玩就是2*3=6
...
int add_player(object me,int flag)
{
	...
	if(flag){
	    ...
	    user_list += ({ db });
		total_exp += db["exp"];
		me->delete_temp("job_dzd");
		me->set_temp("job_dzd/asked",1);
		me->apply_condition("job_busy",30);//防止去ask别的job grin
	}
	return start_time;
}
...
int clear_data(mapping db)
{
	...
	if(!db["ready"]){
		...
		if(objectp(ob = find_player(db["id"]))){
			ob->delete_temp("job_dzd");
			get_item(ob);
			tell_object(ob,"你错过了去"+game_name+"的时间！\n");
			ob->apply_condition("job_busy",5);//防止去ask别的job grin
		}
		return 0;
	}
	return 1;
...
void quit_player(object me,int flag)
{
	...
	//me->delete_temp("job_dzd"); delete this var in room
	//give msg
	//移动到紧闭室
...
void check()
{
	...
	if(stating == "ready"){
		...
		user_list = filter_array(user_list, (:clear_data:),);
		mans=sizeof(user_list);
		if(sizeof(user_list)<min_users) {
			CHANNEL_D->do_channel(this_object(),"rumor",HIY+"由于人数不足,只有"+chinese_number(mans)+"人参加，暂时取消了去大智岛的计划！");						
			job_over();
			return;
		}
		SRM->run_sea();
