...
void create()
{
        ...
        set("neili", 25000);
        set("max_neili", 25000);
        set("combat_exp", 10350000);
        set("liumai-shenjian/shaoyang",1);
		set("env/liumai","shaoyang");
        set_skill("qiantian-yiyang", 485);
        set_skill("tianlong-xiang", 465);
...
string ask_huansu()
{
	...
	if (me->query("tls")&&me->query("family/family_name")=="天龙寺") {
		command ("say 你虽然已经是天龙寺出家弟子了，但既然你凡心未泯，你就回去蓄发重修吧！");
		command("shake "+me->query("id"));
		command("buddhism ");
		me->delete("tls");
		me->set("class","officer");
		me->add("max_neili",-50-random(10));
		return "唉！又是一个凡心未洗之辈！难道他不知道出家人有除家人的好处么？\n";
	}
