...
void create()
{
	...
	set("gender", "男性");
	set("nickname",HIB"奇侠北萧峰"NOR);
	set("title",HIY"南院大王"NOR);
	set("age", 38);
	set("long", 
		"他就是丐帮前任帮主萧峰，江湖人称”北萧峰“。\n"
	...
	prepare_skill("parry","xianglong-zhang");
...
void init()
{
	...
	if (interactive(me) && living(me)) {
		...
		//遇到丁春秋，然后干架三招
		if (objectp(xding=present("ding chunqiu",environment(this_object()))))
		{
			command("heng "+xding->query("id"));
			command("perform strike.sanzhang");
		}
	} 
}
	...
	if(me->query(QUESTDIR3+"over"))
	{
		...
		{    	
			...
			if(i<2)  tell_object(me,HIR"一时间，你是劝其归隐（ask xiao about 归隐），还是为武林除害（kill xiao）？你一时委实难以决定。\n"
									+HIY"请注意，杀了萧峰就是邪线发展线路，大燕国慕容氏会对你甚是感激。若劝其归隐，是正线发展线路，大燕国将视你如寇仇！\n"NOR);
			else 
			{
...
int ask_sanzhao()
{
	...
	{
		command("say 歇口气， 别着急心境要稳当。\n");
		return 1;
	}
    if(me->query("family/family_name")!="丐帮"||me->query("family/master_name")!="洪七公")
    {
		command("heng "+me->query("id"));
		command("say 你不是丐帮弟子，来打听丐帮绝世武技，难道是北辽派遣的间隙么？\n");
	...
	if(me->query("xlz/sanzhao")=="pass")
    {
		...
		me->set("mytitle/quest/xiaofeng",HIC"南院大王"+HIY"萧峰挚友"NOR);
		return 1;
	}
    if(!me->query(QUESTDIR3+"over"))
    {
    ...
    if((time()-(int)me->query("xlz/sanzhao/time"))<86400)
    {
    	command("sigh");
    	command("say 解密时间不到。");
		return 1;
    ...
    {
    	command("sigh");
    	command("say 降龙十八掌需要极高的臂力方能施展出威力，你先天臂力太低还是赶紧参悟去吧。\n");
		return 1;
	}
	if(me->query_skill("xianglong-zhang",1)<450)
    {
    	command("sigh");
    	command("say 降龙十八掌没有大飞，赶紧修炼去吧。\n");
		return 1;
	}
	if(!me->query("quest/gb/xlz/slbw/pass")||!me->query("kanglong/pass"))
    {
    	command("sigh");
		command("say 履霜变量为"+me->query("quest/gb/xlz/slbw/pass")+",亢龙变量为"+me->query("kanglong/pass"));
    	command("say 降龙掌讲究的是动静结合，刚柔并济，你一定要学会亢龙有悔和履霜冰至，那是降龙掌的精髓。\n");
		return 1;
	}
	
	command("say 降龙十八掌是天下第一掌法，也是最为霸道的掌法。");
	command("laugh "+me->query("id"));
	command("think");
	command("say 我当时在少林寺广场前听到星宿派众人说降龙十八掌是垃圾，心中甚是生气，于是就远远喝道");
	command("say 谁说星宿派武功胜得了丐帮的降龙十八掌？");
	message_vision(HIY"$N听着萧峰娓娓道来的故事，似乎自己进入一个奇妙的场景，所有的情况宛如当时重现。\n"NOR,me);
	me->set_temp("xlz/askxiao1",1);
	call_out("move_sl",2,me);
	//this_object()->move("/d/shaolin/guangchang");	

     return 1;
} 
int move_sl()
{
   this_player()->move("/d/shaolin/guangchang");
