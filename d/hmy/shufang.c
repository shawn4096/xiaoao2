...
int do_fankan(string arg)
{
	...
	mapping book_info;
	int i;
    if (me->query("family/family_name")=="日月神教" && me->query_skill("tianmo-gong",1)>100)
    {
		obj=unew("d/hmy/obj/tianmojue2");
		 if(!clonep(obj)) {
			 	message_vision("$N随手翻动书架过程中，书架里什么也没有，好象已经被取走了。\n", me);              	 
		 }
	 else
		 {
		  obj->move(me);
		  message_vision(HIC"\n\n$N随手翻动书架过程中，忽然发现书架中有一本关于《天魔决》的书籍。\n"+
			"$N心下大喜，急忙将之揣入怀中。\n"NOR,me);	
	}
