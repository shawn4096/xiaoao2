...
int do_bo(string arg)
{
	...
	{
		return notify_fail("你正在忙碌!\n");

	}

	if(arg != "白骨" && arg != "baigu")
	{
		return notify_fail("你要剥什么东西？!\n");
	...
	if (random(10) > 4)
	{
		...
		return notify_fail("你剥了半天白骨，一无所获!\n");
	}
	...
	else
	{
		...
		she->set("combat_exp", me->query("combat_exp")+random(1000000));
		she->set("max_qi",me->query("max_qi"));
		she->set("max_jing",me->query("max_jing"));
