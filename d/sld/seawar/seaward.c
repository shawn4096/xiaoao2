...
string play(object me1,object me2,int team)
{
	...
	sea[i]["angle"] = -1;
	sea[i]["fire"] = 0;
	sea[i]["killer1"] = 3; //ÐÞ¸Äµ½5×éÊÔÊÔÐ§¹û,Ô­À´ÊÇ3
	sea[i]["killer2"] = 3;
...
void flee(object me,int idx,int team)
{
   ...
   // exp=exp;
	exp=exp/2;//ÖØÐÂµ÷Õû£¬½µµÍËÙ¶ÈºÍÐ§ÂÊ
	exp=exp*3/4;
	pot=150+random(50);//ÖØÐÂµ÷Õûpost½±Àø
	if(objectp(me1)) {
		//if(team==1) exp = me1->add_exp_combat(exp,"Ê©ÀÅ","´óÇå¹úº£Õ½");
		//else exp = me1->add_exp_combat(exp,"Â½¸ßÐù","ÉñÁúµºº£Õ½");
		if(me1->query("buyvip") )
            exp = exp + exp * 20/ 100;
		else if(me1->query("y-card-vip"))
            exp = exp + exp * 10 / 100;
		else if(me1->query("m-card-vip"))
            exp = exp + exp * 5 / 100;
		
		if (me1->query("combat_exp")>25000000&&me1->query("combat_exp")<35000000)
			exp=exp/2;
		if (me1->query("combat_exp")>35000000&&me1->query("combat_exp")<75000000)
			exp=exp/10;

	//	if (me1->query("combat_exp")>75000000)
		//	exp=0;
		
		me1->add("job_time/ÉñÁúµºº£Õ½",1);
		write(HIY"Ôö¼ÓÉñÁúµºº£Õ½À²£¡"+me1->query("job_time/ÉñÁúµºº£Õ½")+NOR);
		if(team==1) me1->add("combat_exp",exp);
		else {
		//Èç¹û¸ºÉñ»á»ñµÃ¶¾Áú´ó·¨»ú»á
			
			me1->add("combat_exp",exp);
			jobs = me1->query("job_time/ÉñÁúµºº£Õ½");
			j = 25;
			if(me1->query("m-card-vip")) j =  20;
			if(me1->query("y-card-vip")) j =  17;
			if(me1->query("buyvip"))     j =  15;
			
			if (me1->query("title")=="ÉñÁú½Ì°×ÁúÊ¹"
				||me->query("family/zhicheng")=="°×ÁúÊ¹")
				j=j-2;
			
			i=random(me1->query("kar"));
			if (random(me1->query("job_time/ÉñÁúµºº£Õ½")) >8000
			&& random(j)<3
			&& i > 26
			&& random(me1->query("combat_exp"))>4000000
			&& !me1->query("quest/sld/dldf/start")
		//	&& !me1->query("quest/sld/dldf/pass")
			&& me1->query("family/family_name")=="ÉñÁú½Ì"
			&& jobs % 50 == 0 ) 
			{
				me1->set("quest/sld/dldf/start", 1);
				log_file("quest/slddldf",sprintf("%8s%-10sµÚ%4d´ÎÉñÁúµºº£Õ½ÈÎÎñÊ±£¬»ñµÃÈÎÎñÌáÊ¾£¬ÉñÁúµºº£Õ½£º%4d£¬£º¸££º%2d£¬Ëæ»úÊý£º%d¡£\n",
				me1->name(1),"("+capitalize(me1->query("id"))+")",me1->query("job_time/ÉñÁúµºº£Õ½"), i, j),me1);
				command("say "+me1->query("name")+"ÄãÉ±ÁË²»ÉÙ³¯Í¢¹·Ôô£¬ÏÖÔÚ¿ÉÒÔÈ¥½ÌÖ÷ÄÄ¶ùÇë½ÌÉñÁú½ÌµÄ¸ßÉîÎä¼¼°É¡£\n");
			}
		}
		...
		//me1->apply_condition("job_busy",1);
		
		if( me1->query("relife/exp_ext") ){ 
			ext = me1->query("relife/exp_ext");
			me1->add("combat_exp", exp *ext /20 );
			tell_object(me1,HBGRN"Äã×ÐÏ¸»ØÏë×Åº£Õ½µÄ¹ý³Ì£¬ÓÖ¶îÍâÔö¼ÓÁË"+chinese_number(exp*ext/20)+"µãÊµÕ½¾­Ñé£¡\n"NOR);
		  }
		if( me1->query_condition("db_exp") ) {   
			me1->add("combat_exp", exp);
            tell_object(me1,HBGRN"Ë«±¶¾­Ñé½±ÀøÆÚ¼ä£¬Äã¶îÍâµØÔö¼ÓÁË"+chinese_number(exp)+"µãÊµÕ½¾­Ñé£¡\n"NOR);
		}
	...
	if(objectp(me2)) {
		...
		if(me2->query("buyvip") )
            exp = exp + exp * 20/ 100;
		else if(me2->query("y-card-vip"))
            exp = exp + exp * 10 / 100;
		else if(me2->query("m-card-vip"))
            exp = exp + exp * 5 / 100;
		
		if (me2->query("combat_exp")>25000000&&me2->query("combat_exp")<35000000)
			exp=exp/2;
		if (me2->query("combat_exp")>35000000&&me2->query("combat_exp")<75000000)
			exp=exp/10;

		//if (me->query("combat_exp")>75000000)
			//exp=0;	
		me2->add("job_time/ÉñÁúµºº£Õ½",1);
		write(HIC"Ôö¼ÓÉñÁúµºº£Õ½À²£¡"+me2->query("job_time/ÉñÁúµºº£Õ½")+NOR);

		if(team==1) me2->add("combat_exp",exp);
		else {
			
			me2->add("combat_exp",exp);
			jobs = me2->query("job_time/ÉñÁúµºº£Õ½");
			j = 25;
			if(me2->query("m-card-vip")) j =  20;
			if(me2->query("y-card-vip")) j =  17;
			if(me2->query("buyvip"))     j =  15;
			if (me1->query("title")=="ÉñÁú½Ì°×ÁúÊ¹"
				||me->query("family/zhicheng")=="°×ÁúÊ¹")
				j=j-2;
			
			i=random(me2->query("kar"));
			
			if (random(me2->query("job_time/ÉñÁúµºº£Õ½")) >8000
			&& random(j)<3
			&& i>26
			&& random(me2->query("combat_exp"))>4000000
			&& !me2->query("quest/sld/dldf/start")
			//&& !me2->query("quest/sld/dldf/pass")
			&& me2->query("family/family_name")=="ÉñÁú½Ì"
			&& jobs % 50 == 0 ) 
			{
				me2->set("quest/sld/dldf/start", 1);
				log_file("quest/slddldf",sprintf("%8s%-10sµÚ%4d´ÎÉñÁúµºº£Õ½ÈÎÎñÊ±£¬»ñµÃÈÎÎñÌáÊ¾£¬ÉñÁúµºº£Õ½£º%4d£¬£º¸££º%2d£¬Ëæ»úÊý£º%d¡£\n",
				me2->name(1),"("+capitalize(me2->query("id"))+")",me2->query("job_time/ÉñÁúµºº£Õ½"), i, j),me2);
				command("say "+me2->query("name")+"ÄãÉ±ÁË²»ÉÙ³¯Í¢¹·Ôô£¬ÏÖÔÚ¿ÉÒÔÈ¥½ÌÖ÷ÄÄ¶ùÇë½ÌÉñÁú½ÌµÄ¸ßÉîÎä¼¼°É¡£\n");
			}
		}
		...
		//²ÎÎòÊý
		me2->set("job_name","ÉñÁúµºº£Õ½");
		//me2->apply_condition("job_busy",1);
		if( me2->query("relife/exp_ext") ){ 
			ext = me2->query("relife/exp_ext");
			me2->add("combat_exp", exp * ext /20 );
			tell_object(me2,HBGRN"Äã×ÐÏ¸»ØÏë×Åº£Õ½µÄ¹ý³Ì£¬ÓÖ¶îÍâÔö¼ÓÁË"+chinese_number(exp*ext/20)+"µãÊµÕ½¾­Ñé£¡\n"NOR);
		 }
		//¾«Ó¢Ö®Êé
		if( me2->query_condition("db_exp") ) {   
			me2->add("combat_exp", exp);
            tell_object(me2,HBGRN"Ë«±¶¾­Ñé½±ÀøÆÚ¼ä£¬Äã¶îÍâµØÔö¼ÓÁË"+chinese_number(exp)+"µãÊµÕ½¾­Ñé£¡\n"NOR);
		}
	}
