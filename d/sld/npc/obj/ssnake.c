...
string gongji()
{
	...
	if((int)query("huti")) return "金蛇已经在你周身游走，为你防护！\n";
	set("gongji",1);
	remove_call_out("check_fight");
	call_out("check_fight",1,owner);
	return HIY"\n你拍拍金蛇的头，指指敌人，金蛇恶狠狠地盯着所指方向看去!\n"NOR;
}
...
int snake_attack(object owner,object target)
{
	...
	message_vision(HIY"\n$N的金蛇突然窜向$n，狠狠地咬了下去...  \n"NOR,owner,target);
	if( target->is_busy() 
		|| random(owner->query_dex()*6/5+target->query_dex())>target->query_dex() ) 
	{
		message_vision(HIR"\n$N躲闪不及，被金蛇一口咬中！\n"NOR,target);
		//被金蛇咬住后攻击力下降
		...
		if(damage>2000) damage = 2000 + (damage-2000)*2;
		target->receive_damage("qi", damage, owner);
		target->receive_wound("qi", random(damage), owner);
		COMBAT_D->do_attack(owner, target, 0, 3);
		target->add_condition("snake_poison",1);
...
void check_fight(object owner)
{
	...
	if( (int)query("gongji") && random(10)<5 ) {
