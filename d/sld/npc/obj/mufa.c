	...
	if(arg==("mufa")) {
		...
		if(mf->query_temp("curstatus",1)=="run") {
			...
			if(mf->query_temp("curstatus",1)=="run") {
				...
				if(mf->query_temp("curstatus",1)=="run") {
				    if(!objectp(mf=find_object(__DIR__"mu_fa4")))
					    mf=load_object(__DIR__"mu_fa4");
				     if(mf->query_temp("curstatus",1)=="run") {
					    if(!objectp(mf=find_object(__DIR__"mu_fa5")))
					        mf=load_object(__DIR__"mu_fa5");
				        if(mf->query_temp("curstatus",1)=="run") {
							if(!objectp(mf=find_object(__DIR__"mu_fa6")))
					            mf=load_object(__DIR__"mu_fa6");
				            if(mf->query_temp("curstatus",1)=="run"){
								if(!objectp(mf=find_object(__DIR__"mu_fa7")))
									mf=load_object(__DIR__"mu_fa5");
								if(mf->query_temp("curstatus",1)=="run")
