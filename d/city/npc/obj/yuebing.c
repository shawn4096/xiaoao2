...
void create()
{
	...
	string *first_name=({HIR+"五仁",YEL+"鲜肉",HIY+"豆沙"});
	string *first_id=({"wuren","xianrou","dousha"});	
	string third_name="月饼"+NOR;
	string third_id="cake";
   ...
   int i, max, lvl;
	...
	//低等级玩家送不可交易的内容，高等级玩家送玉
		...
		if( me->query("combat_exp") > 5000000 ) {
			...
			obj->set_level(2+random(3),random(2)?1:2);
			obj->move(me);			
			obj->set("date_item",1);
			CHANNEL_D->do_channel( this_object(), "rumor",sprintf("有人看到%s从月饼里吃到了一块%s！", me->name(1), obj->query("name")));
