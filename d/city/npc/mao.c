...
string do_lingwu_dmd()
{
       ...
       if ((time()-me->query("quest/whdmd/menghu/time"))<86400)
        {
			return "我养伤期间不能太频繁回答你，过一阵在再来！\n";
        }
		
		if (random(me->query("kar"))<26
			||random(me->query_int(1))<30)
        {
			me->add("quest/whdmd/menghu/fail",1);
			me->set("quest/whdmd/menghu/time",time());

			return "我在养伤期间，心情不好，隔一阵再来！\n";
        } 
