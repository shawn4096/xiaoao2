...
int do_fan(string arg)
{
  ...
  //难度系数加大,毕竟是sj第一dodge
  if(random(me->query("int")) >= 40

	  && (random(me->query("dex"))+random(me->query("int"))+random(me->query("str"))+random(me->query("con")) ) >= 100
	  && random(me->query("kar")) <31
	  && random(me->query("kar"))>=28
	  //&& ((!me->query("buyvip")&&random(10)==1)||(me->query("buyvip")&&random(8)==1))
	  && j<3)
  {
	  ...
	  me->set("title",HIC"凌波"HIW"微步"HIG"传人");
	  me->set("mytitle/quest",HIC"凌波"HIW"微步"HIG"传人");
	  me->delete_temp("find");
	  me->start_busy(2);
