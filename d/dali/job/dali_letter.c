...
int do_send(string arg)
{
	...
	me->add_busy(1+random(2)); // hehe, 防止利用bug，简单有效 :) snowman@SJ
   ...
   if (!me->query_temp("zuji"))
       TASKREWARD_D->get_reward(me,"送信",0,0,0,-3,0,0,0,t_name); //-3改为1
   else if (me->query_temp("songxin/twice"))
       TASKREWARD_D->get_reward(me,"送信",0,0,0,12,0,0,0,t_name); //5改为7，改为12
   else
       TASKREWARD_D->get_reward(me,"送信",0,0,0,5,0,0,0,t_name); //1改为5
	if (me->query_temp("zuji"))
		me->clear_condition("job_busy");
	//else me->apply_condition("job_busy",9);
	else me->apply_condition("job_busy",30);
	//狙击的直接清除job_busy,非狙击设置9*15秒job_busy
