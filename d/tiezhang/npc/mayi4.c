...
void create()
{
	...
	set_skill("tiezhang-xinfa",180);
	set_skill("axe",300);
	set_skill("lietian-fu",300);
...
void attempt_apprentice(object ob)
{
	...
	if ((int)ob->query("shen") >= -1000000) {
		command("hehe");
		command("say 铁掌帮可不收心慈手软的人做弟子，-1m。\n");
		return;
	}
	...
	if ((int)ob->query_skill("guiyuan-tunafa", 1) <= 200) {
		command("say 要学更为高深的武功，必须有高深的内功修为。");
		command("say " + RANK_D->query_respect(ob)+"是否还应该在归元吐纳法上多下点功夫(200)？");
		return;
	}
