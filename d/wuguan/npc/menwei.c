...
int sort_file(string file)
{
		...
		if( sscanf(file, "bagua%*s.c") )  return 0;
        i = strlen(file);
        while(i--){
                if( file[i] == '.' ) continue;
...
string ask_lchj()
{
	 ...
	 object book;
	 if (me->query(QUESTDIR2+"over") && !me->query("givelchj"))
	 {
		    me->set("givelchj",1);
		    book=new("/d/wuguan/obj/tangshixuanji");			
			book->set("owner",me->query("id"));
			book->move(me);
			return "这次可要收好了，莫要弄丢了。";
	 }
	 else return "哼哼......";
...
string ask_yuebing()
{
		...
		*/
        ...
        if(me->query("registered") < 3)
			...
			i = 15-me->query("quest/gift/mid-autumn/yuebing");
...
string ask_road()
{
        ...
        if( me->query( "wg_help_exp" ) && me->query( "wg_help_exp" ) - me->query( "combat_exp" ) > - 2000 )
                return RANK_D->query_respect( me ) + "为人应当自立自强，怎么可以老是找人帮忙呢？";
         ...
         || target->query("family/family_name") == me->query("family/family_name") ) {
			   ...
			   if (me->query("family/family_name"))
			   {
                switch(me->query("family/family_name")){
					...
					case "日月神教": dir = "/d/hmy/"; break;
                	case "灵鹫宫": dir = "/d/tianshan/"; break;
					case "全真教": dir = "/d/quanzhen/"; break;
					case "血刀门": dir = "/d/xuedao/"; break;
                ...
                "一带惹事挑衅。");
                CHANNEL_D->do_channel(me, "party", HIY"该恶贼自负武功了得，作恶多端，我"HIC + me->query("family/family_name") + HIY"弟子如若遇见，切不可心慈手软。"NOR);
			   }
        }
}
