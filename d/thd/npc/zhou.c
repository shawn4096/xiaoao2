...
/*
	...
	if ( ob->query("int") > 25 && !ob->query("relife/gifts/total")) {
		command("shake");
	...
	if (random(ob->query("kar")) < 26 || random(ob->query("pur")) < 27){
		command("shake");
...
int ask_jiuyin()
{
    ...
    if (ob->query("quest/jiuyin1/pass")){
		...
		book->move(ob);
        ob->set("title",HIR"武林神话"NOR"·"HIC"九阴真经传人"NOR); 
        ob->set("mytitle/quest/jiuyintitle",HIR"武林神话"NOR"·"HIC"九阴真经传人"NOR);		
...
int ask_hubo()
{
	...
	if (ob->query_skill("force") < 100 || ob->query("max_neili") < 500 || ob->query("combat_exp") < 200000) {
...
int do_fangyuan()
{
	...
	if (hubo == 22) {
          if( (ob->query_temp("sj_credit/quest/public/hubo")&&random(ob->query("pur"))>18) 
                || random(ob->query("pur")) > 27 
                || ( ob->query("buyvip")&& random(ob->query("pur"))>=27 ) 
                || ( ob->query_temp("sj_credit/quest/public/hubo") &&ob->query("buyvip") && random(ob->query("pur"))>20) ){
