...
void create()
{
	...
	set("thd/wuji/luoying", 1);
	set("thd/wuji/kuangfeng", 1);
	set("thd/wuji/lhs", 1);
	set("thd/wuji/huayu", 1);
	set("thd/wuji/lingxi", 1);
...
int ask_dongxiao()
{
	...
	if (!me->query("quest/thd/dichuan"))
	{
		command("say 你虽然是我门中弟子，先别问我索取东西，你为桃花岛做了什么先？\n");
		return 1;
	}
...
void attempt_apprentice(object ob)
{
	...
	if (ob->query_skill("literate",1)<200) {
		command("say 你这读书写字水平，连200级都不到，也想来学功夫？先去学点文化吧。");
		return;
	}
	...
	if (ob->query("family/family_name") == "桃花岛") {
		...
		if ((int)ob->query_skill("bihai-chaosheng", 1) < 350)
		{
			command("say 我桃花岛一派武功和碧海潮生关系很大，不足350级，别来找我。");
			command("say " + RANK_D->query_respect(ob) +
				"是否还应该在碧海潮生功上多下点功夫？");
			return;
		}
		
		if ( (int)ob->query("job_time/桃花岛") <= 1000 )
		{
			command("angry " + ob->query("id"));
			command("say 守墓连1000次都不到，你为桃花岛作了什么贡献了，就想和我学功夫？");
			return;
		}
...
int ask_qimendunjia()
{
	...
	if (!(fam = ob->query("family"))
		|| fam["family_name"] != "桃花岛")
		return notify_fail(RANK_D->query_respect(ob) + "不是本派弟子，在这瞎打听什么！\n");
	if (fam["master_name"] != "黄药师")
		return notify_fail(RANK_D->query_respect(ob) + "不是我的亲传弟子，在这瞎打听什么！\n");
	if (ob->query_skill("qimen-bagua",1)<150)
		return notify_fail(RANK_D->query_respect(ob) + "你的奇门八卦不足150，给了你也领悟不了！\n");
	if (ob->query_skill("qimen-bagua",1)<150)
		return notify_fail(RANK_D->query_respect(ob) + "你的奇门八卦不足150，给了你也领悟不了！\n");
    if (!ob->query("thd/finish"))
 		return notify_fail(RANK_D->query_respect(ob) + "我一生最大的遗憾就是九阴真经被盗，哼，谁要是替我找到我就好好教他！\n");

	command( "say 我曾亲手写过一本讲解我桃花岛精义《奇门遁甲》的奇书，或许能对" + RANK_D->query_respect(ob) + "有所帮助。\n");
	command( "say 我把它放在书房了，你自己去书架（shujia）找找看（search）吧。\n");
	ob->set_temp("qmbg/qimendunjia",1);
	return 1;
}
int ask_quest()
{
...
int ask_biguan()
{
	...
	{
		command("say 你的碧海潮生太低了,不足500级！\n");
		return 1;
	}
	if (me->query_skill("qimen-bagua",1)<200)
	{
		command("say 你的奇门八卦太低了，不足200级！\n");
		return 1;
	}
	if (me->query_skill("flute",1)<200)
	{
		command("say 你的萧技太低了，不足200级！\n");
		return 1;
	}
	...
	if (me->query("int")<50)
	{
		command("say 你的先天悟性不足50，去了也白搭?！\n");
		return 1;

	}
	if (me->query("con")<50)
	{
		command("say 你的先天根骨不足50，去了也白搭?！\n");
		return 1;

	}
	me->set_temp("quest/thd/biguan/askhuang",1);
	command("say 好吧，既然你如此优秀，我就把我当初领悟的地点告诉你!\n");
	command("say 那地方原本是桃花岛最高峰，后来我顿悟后，起名叫“试剑峰”！\n");
	command("say 你去哪儿坐(sit)下来好好闭关(biguan)修行!\n");
	command("say 能否领悟还看你自己的造化了!\n");
	command("consider");
	return 1;
}