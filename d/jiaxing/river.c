...
//重新调整为新模式，让更多的人解开,
	if(random(total) >= 87  
	     ...
	     {	      
	    	message_vision(HIR"\n$N只觉手臂麻木，早已不听使唤，只急得大汗淋漓，不知如何是好，慌乱中跑进了柳树林子。\n\n"NOR,me);  
	     	...
	     	log_file("quest/hamagong", sprintf("%8s%-10s失败%d次后，成功进入柳树林，福：%d，容：%d，根：%d。\n", 
	...
	//老代码
/*	  if(random(total) >= 84  //改动 87->85->84
		  && me->query("kar")<31
		  && me->query("per")<31
		  && j<=300  //3改动成300，蛤蟆1不限在线
		  && ( random(7)==1||(me->query("buyvip")&& random(4)==1)) ) 
	     {
	       ...
	       if(random(total) >=80  //改动，原来85
            ...
            me->move(__DIR__"river1");
	    	    ...
	    	    log_file("quest/hamagong", sprintf("%8s%-10s第%d次失败未进入柳树林，福：%d，容：%d，根：%d。\n",
			    me->name(1), "("+me->query("id")+")", me->query("quest/hama/fail"), me->query("kar"), me->query("per"), me->query("con")), me);
              	message_vision(HIR"\n$N脚下突然一软，骨碌碌地滚出了数十丈！\n\n"NOR,me);
