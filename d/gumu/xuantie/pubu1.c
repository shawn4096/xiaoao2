...
int do_hit(string arg)
{
	...
	if (me->query_skill("xuantie-jianfa",1) <= 350
	 && weapon->query("id") != "xuantie jian")
		return notify_fail("凭你现在的玄铁剑法等级不足350，不用玄铁剑怎能领悟呢？\n");

	if (me->query_skill("xuantie-jianfa", 1) > 350 
	 && me->query_skill("xuantie-jianfa", 1) <= 450
	 && weapon->query("id") != "chang jian")
		return notify_fail("你想进一步提高玄铁剑法的修为还没大飞，要换一把剑了。\n");

	if (me->query_skill("xuantie-jianfa", 1) > 450 
