...
void create()
{
	...
	set("questbusy",0);//防止干扰
        ...
        map_skill("parry", "jindao-heijian");
...
int thinking(object me, object ob)
{
	...
	else
	{
		...
		else
		{
		  ...
		  log_file("quest/jindaoheijian", sprintf("%s(%s) 解金刀黑剑阴阳双刃失败：%d次。富源：%d；经验：%d。\n", me->name(1),me->query("id"), me->query(QUESTDIR+"fail"),me->query("kar"),me->query("combat_exp")) );
		}
