...
int do_look(string arg)
{
	...
	if ( arg =="zi") {
		...
		{
			me->set("title", HIM"古墓九阴真经传人"NOR);
			write(HIR"你一瞥间，看到几个小字“九阴真经内功要诀”。\n"NOR);
		...
		if (me->query("con")<40)
		{
			write("你的先天根骨不足，理解不了这内功运行之法的奥秘！\n");
			return 1;
		}
		if( !me->query_temp("ceiling")
                 || me->query("combat_exp", 1) < 12000000 )
		{
...
{ 
        ...
        if ( arg =="down"){ 
             if(!me->query_temp("map") || me->query_skill("force",1)<100 )
