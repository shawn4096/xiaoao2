...
刻着些古怪的图形(ground), 顶上有一些美女图形(top)，北面墙上刻录几行
简短的修炼口诀，字迹(zi)娟秀，一看就是女人所为。
LONG
	);
...
int do_yanxi(string arg)
{
	...
	if ( arg =="zi" ){
		if ((int)me->query_skill("suxin-jue", 1) > 100 )
			return notify_fail("你面对墙壁所描绘经络运转心法，只觉所述早已尽藏心胸。\n");
		if ((int)me->query("jing") < 20)
			return notify_fail("你一阵晕旋,想是用脑过度,该休息一会了. \n");
		write("你按照墙上所绘字迹来练习古墓武学的心法口诀，颇有心得。\n");
		me->receive_damage("jing", 5 + random(15));
		me->improve_skill("suxin-jue", (int)me->query_int()/2);
		return 1;
	}
	if ( arg =="top" ){
