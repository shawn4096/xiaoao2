   ...
   if (!arg||arg!="book")
		...
		&& random(me->query("con"))>=40)
	{
      ...
      // CHANNEL_D->do_channel(this_object(), "rumor",me->name(1) + "机缘巧合之下,得到一张" + HIW + "人皮面具" + NOR + "。\n"); 
	   ...
	   message("channel:rumor", HIM"【谣言】某人："+me->query("name")+"机缘巧合，获得少林易筋经的真正传承！\n"NOR, users());
	   ...
	   me->set("title",HBYEL+RED"易筋经神功传人"NOR);
...
int do_toukan(string arg)
{
   ...
   if (!me->query_temp("quest/mr/canhe/askbo"))
        return notify_fail("你没事跑到藏经楼偷看什么东西？\n不怕被和尚们打死！\n");
   if (me->is_busy()||me->is_fighting())
     return notify_fail("你正在忙碌,到处乱摸什么东西？");
   if (!arg||arg!="指法秘籍")
     return notify_fail("你要看什么东西？");
    if (me->query("quest/mr/canhe/sanjue/pass"))
      return notify_fail("你已经将参合三绝指融会贯通，还在这里干什么？");
    if (time()-me->query("quest/mr/canhe/sanjue/time")<86400)
      return notify_fail("你太勤快了耶？");
    
	message_vision(HIC"\n\n$N蹑手蹑脚行走在藏经楼二楼，生怕被人发现，然后开始仔细查找经楼中指法武功秘籍。\n"
	                 +"\n$N心想：“师傅说让我偷看指法秘籍，我先查看是否有金刚指、拈花指、多罗叶指的秘籍。”，\n"
					 +"静心下来，仔细查找，很快就在书架中找到这三本秘籍，心中大喜，当下就翻阅起来。\n"NOR,me);
	i=20;
	...
	//富源，随机参悟>=15点，且根骨>=45,降低要求
	
	if (k>27
		&&j<3)
	{
      
	   message_vision(HIG"\n\n$N看完三本书，忽然间灵光一闪，结合慕容博所提及的这三招的技巧，顿时明白\n"
	   +"过来，原来参合指居然可以施展出金刚指、拈花指和多罗叶指的招式来。\n"NOR,me);
	   message_vision(HIY"\n\n恭喜你，因机缘巧合，历尽千辛万苦，你终于修得参合指三绝指绝技》。\n"NOR,me);
	   
	 
	   message("channel:rumor", HIM"【谣言】某人："+me->query("name")+"机缘巧合，在少林藏经楼参悟指法获得顿悟！\n"NOR, users());

	   log_file("quest/mrsanjue",sprintf("%s(%s) 机缘巧合得到参合指中三绝指绝技。富源：%d。随机数：%d。失败：%d。经验值：%d。\n",
	...
	{
		message_vision(HIY"$N虽然得知三绝指招式，但因机缘不到，不能得到彻底融会贯通。\n"NOR,me);
		log_file("quest/sanjue",sprintf("%8s%-10s解密三绝指失败，尚须努力。富源：%d。随机数：%d。失败：%d。经验值：%d。\n",
			me->name(1), me->query("id"),k,j,me->query("quest/mr/canhe/sanjue/fail"),me->query("combat_exp"),me));
