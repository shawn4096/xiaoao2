...
int accept_object(object who, object ob)
{
    ...
    object me = this_player();
        ...
        if(ob->query("id") == "xin jian" && ob->query("name") == "家信"&& random(5)==4){
...
string ask_where()
{
       ...
       object ob, where;
       object me = this_player();
       ob = find_living("xia xueyi");
       
	   if (!ob) return "你问外子啊，我也不知道他去哪里了。\n";
       where = environment(ob);
       if (!where) return "夫君好像消失了耶...\n";  
       
	   if (objectp(present("xia xueyi", environment(me)))){
          	command("disapp "+me->query("id"));
          	return "外子就在这里呀...\n";   
       }
       command("look "+me->query("id"));
       
	   return "这位" + RANK_D->query_respect(me) + "，不瞒您说，我那夫君正在" + get_place(base_name(where)) + (string)where->query("short") +"闲逛呢。\n";
