...
void create()
{
	...
	set_name("侍食喇嘛",({"shishi lama", "lama", "shishi"}));
       	set("long","他就是专门服侍门中师傅的小喇嘛。长得眉清目秀。\n");
	set("gender", "男性");
	set("age", 16);
...
string ask_jiu()
{
    ...
    if ((me->query("food")*10/me->max_water_capacity())>8)
		return "你怎么吃饱了还想要？";
	if ( present("qingke jiudai", this_player()))
		return "先吃完了我给你的再说吧。";
	if ( present("qingke jiudai",  environment(me)) )
		return "那不是有吗？先吃完了再说。";
	ob=new("d/xuedao/obj/qingkejiudai");
    ob->move(environment(this_object()));
	return "这袋青稞酒就给你吧，请慢点喝哟。";
}
