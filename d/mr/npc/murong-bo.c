...
int ask_canhezhi();
int ask_sanjue();
string *kill_name=({"左冷禅","张无忌","枯荣长老","玄慈大师","灭绝师太","洪七公",});
void create()
{
        object weapon,armor;
...
int ask_douzhuan()
{
	...
	if (ob->query("quest/mr/dzxy/pass")) {
		...
		ob->set("mytitle/quest/dzxytitle",HIC"斗"HIG"转"HIB"星"HIY"移"NOR);
	...
	if (!ob->query("quest/mr/dzxy/giveguan"))
	{
		command("say 你若有心就替我把慕容世家的皇冠给我找来，否则别来烦我。\n");
		return 1;
	}
	if (!ob->query("quest/mr/dzxy/givebiao"))
	{
...
int ask_canhezhi()
{
	...
	if (ob->query("quest/mr/canhezhi/pass")) {
		command("? " + ob->query("id"));
		ob->set("title",HIG"南慕容 参合指传人"NOR);
		ob->set("mytitle/quest/chztitle",HIG"南慕容 参合指传人"NOR);
	...
	if (ob->query("quest/mr/canhezhi/fail") >=3 && ob->query("registered") < 3){
		command("hmm");
		command("say 你受天资所限,看来此生无望了！");
		return 1;
	}
	if (time()- ob->query("quest/mr/canhezhi/time") < 86400 ) {
		command("shake");
	...
	if (!ob->query("quest/mr/canhe/give_shouji"))
	{
	
	  if (ob->query_temp("quest/mr/canhe/give_shouji")<6)
	  {
		command("nod ");
		...
		command("whisper " + ob->query("id")+" 其中少林、武当人称武林泰山北斗，卧龙藏虎之辈层出不穷，无名神僧人称不死老怪物。");
		...
		command("whisper " + ob->query("id")+" "+ RANK_D->query_respect(ob) +"既然立志复兴天下，你就替我先去杀几个人，以明心志！");
		...
		if (k_name=="张无忌")
		{
			if (!ob->query_temp("quest/mr/canhe/kwuji"))
			{
				...
				ob->set_temp("quest/mr/canhe/wuji",1);
			}else command("say 方才不是已经杀了张无忌了么？再继续问！看我心情再给你分配别的任务。");
		
		}else if (k_name=="左冷禅")
		{
			if (!ob->query_temp("quest/mr/canhe/klengchan"))
			{
				command("say 既然如此，嵩山最近名声鹊起，将影响我大燕复兴，你现在替我去将左冷禅这人的人头拿来。\n");
				ob->set_temp("quest/mr/canhe/lengchan",1);
			}else command("say 方才不是已经杀了左冷禅了么？再继续问！看我心情再给你分配别的任务。");
		...
		}else if (k_name=="灭绝师太")
		{
			if (!ob->query_temp("quest/mr/canhe/kmiejue"))
			{
				...
				ob->set_temp("quest/mr/canhe/miejue",1);
		...
		}else if (k_name=="枯荣长老")
		{
			if (!ob->query_temp("quest/mr/canhe/kkurong"))
			{
				...
				ob->set_temp("quest/mr/canhe/kurong",1);
			}else command("say 方才不是已经杀了枯荣长老了么？再继续问！看我心情再给你分配别的任务。");
		}else if (k_name=="玄慈大师")
		{
			if (!ob->query_temp("quest/mr/canhe/kxuanci"))
			{
				command("say 既然如此，少林派的玄慈最近很不安分，将影响我大燕复兴，你现在替我去将玄慈这老秃驴的人头拿来。\n");
				ob->set_temp("quest/mr/canhe/xuanci",1);
			}else command("say 方才不是已经杀了玄慈了么？再继续问！看我心情再给你分配别的任务。");
		}else if (k_name=="洪七公")
		{
			if (!ob->query_temp("quest/mr/canhe/khong"))
			{
				command("say 既然如此，丐帮好生兴旺，将影响我大燕复兴，你现在替我去将洪七公这个老叫化的人头拿来。\n");
				ob->set_temp("quest/mr/canhe/hong",1);
   ...
   {  
	  ...
	  if (random(me->query("kar"))>28 
			...
			&& i==3)
		{
			...
			me->set("title",HIG"南慕容 参合指传人"NOR);
   ...
   {  	 
	  ...
	  if (random(me->query("kar"))>28 
			...
			&& i<3)
		{
			...
			me->set("mytitle/quest/dzxytitle",HIC"斗"HIG"转"HIB"星"HIY"移"NOR);
			me->start_busy(1);
...
int ask_sanjue()
{
	...
	{
		command("say 你的参合指功力太低，不足500。\n");
		return 1;

	}
	if (me->query_temp("quest/mr/canhe/askbo"))
	{
		command("say 你方才不是问了么，我已经告诉你，赶紧去吧！\n");
		return 1;
	}
	command("say 这三绝指乃是我大慕容参合指的精华招式，但其一切渊源都是源自天下各门各派的指法！\n");
	command("say 其中最主要的就是少林的指法！\n");
	command("say 三绝指也是源自少林的大力金刚指力、拈花指、和多罗叶指的精华！\n");
	command("say 你若想学这门招式，你就去少林寺藏经阁一趟，去多偷看（toukan）少林“指法秘籍”，看能否有所领悟！\n");
	me->set_temp("quest/mr/canhe/askbo",1);
	return 1;
