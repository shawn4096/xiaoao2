...
int do_canwu(string arg)
 {
     ...
     //if ( me->query("relife/exp_ext",1) >=40 )
     if ( me->query("relife/exp_ext",1) >=20 )
	 {
                  if(me->query("registered") == 3)    exp=1800000;
                  else if(me->query("y-card-vip"))    exp=1700000;
                  else if(me->query("buyvip") )       exp=1600000; 
				  else exp=2000000;
				  i=me->query("relife/exp_ext",1)-20;
				  if (i>0)
				  {
					  me->add("combat_exp",i*exp);
					  message_vision(HIC"因为你已经多参悟了"+i+"个经验值参悟点数，现已经将其返回。共返还exp="+i*exp+"。如有问题请于巫师联系\n"NOR,me);
