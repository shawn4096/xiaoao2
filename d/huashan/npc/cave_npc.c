...
int checking(object me, object ob)
{
    ...
    if(!living(me) && living(ob) && ob->query("jing")>0 && ob->query("jingli")>0 && ob->query("qi")>0  ){
        ...
        if (me->query("family/family_name")=="日月神教" && me->query_temp("dlsmf/fightnpc"))
        {
		 ...
		 if (me->query("family/family_name")=="日月神教" && me->query_temp("tmgf/killnpc"))
        {
        
		   log_file("quest/tmgfjueji", sprintf("%s记录：%s(%s)参悟天魔棍法被华山石壁高手打败。失败，经验%d。失败%d。\n", ob->name(1),me->name(1),me->query("id"), me->query("combat_exp"),me->query("tmgf/jueji/fail")) );
		   me->set("jing",100);
           me->set("jingli",100);
		 ...
		 if (me->query("family/family_name")=="嵩山派" && me->query_temp("ssjf/killnpc"))
        {
        
		   log_file("quest/ssjfjueji", sprintf("%s记录：%s(%s)参悟嵩山剑法被华山石壁高手打败。失败，经验%d。失败%d。\n", ob->name(1),me->name(1),me->query("id"), me->query("combat_exp"),me->query("ssjf/jueji/fail")) );
		   me->set("jing",100);
           me->set("jingli",100);
           me->add("ssjf/jueji/fail",1);
           me->set("ssjf/jueji/time",time());
           me->delete_temp("ssjf/killnpc");
		 }
		 // 预留嵩山绝技出口
...
void do_lost()
{
	...
	if (me->query("family/family_name")=="日月神教"&&me->query_temp("hssb/juejiasked"))
	    log_file("quest/dlsmfjueji", sprintf("%s记录：%s(%s)解密天魔棍绝技时被高手打败。经验%d，失败%d。\n", ob->name(1),me->name(1),me->query("id"), me->query("combat_exp"),me->query("tmgf/jueji/fail")) );
    if (me->query("family/family_name")=="嵩山派"&&me->query_temp("hssb/juejiasked"))
	    log_file("quest/dlsmfjueji", sprintf("%s记录：%s(%s)解密大力神魔斧绝技时被高手打败。经验%d，失败%d。\n", ob->name(1),me->name(1),me->query("id"), me->query("combat_exp"),me->query("dlsmf/jueji/fail")) );
