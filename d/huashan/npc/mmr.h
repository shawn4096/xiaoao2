...
void unconcious()
{
	...
	if ( me->query("cure") < me->query("max_cure")-2 && me->query("combat_exp") > 2500000){
		...
		me->receive_curing("jing", to_int(me->query("max_jing")/5));
		me->add("qi", to_int(me->query("max_qi")/5));
...
void die()
{
	...
	if (!me) return;
	if ( me->query("cure") < me->query("max_cure")-2 && me->query("combat_exp") > 2500000){
		me->receive_curing("qi", to_int(me->query("max_qi")/5));
		me->receive_curing("jing", to_int(me->query("max_jing")));
		me->set("jing", me->query("max_jing"));
		me->add("qi", to_int(me->query("max_qi")/5));
