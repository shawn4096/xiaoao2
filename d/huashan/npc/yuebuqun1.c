...
void attempt_apprentice(object ob)
{
	...
	ob->set("title",HIG"华山派气宗亲传弟子"NOR);
...
int accept_object(object me, object ling)
{
   ...
   if(done!=2)
       ...
       TASKREWARD_D->get_reward(me,"华山",getshen,0,0,3-random(2),0,0,0,this_object());
...
//edit by sohu
	...
	if (random(me->query("job_time/华山")) >1
		&& random(me->query("job_time/送信")) >1
		//&& random(20)==7
        //&&( (me->query_temp("sj_credit/quest/hs/jiujian")&& random(me->query("kar")) >= 25 )|| random(me->query("kar")) >= 28) //修改之处
		//&&( random(me->query("job_time/丐帮")) >1000 || random(me->query("combat_exp"))>12000000 )
		&& random(me->query("combat_exp"))>10
		&& !me->query("fengset")
		&& !me->query_temp("mb")
        && !me->query("quest/hsjf/fengyi4_time")
		&& (me->query("mianbi") < me->query("combat_exp")/100000 + 1)
...
string ask_fengyi()
{
	...
	if( (time()- me->query("quest/hsjf/fengyi4/time"))<86400)		
     	  return "你来的也太勤快了耶，还是去休息一下吧。\n";
	if(me->query("quest/hsjf/fengyi4"))
	{
